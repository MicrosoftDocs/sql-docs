...
There are two non-interactive authentication models for Azure AD, which can be used in many different applications (ADO.NET, JDCB, ODBC, and so on). These two methods never result in pop-up dialog boxes:
