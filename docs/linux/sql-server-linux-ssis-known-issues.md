...
| Flat File source and destination | 1. Only support Windows-style file paths, to which the default path mapping rule is applied. For example `D:\home\ssis\travel.csv` becomes `/home/ssis/travel.csv`. 2. Azure File share that mounted on Linux is not supported. |
