...
| **DATEADD number allows bigint** | November 2025 | For `DATEADD (datepart, number, date)`, number can be expressed as a **bigint**. For more information, see [DATEADD](/sql/t-sql/functions/dateadd-transact-sql). |
...
| **UNISTR (Transact-SQL)** | July 2025 | Azure SQL Database now supports the `UNISTR` T-SQL syntax for Unicode string literals. For more information, see [UNISTR](/sql/t-sql/functions/unistr-transact-sql). |
...
| Changes | Details |
| --- | --- |
| **AES keys for transparent data encryption preview** | The TDE protector for Azure SQL Database can now use symmetric (AES) keys stored in Azure Key Vault Managed HSM, in addition to asymmetric (RSA) keys. This capability is in public preview. For more information, see [Azure SQL transparent data encryption with customer-managed key](transparent-data-encryption-byok-overview.md). |
