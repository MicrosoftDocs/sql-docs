...
### <a id="QueryStoreOptions"></a> Query Store defaults in [!INCLUDE[ssSDSfull](../../includes/ssazure-sqldb.md)]
...
> [!IMPORTANT]  
> These defaults are automatically applied in the final stage of Query Store activation in an [!INCLUDE[ssSDSfull](../../includes/ssazure-sqldb.md)]. After it's enabled, [!INCLUDE[ssSDSfull](../../includes/ssazure-sqldb.md)] won't change configuration values that are set by customers, unless they negatively impact primary workload or reliable operations of the Query Store.
