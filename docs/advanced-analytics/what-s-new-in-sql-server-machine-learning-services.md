...
|---------|----------------|
| CTP 2.2 | On Windows only, Java code can be now accessed in an external library using the [CREATE EXTERNAL LIBRARY (Transact-SQL)](../t-sql/statements/create-external-library-transact-sql.md) statement. Equivalent functionality will be available on Linux in an upcoming CTP. For more information about this approach, see [How to call Java from SQL Server](java/howto-call-java-from-sql.md). |
| CTP 2.0 | Linux platform support for R and Python machine learning. For help getting started, see [Install SQL Server Machine Learning Services on Linux](../linux/sql-server-linux-setup-machine-learning.md). |
|   | [Java language extension](java/extension-java.md) on both Windows and Linux is supported in SQL Server 2019 preview. By choosing the **Machine Learning Services** feature, the Java extension is  automatically included. Unlike R and Python, there is no explicit **Java** language selection in setup. | 
