...
1. Calculate the space used to store index information in the non-leaf levels of the nonclustered index.
...
1. Account for the data row locator that is required if the index is nonunique:
   ...
   If the nonclustered index is over a clustered index, the data row locator is the clustering key. The columns that must be combined with the nonclustered index key are those columns in the clustering key that aren't already present in the set of nonclustered index key columns.
...
1. Part of the row, known as the null bitmap, might be reserved to manage column nullability. Calculate its size:
...
You can use the following steps to estimate the amount of space that is required to store the leaf level of the index. You' need the values preserved from Step 1 to complete this step.
...
1. Specify the number of fixed-length and variable-length columns at the leaf level and calculate the space that is required for their storage:
   ...
   If the nonclustered index doesn't have any included columns, use the values from Step 1, including any modifications determined in Step 1.3:
   ...
   If the nonclustered index does have included columns, add the appropriate values to the values from Step 1, including any modifications in Step 1.3. The size of a column depends on the data type and length specification. For more information, see [Data types](../../t-sql/data-types/data-types-transact-sql.md).
...
1. Account for the data row locator:

   If the nonclustered index is nonunique, the overhead for the data row locator has already been considered in Step 1.3, and no further modifications are required. Go to the next step.
...
Follow these steps to estimate the amount of space that is required to store the intermediate and root levels of the index. You need the values preserved from steps 2 and 3 to complete this step.
...
1. Calculate the number of non-leaf levels in the index:
   ...
   Round this value up to the nearest whole number. This value doesn't include the leaf level of the nonclustered index.

1. Calculate the number of non-leaf pages in the index:
...
- **Allocation pages**: There's at least one IAM page used to track the pages allocated to a heap, but the space overhead is minimal and there's no algorithm to deterministically calculate exactly how many IAM pages to use.
