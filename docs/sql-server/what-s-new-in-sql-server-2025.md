...
[Get SQL Server 2025](https://info.microsoft.com/ww-landing-sql-server-2025.html).
...
- Increased capacity limits for [!INCLUDE [sssql25-md](../includes/sssql25-md.md)] Standard edition:
  - Maximum compute capacity used by a single instance: limited to lesser of 4 sockets or 32 cores.
  - Maximum memory for buffer pool per instance of [!INCLUDE [sssql25-md](../includes/sssql25-md.md)]: 256 GB.

- [Resource governor](../relational-databases/resource-governor/resource-governor.md) is now available in the Standard and Standard Developer editions, with the same functionality as in the Enterprise edition.
...
Increased capacity limits for [!INCLUDE [sssql25-md](../includes/sssql25-md.md)] Express edition:

- Maximum relational database size: 50 GB.
- Express edition with Advanced Services (SQLEXPRADV) is discontinued.
  - Express edition now includes all features previously available in Express edition with Advanced Services.
...
The following sections identify features that are improved or introduced in [!INCLUDE [sssql25-md](../includes/sssql25-md.md)].
...
| Fast failover for [persistent AG health issues](../database-engine/availability-groups/windows/failover-and-failover-modes-always-on-availability-groups.md#fast-failover-for-persistent-health-issues) | Set the `RestartThreshold` value for an Always On availability group to 0, which tells the Windows Server Failover Cluster (WSFC) to fail over the availability group resource immediately when a persistent health issue is detected. |
...
| [Configure TLS 1.3 encryption with TDS 8.0](../relational-databases/security/networking/connect-with-strict-encryption.md#connect-to-an-always-on-availability-group) | Configure [TLS 1.3](../relational-databases/security/networking/tls-1-3.md) encryption for communication between WSFC and an Always On availability group replica with [TDS 8.0](../relational-databases/security/networking/tds-8.md) support. |
| **Always On failover cluster instance** | |
| [Configure TLS 1.3 encryption with TDS 8.0](../relational-databases/security/networking/connect-with-strict-encryption.md#connect-to-a-failover-cluster-instance) | Configure [TLS 1.3](../relational-databases/security/networking/tls-1-3.md) encryption for communication between WSFC and Always On failover cluster instance (FCI) with [TDS 8.0](../relational-databases/security/networking/tds-8.md) support. |
| **Backups** | |
...
:::image type="content" source="media/what-s-new-in-sql-server-2025/intelligent-query-processing-features.png" alt-text="Diagram representing the features in the intelligent query processing family and when they were introduced in SQL Server." lightbox="media/what-s-new-in-sql-server-2025/intelligent-query-processing-features.png":::
...
| [JARO_WINKLER_DISTANCE](../t-sql/functions/jaro-winkler-distance-transact-sql.md) | Calculates the edit distance between two strings giving preference to strings that match from the beginning for a set prefix length. Returns **float**. Requires [PREVIEW_FEATURES database scoped configuration](../t-sql/statements/alter-database-scoped-configuration-transact-sql.md#preview-features). |
| [JARO_WINKLER_SIMILARITY](../t-sql/functions/jaro-winkler-similarity-transact-sql.md) | Calculates a similarity value ranging from 0 (indicating no match) to 100 (indicating full match). Returns **int**. Requires [PREVIEW_FEATURES database scoped configuration](../t-sql/statements/alter-database-scoped-configuration-transact-sql.md#preview-features). |
...
<a id="fabric-mirroring-for-sql-server-2025-rc-1-preview"></a>
...
For changes related to SQL Server Integration Services, see [What's New in SQL Server 2025 Integration Services](../integration-services/what-s-new-in-integration-services-in-sql-server-2025.md).

## Discontinued services and deprecated features

**Data Quality Services** (DQS) is discontinued in this version of [!INCLUDE [ssNoVersion](../includes/ssnoversion-md.md)]. We continue to support DQS in [!INCLUDE [sssql22-md](../includes/sssql22-md.md)] and earlier versions. For more information, including how to uninstall DQS during an upgrade, see [SQL Server 2025 known issues](sql-server-2025-known-issues.md#upgrade-fails-if-data-quality-services-is-installed).
...
The [Hot add CPU](../relational-databases/thread-and-task-architecture-guide.md#hot-add-cpu) feature is deprecated in this version of [!INCLUDE [ssNoVersion](../includes/ssnoversion-md.md)], and is planned for removal in a future version.
...
For more information, see [Breaking changes to Database Engine features in SQL Server 2025](../database-engine/breaking-changes-to-database-engine-features-in-sql-server-2025.md).
