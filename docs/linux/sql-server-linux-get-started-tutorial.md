...
Get started using SQL Server vNext CTP 1.4 on Linux. Here are basic steps with links to how-to information.
...
> [!NOTE]
> One option is to create use a pre-configured Linux VM in Azure. In addition to OS-only VMs, there is also a VM image with SQL Server vNext CTP 1.4 already installed. For more information, see [Provision a Linux VM in Azure for SQL Server](sql-server-linux-azure-virtual-machine.md). 
