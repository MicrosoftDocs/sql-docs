...
This tutorial demonstrates how to configure a highly available SQL Server instance in a container on AKS. You can also [create a SQL Server availability group on Kubernetes](tutorial-sql-server-ag-kubernetes.md). To compare the two different Kubernetes solutions, see [High availability for SQL Server containers](sql-server-linux-containter-ha-overview.md).
