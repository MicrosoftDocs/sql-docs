...
|---|---|---|---|---|---|
| Persisting credentials via `cmdkey` command | Per Azure-SSIS IR | Execute `cmdkey` command in a custom setup script (`main.cmd`) when provisioning/reconfiguring your Azure-SSIS IR, e.g. `cmdkey /add:fileshareserver /user:xxx /pass:yyy`.<br/><br/> For more info, see [Customize setup for Azure-SSIS IR](https://docs.microsoft.com/en-us/azure/data-factory/how-to-configure-azure-ssis-ir-custom-setup). | Access resources directly in packages via UNC path, e.g.  `\\fileshareserver\folder` | Support multiple credential sets for different connected resources | - File shares on premises/Azure VMs<br/><br/> - Azure Files, see [Use an Azure file share](https://docs.microsoft.com/en-us/azure/storage/files/storage-how-to-use-files-windows) <br/><br/> - SQL Server with Windows Authentication<br/><br/> - Other resources with Windows Authentication |
