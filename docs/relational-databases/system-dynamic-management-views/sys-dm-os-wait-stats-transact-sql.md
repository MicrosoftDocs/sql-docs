...
|CXPACKET <a name="cxpacket"></a>|Occurs with parallel query plans when synchronizing the Query Processor [Exchange Iterator](../../relational-databases/showplan-logical-and-physical-operators-reference.md), and when producing and consuming rows. If waiting is excessive and cannot be reduced by tuning the query (such as adding indexes), consider adjusting the Cost Threshold for Parallelism or lowering the Max Degree of Parallelism (MaxDOP). <br /> **Note:** Starting with [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2 and [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] CU3, CXPACKET only refers to producing rows and synchronizing the Exchange Iterator. Threads consuming rows are tracked separately in the CXCONSUMER wait type. If the consumer threads are too slow, the Exchange Iterator buffer may become full and cause CXPACKET waits. <br /> **Note:** In [!INCLUDE[ssSDS](../../includes/sssds-md.md)], CXPACKET only refers to waits on threads producing rows. Exchange Iterator synchronization is tracked separately in the CXSYNC_PORT and CXSYNC_CONSUMER wait types. Threads consuming rows are tracked separately in the CXCONSUMER wait type.<br /> | 
|CXSYNC_PORT|Occurs with parallel query plans when opening, closing, and synchronizing [Exchange Iterator](../../relational-databases/showplan-logical-and-physical-operators-reference.md) ports between producer and consumer threads. For example, if a query plan has a long sort operation, CXSYNC_PORT waits may be higher because the sort must complete before the Exchange Iterator port can be synchronized. <br /> **Applies to**: [!INCLUDE[ssSDS](../../includes/sssds-md.md)]| 
|CXSYNC_CONSUMER|Occurs with parallel query plans when waiting to reach an [Exchange Iterator](../../relational-databases/showplan-logical-and-physical-operators-reference.md) synchronization point among all consumer threads. <br /> **Applies to**: [!INCLUDE[ssSDS](../../includes/sssds-md.md)]| 
