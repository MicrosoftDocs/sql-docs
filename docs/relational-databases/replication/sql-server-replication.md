...
Azure SQL Database Managed Instance (preview) introduces support for transactional replication. A managed instance can participate in any replication role. It can be a publisher, distributor, or subscriber. For more information, see [Replication on Azure SQL Database SQL Server Managed Instance](http://docs.microsoft.com/azure/sql-database/sql-database-managed-instance).
