...
1. Make a note of the service account that Launchpad is running under. Each instance where R or Python is enabled should have its own instance of the Launchpad service. For example, the service for a named instance might be something like *MSSQLLaunchpad$InstanceName*.

1. If the service is stopped, restart it. On restarting, if there are any issues with configuration, a message is published in the system event log, and the service is stopped again. Check the system event log for details about why the service stopped.

1. Review the contents of RSetup.log, and make sure that there are no errors in the setup. For example, the message *Exiting with code 0* indicates failure of the service to start.

1. To look for other errors, review the contents of rlauncher.log.
...
> [!TIP]  
> If you're familiar with the use of the Support Diagnostics Platform (SDP) tool for SQL Server diagnostics, you can use SDP to review the output file with the name MachineName_UserRights.txt.

## User group for Launchpad can't log on locally

During setup of Machine Learning Services, SQL Server creates the Windows user group **SQLRUserGroup** and then provisions it with all rights necessary for Launchpad to connect to SQL Server and run external script jobs. If this user group is enabled, it's also used to execute Python scripts.

However, in organizations where more restrictive security policies are enforced, the rights that are required by this group might have been manually removed, or they might be automatically revoked by policy. If the rights have been removed, Launchpad can no longer connect to SQL Server, and SQL Server can't call the external runtime.
...
Even if Launchpad is configured correctly, it returns an error if the user doesn't have permission to run R or Python scripts.

If you installed SQL Server as a database administrator or you're a database owner, you're automatically granted this permission. However, other users usually have more limited permissions. If they try to run an R script, they get a Launchpad error.
...
For more information, see [GRANT](../../t-sql/statements/grant-transact-sql.md).
...
If the Windows group for R users (also used for Python) can't log on to the instance that is running R Services, you might see the following errors:
...
> [!NOTE]  
> This limitation doesn't apply if you use SQL logins to run R scripts from a remote workstation.
::: moniker-end
...
1. Right-click SQL Server Launchpad for the instance, and then select **Properties**.

1. Select the **Service** tab, and then verify that the service is running. If it's not running, change the **Start Mode** to **Automatic**, and then select **Apply**.

1. Restarting the service usually fixes the problem so that machine learning scripts can run. If the restart doesn't fix the issue, note the path and the arguments in the **Binary Path** property, and do the following:
...
This error indicates that the account that Launchpad is attempting to use doesn't have permission to log on to the database. This situation can happen when strict security policies are implemented. To determine whether this is the case, review the SQL Server logs, and check to see whether the MSSQLSERVER01 account was denied at login. The same information is provided in the logs that are specific to R_SERVICES or PYTHON_SERVICES. Look for ExtLaunchError.log.
...
- Launchpad might have insufficient external users to run the external query. For example, if you're running more than 20 external queries concurrently, and there are only 20 default users, one or more queries might fail.

- Insufficient memory is available to process the R task. This error happens most often in a default environment, where SQL Server might be using up to 70 percent of the computer's resources. For information about how to modify the server configuration to support greater use of resources by R, see [Python Tutorial: Deploy a linear regression model with SQL machine learning](../tutorials/python-ski-rental-linear-regression-deploy-model.md).

## "Can't find package"

If you run R code in SQL Server and get this message, but didn't get the message when you ran the same code outside SQL Server, it means that the package was not installed to the default library location used by SQL Server.
...
1. Upgrade SQL Server 2016 Cumulative Update 2.
1. Install R Services (In-Database) by using the RTM media.

**The correct way to upgrade:**

1. Install SQL Server 2016 without R Services.
1. Upgrade SQL Server 2016 to the desired patch level. For example, install Service Pack 1 and then Cumulative Update 2.
1. To add the feature at the correct patch level, run SP1 and CU2 setup again, and then choose R Services (In-Database).

## Launchpad fails to start if 8dot3 notation is required

> [!NOTE]  
> On older systems, Launchpad can fail to start if there's an 8dot3 notation requirement. This requirement has been removed in later releases. SQL Server 2016 R Services customers should install one of the following:
>
> - SQL Server 2016 SP1 and CU1: [KB3208177 - Cumulative Update 1 for SQL Server 2016 SP1](/troubleshoot/sql/releases/sqlserver-2016/servicepack1-cumulativeupdate1).
...
If the volume where you're installing R doesn't support short file names, the processes that launch R from SQL Server might not be able to locate the correct executable, and Launchpad will not start.
...
1. To enable 8dot3 notation, run the [fsutil](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/ff621566(v=ws.11)) utility with the `8dot3name` argument.

1. After the 8dot3 notation is enabled, open the RLauncher.config file and note the property of `WORKING_DIRECTORY`. For information about how to find this file, see [Collect data to troubleshoot Python and R scripts with SQL Server Machine Learning Services](data-collection-ml-troubleshooting-process.md).

1. Use the fsutil utility with the *file* argument to specify a short file path for the folder that's specified in WORKING_DIRECTORY.

1. Edit the configuration file to specify the same working directory that you entered in the WORKING_DIRECTORY property. Alternatively, you can specify a different working directory and choose an existing path that's already compatible with the 8dot3 notation.
   ::: moniker-end
