...
In this article, learn about change data capture (CDC) in SQL Server and Azure SQL Managed Instance, which records activity on a database when tables and rows have been modified. 

CDC is implemented differently in Azure SQL Database. To learn more, see [CDC with Azure SQL Database](/azure/azure-sql/database/change-data-capture-overview). 
