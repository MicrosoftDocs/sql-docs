 ...
 **Login-Related Options**  
  **-A**  
 Logs in to [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] with a Dedicated Administrator Connection (DAC). This kind of connection is used to troubleshoot a server. This will only work with server computers that support DAC. If DAC is not available, `sqlcmd` generates an error message and then exits. For more information about DAC, see [Diagnostic Connection for Database Administrators](../database-engine/configure-windows/diagnostic-connection-for-database-administrators.md).  
 ...
 **-i** "C:\Some Folder\\<file name\>"  
 ...
 This option may be used more than once: **-i**_input_file_ **-I**_I input_file._  
