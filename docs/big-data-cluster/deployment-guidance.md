...
- Install the cluster configuration tool Azure Data CLI on your client machine.
...
The first time you run Azure Data CLI you must include `--accept-eula=yes` to accept the end user license agreement (EULA).
...
| `ACCEPT_EULA`| Required for first use of Azure Data CLI| Set to "yes". When set as an environment variable, it applies EULA to both SQL Server and Azure Data CLI. If not set as environment variable, you can include `--accept-eula=yes` in the first use of Azure Data CLI command.|
...
In addition to using Azure Data CLI, you can also use Azure Data Studio to find both endpoints and status information. For more information about viewing cluster status with Azure Data CLI and Azure Data Studio, see [How to view the status of a big data cluster](view-cluster-status.md).
