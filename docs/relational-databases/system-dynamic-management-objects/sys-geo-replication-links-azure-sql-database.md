...
| `secondary_allow_connections` |**tinyint**|The secondary type, one of:<br /><br /> 0 = No. The secondary database is not accessible until failover.<br /><br /> 1 = ReadOnly. The secondary database is accessible only to client connections with [ApplicationIntent=ReadOnly](/azure/azure-sql/database/read-scale-out?view=azuresql-db&preserve-view=true#connect-to-a-read-only-replica).<br /><br /> 2 = All. The secondary database is accessible to any client connection.|  
...
- [sys.dm_geo_replication_link_status (Azure SQL Database)](sys-dm-geo-replication-link-status-azure-sql-database.md?view=azuresqldb-current&preserve-view=true)
- [sys.dm_operation_status (Azure SQL Database)](sys-dm-operation-status-azure-sql-database.md?view=azuresqldb-current&preserve-view=true)
