...
-   The buffer pool extension size can be up to 32 times the value of max_server_memory for Enterprise editions, and up to 4 times for Standard edition.  We recommend a ratio between the size of the physical memory (max_server_memory) and the size of the buffer pool extension of 1:16 or less. A lower ratio in the range of 1:4 to 1:8 may be optimal. For information about setting the max_server_memory option, see [Server Memory Server Configuration Options](../../2014/database-engine/server-memory-server-configuration-options.md).  
...
|Monitor the buffer pool extension|[sys.dm_os_buffer_descriptors &#40;Transact-SQL&#41;](../Topic/sys.dm_os_buffer_descriptors%20\(Transact-SQL\).md)<br /><br /> [Performance counters](../../2014/database-engine/sql-server-buffer-manager-object.md)|  
