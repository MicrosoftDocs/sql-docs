...
|is_azure_user_plan|**tinyint** | 1 for queries executed in an [!INCLUDE[ssSDSfull](../../includes/ssazure-sqldb.md)] from a session initiated by a user. <BR><BR>0 for queries that have been executed from a session not initiated by an end user, but by applications running from within Azure infrastructure that issue queries for other purposes of collecting telemetry or executing administrative tasks. Customers are not charged for resources consumed by queries where is_azure_user_plan = 0.<BR><BR>**[!INCLUDE[ssSDSfull](../../includes/ssazure-sqldb.md)]** only.|
