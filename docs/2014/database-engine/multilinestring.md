...
### Accepted instances  
 For a `MultiLineString` instance to be accepted it must either be empty or comprised of only `LineString` intances that are accepted. For more information on accepted `LineString` instances, see [LineString](../../2014/database-engine/linestring.md). The following are examples of accepted `MultiLineString` instances.  
