...
|Check the database permissions|The database will not appear offline in the SQL Server Management Studio, and other users, for example the DBA will be able to connect to it. <br/> The user account in question will need to be granted explicit access to the database or be added to a SQL Server role or a local Windows group or domain group that has access to the database.|
