...
```  
 ...
 To enable [!INCLUDE[ssSB](../../../includes/sssb-md.md)] in a database, use the ENABLE_BROKER SET option of the [ALTER DATABASE](../Topic/ALTER%20DATABASE%20\(Transact-SQL\).md) statement.  
...
```  
 ...
 If a WQL query specifies an event notification scoped at the server level, for example by issuing the query SELECT * FROM ALTER_TABLE, the calling application must have the server-level CREATE DDL EVENT NOTIFICATION permission. Note that server-scoped event notifications are stored in the master database. You can use the [sys.server_event_notifications](../Topic/sys.server_event_notifications%20\(Transact-SQL\).md) catalog view to see their metadata.  
