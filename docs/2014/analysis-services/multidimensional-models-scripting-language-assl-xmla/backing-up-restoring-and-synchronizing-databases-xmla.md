...
### Restoring Remote Partitions  
 For each remote backup file created during a previous `Backup` command, you can restore its associated remote partition by including a `Location` element in the `Locations` property of the `Restore` command. The [DataSourceType](../xmla/xml-elements-properties/type-element-xmla.md) property for each `Location` element must be excluded or explicitly set to *Remote*.  
