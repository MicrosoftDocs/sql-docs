...
> [!IMPORTANT]  
> A relationship chain cannot pass through a secured table. For example, if there is a relationship between tables A and B, and B and C, you cannot secure table B. If table B were secured, a query on table A could not transit the relationship between table A and B, and table B and C.
...
Similar to table-level security, with table-level security you can not only restrict access to column data, but also sensitive column names, helping prevent malicious users from discovering if a column exists. Access to the column’s metadata and data is restricted.
