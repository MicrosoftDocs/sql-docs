...
- Used in statistics generated by the CREATE STATISTICS statement, unless the column is a **varchar**, **nvarchar**, or **varbinary** data type, the data type isn't changed, and the new size is equal to or greater than the old size or if the column is changed from not null to null.  

Use this query to see all the statistics and statistics columns for a table.
...
```
To remove the user statistics, use DROP STATISTICS statement.
