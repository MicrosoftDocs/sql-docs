...
|CXPACKET <a name="cxpacket"></a>|Occurs with parallel query plans when synchronizing the Query Processor [Exchange Iterator](../../relational-databases/showplan-logical-and-physical-operators-reference.md), and when producing and consuming rows. If waiting is excessive and cannot be reduced by tuning the query (such as adding indexes), consider adjusting the Cost Threshold for Parallelism or lowering the Max Degree of Parallelism (MaxDOP). <br /> **Note:** Starting with [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2 and [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] CU3, CXPACKET only refers to synchronizing the Exchange Iterator and when producing rows. Threads consuming rows are tracked separately in the CXCONSUMER wait type. If the consumer threads are too slow, the Exchange Iterator buffer may become full and cause CXPACKET waits. <br /> **Note:** In [!INCLUDE[ssSDS](../../includes/sssds-md.md)], CXPACKET only refers to waits on threads producing rows. Exchange Iterator synchronization is tracked separately in the CXSYNC_PORT and CXSYNC_CONSUMER wait types. Threads consuming rows are tracked separately in the CXCONSUMER wait type.<br /> | 
|CXSYNC_PORT|Occurs with parallel query plans when opening and closing [Exchange Iterator](../../relational-databases/showplan-logical-and-physical-operators-reference.md) ports between producer and consumer threads. <br /> **Applies to**: [!INCLUDE[ssSDS](../../includes/sssds-md.md)]| 
