...
### Impact of changing service tier or scaling compute size
...
### Latency of changing service tier or scaling compute size
...
### Additional considerations when changing service tier or scaling compute size

- When you reduce vCores or eDTUs for an elastic pool, the pool used space must be smaller than the max size limit of the target service tier and pool compute.
- When you increase eDTUs for an elastic pool, an extra storage cost might apply if:
  - The max size of the pool is supported by the target pool, and
  - The max size of the pool exceeds the included storage amount of the target pool.
- For example, if a 100 eDTU Standard pool with a max size of 100 GB is downsized to a 50 eDTU Standard pool, then an extra storage cost applies since target pool supports a max size of 100 GB and its included storage amount is only 50 GB. So, the extra storage amount is 100 GB – 50 GB = 50 GB. For pricing of extra storage, see [SQL Database pricing](https://azure.microsoft.com/pricing/details/sql-database/). If the actual amount of space used is less than the included storage amount, then this extra cost can be avoided by reducing the max size to the included amount.

### Billing during scaling
...
The Storage size (max size) for the elastic pool can be specified using the [Azure portal](elastic-pool-manage.md#azure-portal), [PowerShell](/powershell/module/az.sql/Get-AzSqlElasticPool), the [Azure CLI](/cli/azure/sql/elastic-pool#az-sql-elastic-pool-update), or the [REST API](/rest/api/sql/elasticpools/update). When you increase the max size of the elastic pool, the value specified cannot exceed the max size limit specified for the pool's service objective. When decreasing the max size, the new value specified has to be equal to or greater than the summation of space allocated across all databases in the pool.
...
- The storage size (max size) for elastic pools in the General Purpose or Business Critical tiers can be specified up to the max size limits specified in [Resource limits for elastic pools using the vCore purchasing model](resource-limits-vcore-elastic-pools.md). The max size for the elastic pool can be increased or decreased in multiples of 1GB.
- The price of storage for an elastic pool is the max size specified, multiplied by the storage unit price of the service tier. For details on the price of extra storage, see [SQL Database pricing](https://azure.microsoft.com/pricing/details/sql-database/).
...
- The eDTU price for an elastic pool includes a certain amount of storage at no additional cost. Extra storage beyond the included amount can be provisioned for an additional cost up to the max size limit corresponding to the eDTUs provisioned. For included storage amounts and max size limits, see [Resources limits for elastic pools using the DTU purchasing model](resource-limits-dtu-elastic-pools.md#elastic-pool-storage-sizes-and-compute-sizes).
...
- Valid values for the max size for a Standard or Premium tier elastic pool should be one of these values: 50 GB, 100 GB, 150 GB, 200 GB, 250 GB, 300 GB, 400 GB, 500 GB, 750 GB, 800 GB, 1024 GB, 1200 GB, 1280 GB, 1536 GB, 1600 GB, 1792 GB, 2000 GB, 2048 GB, 2304 GB, 2500 GB, 2560 GB, 2816 GB, 3000 GB, 3072 GB, 3328 GB, 3584 GB, 3840 GB, 4096 GB. The max size specified cannot exceed the max size limit specified for the eDTUs provisioned.
