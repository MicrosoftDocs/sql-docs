---
title: "Use Date and Time Types"
description: Learn how to initialize the date/time data structures that were added in SQL Server 2008.
...
This sample shows how to initialize the date/time data structures that were added in [!INCLUDE [sql2008-md](../../includes/sql2008-md.md)]. It then prepares the input values, binds parameters, and executes the query. For more information about using these types, see [Date and Time Improvements (ODBC)](../native-client-odbc-date-time/date-and-time-improvements-odbc.md).

## Examples

You need an ODBC data source called `DateTime`. The default database for `DateTime` should be `tempdb`. This data source must be based on the ODBC driver for [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] Native Client.

If you build and run this sample as a 32-bit application on a 64-bit operating system, you must create the ODBC data source with the ODBC Administrator in `%windir%\SysWOW64\odbcad32.exe`.

This sample connects to your computer's default [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] instance. To connect to a named instance, change the definition of the ODBC data source to specify the instance using the following format: `<server>\<named-instance>`. By default, [!INCLUDE [ssExpress](../../includes/ssexpress-md.md)] installs to a named instance.

### A. Create table

This [!INCLUDE [tsql](../../includes/tsql-md.md)] code creates a table used by this sample.
...
(
    ...
    datetimeoffsetcol DATETIMEOFFSET (7)
);
GO
```
...
Compile this C++ code listing with `odbc32.lib` and `user32.lib`. Make sure your `INCLUDE` environment variable includes the directory that contains `sqlncli.h`.
...
This [!INCLUDE [tsql](../../includes/tsql-md.md)] code deletes the table used by this sample.
