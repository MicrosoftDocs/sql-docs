...
2. Whether sufficient worker worker threads are available.  
  Each query or index operation requires a certain number of worker worker threads to execute. Executing a parallel plan requires more worker worker threads than a serial plan , and the number of required worker threads increases with the degree of parallelism. When the worker thread requirement of the parallel plan for a specific degree of parallelism cannot be satisfied, the [!INCLUDE[ssDEnoversion](../includes/ssdenoversion-md.md)] decreases the degree of parallelism automatically or completely abandons the parallel plan in the specified workload context. It then executes the serial plan (one worker thread). 
...
![worker thread1](../relational-databases/media/worker thread1.gif)
...
Shows worker thread reassigned after it finishes
If the number of worker threads is equal to the number of partitions, the query processor assigns one worker thread to each partition. When a worker thread finishes, it is not reallocated to another partition.  
![worker thread2](../relational-databases/media/worker thread2.gif)  
...
![worker thread3](../relational-databases/media/worker thread3.gif)  
