...
|**DatabaseName**|`nvarchar`|Name of the database in which the lock acquire was attempted.|35|Yes|  
|**Duration**|`bigint`|Amount of time (in microseconds) between the time the lock request was issued and the time the lock was canceled.|13|Yes|  
|**EndTime**|`datetime`|Time at which the event ended.|15|Yes|  
|**EventClass**|`int`|Type of event = 26.|27|No|  
