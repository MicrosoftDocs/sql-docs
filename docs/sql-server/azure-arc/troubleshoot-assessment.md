...
While performing any operation related to assessment, if such a notification appears and the user keeps the page open, the operation is automatically re-triggered after five minutes. If the page is refreshed, the user is advised to wait for five minutes before retrying the operation. In case the same error persists after a long time, verify the state of the **WindowsAgent.SqlServer** extension and ensure that the extension is not stuck in an **Updating** state. In case the extension is still stuck, verify the connectivity of the Arc machine.

## Asssessment run failed

In case the assessment run fails, click on the "Failed" hyperlink which should open a blade that displays the error message.
...
2. AMA upload failed
    ...
    - Verify that the Azure Monitor Agent is provisioned correctly and verify that the rest of the setup has not been deleted. The following components must be configured correctly to ensure that the agent can upload logs to the workspace:
        ...
        6. **SqlAssessment_CL** with above LA Workspace must be present in the **Data Sources** tab for Data Collection Rule.
