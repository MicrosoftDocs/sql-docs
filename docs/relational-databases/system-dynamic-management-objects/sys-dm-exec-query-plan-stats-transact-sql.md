...
description: sys.dm_exec_query_plan_stats returns the equivalent of the last known actual execution plan for a previously cached query plan.
author: rwestMSFT
...
| `number` | **smallint** | Numbered stored procedure integer. For example, a group of procedures for the `orders` application might be named `orderproc;1`, `orderproc;2`, and so on. For ad hoc and prepared batches, this column is **null**.<br /><br />Column is nullable. |
| `encrypted` | **bit** | Indicates whether the corresponding stored procedure is encrypted.<br /><br />0 = not encrypted<br />1 = encrypted<br /><br />Column isn't nullable. |
...
- The plan can be found in [sys.dm_exec_cached_plans](sys-dm-exec-cached-plans-transact-sql.md).
...
- The query plan wasn't cacheable in the first place. For more information, see [Execution Plan Caching and Reuse](../query-processing-architecture-guide.md#execution-plan-caching-and-reuse).
