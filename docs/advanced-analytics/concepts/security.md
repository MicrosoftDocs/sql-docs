...
manager: cgronlun
---
# Security overview for the extensiblity framework in SQL Server Machine Learning Services
...
+ Permission to access the database where external scripts is enabled.
...
Therefore, each person who runs an external script using SQL Server as the execution context must be mapped to a login in the database. Under SQL Server security, it is generally easiest to create roles to manage sets of permissions, and assign users to those roles, rather than individually set user permissions.
...
## [!INCLUDE[rsql_launchpad_md](../../includes/rsql-launchpad-md.md)] service
...
During setup of Machine Learning Services, new Windows user accounts are created to support execution of R or Python script tasks under the security token of the [!INCLUDE[rsql_launchpad_md](../../includes/rsql-launchpad-md.md)] service.
