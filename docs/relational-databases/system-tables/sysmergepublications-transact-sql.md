...
|**max_concurrent_merge**|**int**|The maximum number of allowed concurrent merge processes. A value of **0** for this property means that there is no limit to the number of concurrent merge processes running at any given time. This property sets a limit as to the number of concurrent merge processes that can be run against a merge publication at one time. If there are more snapshot processes scheduled at the same time than the value allows, then the excess jobs will be put into a queue and wait until a currently-running merge process finishes.|  
