...
> [!IMPORTANT]  
>  Expressions are deterministic if they always return the same result for a specified set of inputs. The **IsDeterministic** property of the [COLUMNPROPERTY](../Topic/COLUMNPROPERTY%20\(Transact-SQL\).md) function reports whether a *computed_column_expression* is deterministic.  
...
-   The ANSI_NULLS connection-level option must be set to ON when the CREATE TABLE or ALTER TABLE statement that defines the computed column is executed. The [OBJECTPROPERTY](../Topic/OBJECTPROPERTY%20\(Transact-SQL\).md) function reports whether the option is on through the **IsAnsiNullsOn** property.  
