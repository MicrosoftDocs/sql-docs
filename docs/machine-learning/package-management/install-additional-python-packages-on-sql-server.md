...
+ Before adding a package, consider whether the package is a good fit for the SQL Server environment.
  ...
  > [!NOTE]
  > The Python package **tensorflow** cannot be installed using sqlmlutils. For more information and a workaround, see [Known issues in SQL Server Machine Learning Services](../known-issues-for-sql-server-machine-learning-services.md#9-cannot-install-tensorflow-package-using-sqlmlutils) 

Use the following commands from an administrator level command prompt, replacing "MSSQLSERVER" with the name of your SQL instance (if you get a "TLS/SSL" error, see [Known issues in SQL Server Machine Learning Services](../known-issues-for-sql-server-machine-learning-services.md#7-unable-to-install-python-packages-using-pip-after-installing-sql-server-2019-on-windows)).
  > ```cmd
