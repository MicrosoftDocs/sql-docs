...
#### REQUEST_MAX_MEMORY_GRANT_PERCENT = *value*     
 Specifies the maximum amount of memory that a single request can take from the pool. *value* is a percentage relative to the resource pool size specified by MAX_MEMORY_PERCENT. Default value is 25. The amount specified only refers to query execution grant memory.  
...
> It is not recommended to set *value* greater than 70 because the server may be unable to set aside enough free memory if other concurrent queries are running. This may eventually lead to query time-out error 8645.      
 ...
 Be aware that both cases are subject to [time-out error 8645](/sql/relational-databases/errors-events/mssqlserver-8645-database-engine-error.md) if the server has insufficient physical memory.  
