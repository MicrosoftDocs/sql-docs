...
- When you first create a database using the FULL recovery model, the transaction log will be reused as needed (similar to a SIMPLE recovery database), up until the time you create a full database backup.
 ...
 For more information, see [Factors that can delay log truncation](#FactorsThatDelayTruncation), later in this article.  
...
Really, log truncation can be delayed by various reasons. Learn what, if anything, is preventing your log truncation by querying the **log_reuse_wait** and **log_reuse_wait_desc** columns of the [sys.databases](../../relational-databases/system-catalog-views/sys-databases-transact-sql.md) catalog view. The following table describes the values of these columns.  
