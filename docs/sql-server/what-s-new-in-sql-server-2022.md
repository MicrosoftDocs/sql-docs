...
| Reduced buffer pool I/O promotions | Reduced the incidents of a single page being promoted to 8 pages when populating the buffer pool from storage, causing unnecessary I/O. The buffer pool can be populated more efficiently. |
| Enhanced spinlock algorithms | Spinlocks are a huge part of the consistency inside the engine for multiple threads. Internal adjustments to the Database Engine introduced in [!INCLUDE[sssql22-md](../../includes/sssql22-md.md)] make spinlocks more efficient. |
...
| Instant file initialization for transaction log file growth events | In general, transaction log files cannot benefit from instant file initialization (IFI). Starting with [!INCLUDE[sssql22-md](../../includes/sssql22-md.md)], instant file initialization can benefit transaction log *growth events* up to 64 MB. The default auto growth size increment for new databases is 64 MB. | 
