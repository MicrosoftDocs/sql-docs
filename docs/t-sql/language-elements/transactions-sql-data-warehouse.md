...
description: Learn about transactions in Azure Synapse Analytics and how the AUTOCOMMIT option affects tranactions.
...
[!INCLUDE [applies-to-version/asa-pdw](../../includes/applies-to-version/asa-pdw.md)]
 ...
 > [!NOTE]
 > The `SET AUTOCOMMIT` command is not supported in Microsoft Fabric Data Warehouse. For more information on transactions, isolation, concurrency, and consistency in [!INCLUDE [fabric](../../includes/fabric.md)] Data Warehouse, instead see [Transactions in Fabric Data Warehouse](/fabric/data-warehouse/transactions).
...
- *Auto-commit transactions* initiate automatically within a session and do not start with the `BEGIN TRANSACTION` statement. 
   - When the `AUTOCOMMIT` setting is `ON`, each statement runs in a transaction and no explicit `COMMIT` or `ROLLBACK` is necessary. 
   - When the `AUTOCOMMIT` setting is `OFF`, a `COMMIT` or `ROLLBACK` statement is required to determine the outcome of the transaction. Autocommit transactions begin immediately after a `COMMIT` or `ROLLBACK` statement, or after a `SET AUTOCOMMIT OFF` statement.
...
If you change the `AUTOCOMMIT` setting within an active transaction, the setting does affect the current transaction and does not take effect until the transaction is completed.
...
## Error handling
 ...
 If an error other than a run-time statement error prevents the successful completion of an explicit transaction, the database engine automatically rolls back the transaction and frees all resources held by the transaction. For example, if the client's network connection is broken or the client signs out of the application, any uncommitted transactions for the connection are rolled back when the network notifies the instance of the break.  
...
## Limitations
 ...
 The [CREATE DATABASE](../statements/create-database-transact-sql.md) and [DROP DATABASE (Transact-SQL)](../statements/drop-database-transact-sql.md) commands cannot be used inside an explicit transaction.  
...
<a id="a-using-an-explicit-transaction"></a>
...
<a id="b-rolling-back-a-transaction"></a>

### B. Roll back an open transaction

 The following example shows the effect of rolling back a transaction. In this example, the `ROLLBACK` statement rolls back the `INSERT` statement, but the created table will still exist.  
...
<a id="d-using-an-implicit-multi-statement-transaction"></a>

### D. Use an implicit multi-statement transaction
