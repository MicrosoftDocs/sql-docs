---
title: "Common Errors with Database Mail"
description: "Learn how to diagnose and troubleshoot common errors with Database Mail."
author: MashaMSFT
ms.author: mathoma
ms.date: 05/16/2025
ms.service: sql
...
Use [Configure database mail](configure-database-mail.md) to enable and configure database mail.
...
Database Mail relies on a number of internal stored procedures. To reduce the surface area, these stored procedures are disabled on new installation of SQL Server. To enable these stored procedures, use the [Database Mail XPs (server configuration option)](../../database-engine/configure-windows/database-mail-xps-server-configuration-option.md) of the `sp_configure` system stored procedure, as in the following example:
...
Database Mail might be stopped in the `msdb` database. To check status of Database Mail, execute the following statement:
