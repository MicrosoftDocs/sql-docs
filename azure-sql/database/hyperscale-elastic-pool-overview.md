...
| *-- | *-- |
| When adding a non-Hyperscale database to a Hyperscale elastic pool, you may receive an error `{"code":"ElasticPoolDatabaseCountOverLimit","message":"The elastic pool 'MyHyperscaleElasticPool' has reached its database count limit. The database count for the elastic pool cannot exceed (25) for service tier 'Hyperscale'."}` even though there are only 24 databases in the Hyperscale elastic pool. This issue is due to a code defect. | To work around the issue, you can convert the non-Hyperscale database to a *standalone* Hyperscale database first before adding it to the Hyperscale elastic pool. |
