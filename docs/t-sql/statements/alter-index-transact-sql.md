...
> [!NOTE]
> The DDL command runs until it completes, pauses or fails. In case the command pauses, an error will be issued indicating that the operation was paused and that the index creation did not complete. More information about the current index status can be obtained from [sys.resumable_index_operations](../../relational-databases/system-catalog-views/sys-index_resumable_operations). As before in case of a failure an error will be issued as well. 
