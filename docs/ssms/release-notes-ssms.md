...
| Compatibility Mode | Added support for compatibility mode 160. **SSMS 19** is the recommended SSMS release for use with [!INCLUDE [sssql22-md](../includes/sssql22-md.md)]. Visit [Download SQL Server Management Studio 19](download-sql-server-management-studio-ssms-19.md) to obtain SSMS 19. |
...
| Integration Services | Fixed an issue that caused the error "System.ArgumentException: Value doesn't fall within the expected range" that would be encountered unexpectedly at step "Changing protection level" when deploying SSIS project. |
...
| Data Classification | Updated the Add Classification pane to allow users to add new classifications for columns that aren't yet classified. |
...
| General SSMS | Fixed an issue where SSMS could show an error dialog with an unhelpful message (for example "syntax error ( *EngineEdition != 11)" ). |
...
| Profiler | Fixed an issue where SSMS wasn't able to start SQL Profiler when spaces existed between the server name and its port number. |
...
| XEvents | Fixes issue where long string doesn't include ellipsis at end when truncated in display |
...
| Auditing | Fixed an issue were some Audit Specifications wizards and property dialogs weren't working properly on some localized versions of SSMS (for example, Create, Japanese). |
...
| General SSMS | Fixed a crash (in the Visual Studio Shell) that was causing SSMS to crash when redocking tabs. See [SQL Server user feedback](https://feedback.azure.com/forums/908035-sql-server/suggestions/42651556-ssms-18-8-crashes-when-re-docking-tabs) |
...
| Log Shipping Wizard | Fixed in issue where Log Shipping Wizard may throw an error "Can't show requested dialog" when selecting on Edit Job for Transaction Log Backup/Copy/Restore Setting Tab. |
| Log Viewer | Addressed an issue where on some locales (for example, Korean) some UI elements looked truncated/clipped. |
...
| General SSMS | SSMS Extensions using SMO should be recompiled targeting the new SSMS-specific SMO v161 package. A preview version is available at https://www.nuget.org/packages/Microsoft.SqlServer.SqlManagementObjects.SSMS/<br /><br />Extensions compiled against previous 160 versions of Microsoft.SqlServer.SqlManagementObjects package will still function. | N/A |
| Integration Services | When importing or exporting packages in Integration Services or exporting packages in Azure-SSIS Integration Runtime, scripts are lost for packages containing script tasks/components. | Remove folder "C:\Program Files (x86)\Microsoft SQL Server Management Studio 18\Common7\IDE\CommonExtensions\MSBuild". |
| Integration Services | Remote connections to Integration services may fail with "The specified service doesn't exist as an installed service." on newer Operating system. | Identify the Integration services-related registry location under Computer\HKEY_CLASSES_ROOT\AppID & Computer\HKEY_CLASSES_ROOT\ WOW6432Node\AppID and within these hives, rename the registry key named 'LocalService' to 'LocalService_A' for the specific version of Integration services that we're trying to connect |
...
| New Item | Details |
| --- | --- |
| Accessibility | Fixed tab ordering of buttons in the "The following settings aren't supported by the database" window that pops up when querying against DW. |
...
| Database Diagrams | Fixed an issue that could cause SSMS to crash or stop responding when using Database Diagrams where "Add Table" dialog wasn't properly displayed. |
...
| Integration Services | Remote connections to Integration services may fail with "The specified service doesn't exist as an installed service." on newer Operating system. Workaround: Identify the Integration services-related registry location under Computer\HKEY_CLASSES_ROOT\AppID & Computer\HKEY_CLASSES_ROOT\ WOW6432Node\AppID and within these hives, rename the registry key named 'LocalService' to 'LocalService_A' for the specific version of Integration services that we're trying to connect | N/A |
...
| General SSMS | Fixed an issue on the **Statistic Properties** dialog were selecting the **Update statistics for these columns** checkbox and selecting **OK** yields no effect. Statistics aren't updated, and trying to script the action yields a *There's no action to be scripted* message). See [SQL Server user feedback](https://feedback.azure.com/forums/908035/suggestions/37799992). |
...
| Data Classification | We're now enforcing different GUIDs for every label/infoType and GUID's structure in the validation process. |
...
- Database Diagram created from SSMS running on machine A can't be modified from machine B (SSMS crashes). See [SQL Server user feedback 37992649](https://feedback.azure.com/forums/908035/suggestions/37992649) for more details.
...
| SQL Agent | Fixed an issue where the agent operator UI wouldn't update the operator name when it was changed in the UI, nor is it scripted. See [SQL Server user feedback](https://feedback.azure.com/forums/908035/suggestions/32897647) for more details. |

#### Known issues (18.4)

- Database Diagram created from SSMS running on machine A can't be modified from machine B (SSMS crashes). See [SQL Server user feedback](https://feedback.azure.com/forums/908035/suggestions/37992649) for more details.
...
| Query Execution or Results | Allow more data to be displayed (Result to Text) and stored in cells (Result to Grid). SSMS now allows up to 2M characters for both (up from 256 K and 64 K, respectively). This also addressed the issue of users not able to grab more than 43,680 chars from the cells of the grid. |
...
- Database Diagram created from on an SSMS running on machine A can't be modified from machine B (it would crash SSMS). See [SQL Server user feedback](https://feedback.azure.com/forums/908035/suggestions/37992649) for more details.
...
| General SSMS | Fixed an issue in the Login properties page where selecting on the *Add Credential* button could throw a Null Reference Exception. |
...
- Users may see an error when dragging a table object from the Object Explorer into the Query Editor. We're aware of the issue, and the fix is planned for the next release.

- The *Group connections* and *Single-server connections* color options under the Options -> Text Editor -> Editor Tab and Status Bar -> Status Bar Layout and Colors don't persist after closing SSMS 18.1. After you reopen SSMS, the Status Bar Layout and Colors option revert to default (white).

- There's a limitation on the size of the data you see from SSMS results shown in the grid, text, or file.
...
| Support for SQL Server 2019 | Always Encrypted: connection dialog has a new tab "Always Encrypted" when the user selects on the "Options" button to enable/configure Enclave support. |
...
| SSMS isn't supported on Windows 8. Windows 10 and Windows Server 2016 require version 1607 (10.0.14393) or later | Due to the new dependency on NetFx 4.7.2, SSMS 18.0 doesn't install on Windows 8 and older versions of Windows 10 and Windows Server 2016. SSMS setup blocks those systems. Windows 8.1 is still supported. |
...
| ShowPlan | Modified tooltip and added comment when selecting on Edit Query Button for a ShowPlan, to indicate to user that the ShowPlan might be truncated by the SQL engine if the query is over 4000 characters. |
...
| Backup/Restore/Attach/Detach DB | Fixed an issue where the user was unable to attach a database when physical filename of .mdf file doesn't match the original filename. |
...
| Backup/Restore/Attach/Detach DB | Fixed issue where the "Attach Database" wizard wasn't displaying secondary files that were renamed. Now, the file is displayed, and a comment about it's added (for example "Not Found"). For details, see [https://feedback.azure.com/forums/908035/suggestions/32897434](https://feedback.azure.com/forums/908035/suggestions/32897434). |
...
| Flat File Import Wizard | Fixed issue where Flat File Import doesn't allow changing destination table when table is already existing. For details, see [https://feedback.azure.com/forums/908035-sql-server/suggestions/32896186](https://feedback.azure.com/forums/908035-sql-server/suggestions/32896186). |
...
| Object Explorer | Fixed an issue where SSMS was throwing an "Object can't be cast from DBNull to other types" exception when trying to expand "Management" node in OE (misconfigured DataCollector). |
...
| Object Explorer | Disabled the "Design" context-menu item on "Graph Tables" since there's no support for those tables in the current version of SSMS. |
...
| Object Explorer | Fixed/improved the way an issue where a database size ("Size (MB)") is displayed in Object Explorer details: only two decimal digits and formatted using the thousands separator. For details, see [https://feedback.azure.com/forums/908035/suggestions/34379308](https://feedback.azure.com/forums/908035/suggestions/34379308). |
...
| Results Grid | Fixed an issue, which resulted in an "Index out of range" exception when selecting on the grid. |
...
| SSMS Editor | Fixed an issue where selecting on editor tabs could cause the tab to be closed instead of getting the focus. For details, see [https://feedback.azure.com/forums/908035/suggestions/37291114](https://feedback.azure.com/forums/908035/suggestions/37291114). |
...
- DMF Standard Policies
  - The policies aren't installed with SSMS anymore. They're moved to Git. Users are able to contribute and download/install them, if they want to.
