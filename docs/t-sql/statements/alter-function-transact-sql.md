...
dev_langs:
  - "TSQL"
monikerRange: ">=aps-pdw-2016 || =azuresqldb-current || >=sql-server-2017 || >=sql-server-linux-2017 || =azuresqldb-mi-current || =azure-sqldw-latest || =fabric || =fabric-sqldb"
---
...
Supported only for natively compiled, scalar user-defined functions, and is required. For more information, see [Atomic Blocks in Native Procedures](../../relational-databases/in-memory-oltp/atomic-blocks-in-native-procedures.md).
...
For a list of conditions that must be met before a function can be schema bound, see [CREATE FUNCTION (Transact-SQL)](create-function-transact-sql.md).
