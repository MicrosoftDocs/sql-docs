...
# Working with R Data Types
  Whereas [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] supports several dozen  data types, R has a limited number of scalar data types (numeric, integer, complex, logical, character, date/time and raw). When you use data from  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in R scripts, sometimes data is implicitly converted to a compatible data type, but more often an exact conversion cannot be performed automatically, and an error is returned, such as "Unhandled SQL data type".  
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] type|R class|RESULT SET type|Comments|  
|-|-|-|-|  
