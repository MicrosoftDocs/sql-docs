...
| **Backup storage redundancy** | Select **Geo-redundant backup storage**. | Storage redundancy inside Azure for backup storage. Geo-redundant backup storage is default and recommended, though Zone and Local redundancy allow for greater cost flexibility and single region data residency. For more information, see [Backup Storage redundancy](automated-backups-overview.md?tabs=managed-instance#backup-storage-redundancy).|
