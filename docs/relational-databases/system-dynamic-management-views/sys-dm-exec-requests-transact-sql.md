...
| database_id | **smallint** | ID of the database the request is executing against. Not nullable. <br /><br />In [!INCLUDE[ssSDSfull](../../includes/ssazure-sqldb.md)], the values are unique within a single database or an elastic pool, but not within a logical server.|
