...
To use the functionality of the secure enclave, your application must enable enclave computations when connecting to the database. This achieved via by setting a connection string keyword or a connection property. Your application also needs to specify an attestation service endpoint (an enclave attestation URL) that points to an attestation service for your enclave. The details depend on a driver and an attestation service/protocol, you are using.
