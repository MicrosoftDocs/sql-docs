...
|**bulkadmin**|Members of the **bulkadmin** fixed server role can run the `BULK INSERT` statement.<br /><br /> **Note:** **bulkadmin** role or ADMINISTER BULK OPERATIONS permissions is not supported for SQL Server on Linux. Only the sysadmin can perform bulk inserts for SQL Server on Linux. |  |  
