...
- **Database mail to external SMTP relays on port 25**: Sending [database mail](/sql/relational-databases/database-mail/configure-database-mail) via port 25 to external email services is available only to certain subscription types in Microsoft Azure. Instances on other subscription types should use a different port, e.g., 587, to contact external SMTP relays. Otherwise, instances may fail to deliver database mail. For more information, please refer to [Troubleshoot outbound SMTP connectivity problems in Azure](/azure/virtual-network/troubleshoot-outbound-smtp-connectivity).
