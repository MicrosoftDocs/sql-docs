...
-   [Description of the sample tables and procedures](#Descriptionofthesampletablesandprocedures) - includes descriptions of the tables and procedures added to AdventureWorks by the In-Memory OLTP sample, as well as considerations for migrating some of the original AdventureWorks tables to memory-optimized  
...
-   *Computed columns* - the computed column LineTotal was not migrated as computed columns are not supported with memory-optimized tables in [!INCLUDE[ssSQL15](../../includes/sssql16-md.md)]. To access this column, use the view Sales.vSalesOrderDetail_extended_inmem.  
...
-   Sales.usp_UpdateSalesOrderShipInfo_inmem  
    ...
    -   This is a wrapper procedure for the natively compiled stored procedures Sales.usp_UpdateSalesOrderShipInfo_native with retry logic to deal with (unexpected) potential conflicts with concurrent transactions updating the same order. For more information about retry logic see the Books Online topic [here](./transactions-with-memory-optimized-tables.md).  
