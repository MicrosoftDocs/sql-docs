...
You can configure backup storage redundancy when you create your instance, and you can update it at a later time at the instance level. The changes that you make to an existing instance apply to future backups only. After you update the backup storage redundancy of an existing instance, the changes are typically applied within 24 hours. In some cases, particularly for Azure SQL Managed Instances with high storage utilization or larger databases, the process can take longer because the change is reflected only after the required full backups have completed. Changes made to backup storage redundancy apply to short-term backups only. Long-term retention policies inherit the redundancy option of short-term backups when the policy is created. The redundancy option persists for long-term backups even if the redundancy option for short-term backups subsequently changes.
