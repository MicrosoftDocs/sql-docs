...
---
# Report and group variables references in a paginated report (Report Builder)
...
## Report variables  
 ...
 To add a report variable, open the **ReportProperties** dialog, select **Variables**, and provide a name and a value. Names are case-sensitive strings that begin with a letter and have no spaces. A name can include letters, numbers, or underscores (_).  
...
-   **Read-write use**: Set a value once and serialize the value within a report session. The read-write option for variables provides a better alternative than using a static variable in the Code block in the report definition.  
    ...
    > [!NOTE]  
    >  You can't control when the report processor initializes a variable or evaluates an expression that updates a variable. The order of execution for variable initialization is undefined.  
  
 For more information about sessions, see [Preview reports in Report Builder](../../reporting-services/report-builder/previewing-reports-in-report-builder.md).  
...
-   For a text box in a row in the child group, add the expression `=Variables!ItemsTax.Value`.  
 ...
 To add a group variable, open the **Tablix Group Properties** dialog, select **Variables**, and provide a name and a value. The group variable is calculated once per unique group value.  
