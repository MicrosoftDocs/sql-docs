...
| **encrypt** = true<br/> **trustServerCertificate** = false or blank<br/> **hostNameInCertificate** = value<br/> **trustStore** = value<br/> **trustStorePassword** = value<br/> **serverCertificate** = blank<br/> | The driver requests to use TLS encryption with the server.<br /><br /> If the server requires the client to support TLS encryption or if the server supports encryption, the driver will initiate the TLS certificate exchange.<br /><br /> The driver will use the **trustStore** property value to find the certificate trustStore file and **trustStorePassword** property value to check the integrity of the trustStore file. Also, the driver will use the **hostNameInCertificate** property value to validate the TLS certificate.<br /><br /> If the server isn't configured to support encryption, the driver will raise an error and terminate the connection. |
| **encrypt** = strict<br/> **trustServerCertificate** = false or blank<br/> **hostNameInCertificate** = blank<br/> **trustStore** = blank<br/> **trustStorePassword** = blank<br/> **serverCertificate** = value<br/> | The driver requests to use `strict` TLS encryption with the server.<br /><br /> If the server requires the client to support TLS encryption or if the server supports encryption, the driver will initiate the TLS certificate exchange.<br /><br /> The driver will use the **serverCertificate** property specified on the connection URL to validate the server TLS certificate.<br /><br /> If the server isn't configured to support encryption, the driver will raise an error and terminate the connection. |
...
For use of TDS 8.0 with `strict` encryption, the **serverCertifcate** property value provides the path to a server certificate to be used for server certificate validation. This file must use the PEM file extension. The certificate recieved from the server must match this certificate exactly.
