...
##  <a name="create"></a> Create a Clustered Columnstore Index  
 To create a clustered columnstore index, first create a rowstore table as a heap or clustered index, and then use the [CREATE CLUSTERED COLUMNSTORE INDEX &#40;Transact-SQL&#41;](../Topic/CREATE%20CLUSTERED%20COLUMNSTORE%20INDEX%20\(Transact-SQL\).md) statement to convert the table to a clustered columnstore index. If you want the clustered columnstore index to have the same name as the clustered index, use the DROP_EXISTING option.  
...
```  
 ...
 For more examples, see the Examples section in [CREATE CLUSTERED COLUMNSTORE INDEX &#40;Transact-SQL&#41;](../Topic/CREATE%20CLUSTERED%20COLUMNSTORE%20INDEX%20\(Transact-SQL\).md).  
  
##  <a name="drop"></a> Drop a Clustered Columnstore Index  
 Use the [DROP INDEX &#40;Transact-SQL&#41;](../Topic/DROP%20INDEX%20\(Transact-SQL\).md) statement to drop a clustered columnstore index. This operation will drop the index and convert the columnstore table to a rowstore heap.  
...
##  <a name="change"></a> Change Data in a Clustered Columnstore Index  
 ...
 Use [INSERT &#40;Transact-SQL&#41;](../Topic/INSERT%20\(Transact-SQL\).md) to insert a row. The row will be added to the deltastore.  
  
 Use [DELETE &#40;Transact-SQL&#41;](../Topic/DELETE%20\(Transact-SQL\).md) to delete a row.  
...
-   If the row is in the deltastore, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] logically and physically deletes the row.  
 ...
 Use [UPDATE &#40;Transact-SQL&#41;](../Topic/UPDATE%20\(Transact-SQL\).md) to update a row.  
...
##  <a name="rebuild"></a> Rebuild a Clustered Columnstore Index  
 Use [CREATE CLUSTERED COLUMNSTORE INDEX &#40;Transact-SQL&#41;](../Topic/CREATE%20CLUSTERED%20COLUMNSTORE%20INDEX%20\(Transact-SQL\).md) or [ALTER INDEX &#40;Transact-SQL&#41;](../Topic/ALTER%20INDEX%20\(Transact-SQL\).md) to perform a full rebuild of an existing clustered columnstore index. Additionally, you can use ALTER INDEX … REBUILD to rebuild a specific partition.  
...
##  <a name="reorganize"></a> Reorganize a Clustered Columnstore Index  
 Reorganizing a clustered columnstore index moves all CLOSED rowgroups into the columnstore. To perform a reorganize, use [ALTER INDEX &#40;Transact-SQL&#41;](../Topic/ALTER%20INDEX%20\(Transact-SQL\).md)with the REORGANIZE option.  
