...
2.  Use **sp_pdw_database_encryption** to enable TDE on the SQL Server PDW. This operation modifies the temporary databases in order to ensure the protection of future temporary data, and will fail if attempted when there are any active sessions that have temporary tables. **sp_pdw_database_encryption** turns on user data masking in PDW system logs. (For more information about user data masking in PDW system logs, see [sp_pdw_log_user_data_masking](https://msdn.microsoft.com/library/mt204023.aspx).)  
  
3.  Use [sp_pdw_add_network_credentials](https://msdn.microsoft.com/library/mt204011.aspx) to create a credential that can authenticate and write to the share where the backup of the certificate will be stored. If a credential already exist for the intended storage server, the existing credential can be used.  
...
|-----------------------|-----------|  
|[CREATE DATABASE ENCRYPTION KEY](https://msdn.microsoft.com/library/bb677241.aspx)|Creates a key that is used to encrypt a database.|  
|[ALTER DATABASE ENCRYPTION KEY](https://msdn.microsoft.com/library/bb630389.aspx)|Changes the key that is used to encrypt a database.|  
|[DROP DATABASE ENCRYPTION KEY](https://msdn.microsoft.com/library/bb630256.aspx)|Removes the key that was used to encrypt a database.|  
|[ALTER DATABASE](https://msdn.microsoft.com/library/mt631606.aspx)|Explains the **ALTER DATABASE** option that is used to enable TDE.|  
...
|-------------------------------------------|-----------|  
|[sys.databases](https://msdn.microsoft.com/library/ms178534.aspx)|Catalog view that displays database information.|  
|[sys.certificates](https://msdn.microsoft.com/library/ms189774.aspx)|Catalog view that shows the certificates in a database.|  
|[sys.dm_pdw_nodes_database_encryption_keys](https://msdn.microsoft.com/library/mt203922.aspx)|Dynamic management view that provides information for each node, about the encryption keys used in a database, and the state of encryption of a database.|  
...
The tempdb system database is encrypted when encryption is enabled by using [sp_pdw_database_encryption](https://msdn.microsoft.com/library/mt219360.aspx). This is required before any database can use TDE. This might have a performance effect for unencrypted databases on the same instance of SQL Server PDW.  
...
-   [Encryption Hierarchy](https://msdn.microsoft.com/library/ms189586.aspx)  
  
-   [SQL Server and Database Encryption Keys](https://msdn.microsoft.com/library/bb964742.aspx)  

  
## See Also  
[ALTER DATABASE](https://msdn.microsoft.com/library/mt631606.aspx)  
