...
|`COLUMN_FLAGS`|`DBTYPE_UI4`||-   A bitmask that describes column characteristics. The `DBCOLUMNFLAGS` enumerated type specifies the bits in the bitmask. This column cannot contain a `NULL` value. Valid values include:<br />-   **DBCOLUMNFLAGS_ISNULLABLE** (`0x20`)<br />-   **DBCOLUMNFLAGS_MAYBENULL** (`0x40`)<br />-   **DBCOLUMNFLAGS_ISLONG** (`0x80`)|  
...
|`DATA_TYPE`|`DBTYPE_UI2`||The indicator of the column's data type. For example:<br /><br /> -   "`TABLE`" = `DBTYPE_HCHAPTER`<br />-   "`TEXT`" = `DBTYPE_WCHAR`<br />-   "`LONG`" = `DBTYPE_I8`<br />-   "`DOUBLE`" = `DBTYPE_R8`<br />-   "`DATE`" = `DBTYPE_DATE`|  
