...
The following release notes apply to [!INCLUDE[big-data-clusters-nover](../includes/ssbigdataclusters-ss-nover.md)]. This article is broken into sections for each release describing the cumulative update (CU) changes. The article also lists [known issues](#known-issues) for the most recent releases of [!INCLUDE[big-data-clusters-nover](../includes/ssbigdataclusters-ss-nover.md)].

## Tested configurations

[!INCLUDE[big-data-clusters-nover](../includes/ssbigdataclusters-ss-nover.md)] is a fully containerized solution orchestrated by Kubernetes. Starting with CU12, each release of SQL Server 2019 Big Data Clusters is tested against a fixed configuration of components. The configuration is evaluated with each release and adjustments are made to stay in-line with the ecosystem as Kubernetes continues to evolve.
