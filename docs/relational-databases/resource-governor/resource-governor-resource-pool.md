...
-   **MIN_MEMORY_PERCENT and MAX_MEMORY_PERCENT**  
     ...
     In order to govern memory for memory-optimized tables with Resource Governor, you need to bind the database to a separate resource pool. This type of resource pool would not be used for user sessions directly, and the guidance for setting min/max memory is different for this situation. For more information see [Bind a Database with Memory-Optimized Tables to a Resource Pool](../in-memory-oltp/bind-a-database-with-memory-optimized-tables-to-a-resource-pool.md?view=sql-server-ver15). 
