...
Replay trace lets you replay a captured trace file (.trc) against an upgraded test environment. For example, if a production workload is currently run out of SQL Server 2008 R2, the trace file for this workload would need to be replayed twice: once on an environment with the same SQL Server as production, and secondly on an environment with the upgrade target SQL Server version (SQL Server 2016).

> [!NOTE]
> To perform this action, virtual machines or machines must be manually set up to run Distributed Replay traces. Read the blog about [how to set up Distributed Replay controller and clients](https://blogs.msdn.microsoft.com/datamigration/distributed-replay-controller-and-client-setup/).
>
>

## Open Replay Traces
Open the tool and select the menu icon on the left side of screen. This opens the left side-bar menu. Choose **Replay Traces** next to the play icon.
...
> [!NOTE]
> The Distributed Replay controller machine requires permissions to the user account you will be using to remote.
>
>

### To grant access to the Distributed Replay controller

1. Open command prompt and type **dcomcnfg**. This opens the **Component Services** interface.
...
- **Path to store replay preprocess output:**  
    \- If you do not already have the IRF file, provide the path to the location in which you want to store the IRF file and other pre-process outputs.  
...
If the inputs are valid, the Distributed Replay process begins. Otherwise, the fields that contain invalid inputs are highlighted with red. Make sure that the entered values are accurate and again select **Start**.
...
- The Windows user performing the trace operation in the DEA Application must have sysadmin privileges in the target SQL Server. This is needed to start a trace.
...
A replay typically takes the same amount of time as the source trace plus the amount of time taken to preprocess the source trace. However, if the client machines registered with the controller are not sufficient to manage the load produced from the replay, the replay can take longer to complete. Distributed Replay allows a maximum of 16 client machines to be registered with the controller.

### How large can the target trace files get?

The target trace files can be anywhere from 5 to 15 times the size of the source trace depending on how many queries are executed. For instance, query plan blobs can be large and if the statistics change often, there will be more events captured for such queries.

### Why do I need to restore databases?

SQL Server is a stateful relational database management system. To properly perform an A/B test, the state of the database must be retained at all times. Otherwise, you might see errors in queries during replay that won't appear in production. To prevent such errors, we recommend taking a backup right before the source capture. Similarly, restoring of the backup on the target SQL Server is required to prevent errors during replay.

### What does Pass % on the Replay screen mean?

Pass % means that only a percentage of queries passed. It lets you diagnose whether the number of errors is expected or not. The errors could be because they are expected or because something went wrong and the database has lost its integrity. If the pass % is not as expected, you can stop the trace and look at the trace file in SQL Profiler to see which queries did not succeed.
...
The trace events captured contain performance-related information. The capture configuration is present in the StartReplayCaptureTrace.sql script. These are typical SQL Trace Events that are listed here: https://msdn.microsoft.com/en-us/library/ms186265.aspx.
...
### I am unable to connect to the Distributed Replay controller.
...
Distributed Replay can only access local disk resources. So the source trace files must be copied over to the Distributed Replay controller machine prior to starting the replay, and the path must be provided on the New Replay screen of DEA. 

Also, UNC paths are not compatible with Distributed Replay. They must be local absolute paths to the first source trace file, including extension.

### Why can't I browse for files on the New Replay screen?

Since we cannot browse a remote machine's folders, browsing for files is not useful. Copy-pasting the absolute paths is more efficient.

### I started replay with a trace, but Distributed Replay did not replay any events.

This can happen because the trace file does not contain replayable events or does not contain information on how to replay events. Confirm whether the trace file path provided is a source trace file. The source trace file is created using the configuration provided in the StartCaptureTrace.sql script.

### I am getting an "Unexpected error occurred!" while trying to preprocess my trace files using SQL Server 2017 Distributed Replay controller.

This is a known issue in the RTM version of SQL Server 2017. See [https://support.microsoft.com/en-us/help/4045678/fix-unexpected-error-when-you-use-the-dreplay-feature-to-replay-a](https://support.microsoft.com/en-us/help/4045678/fix-unexpected-error-when-you-use-the-dreplay-feature-to-replay-a) for more information.  
