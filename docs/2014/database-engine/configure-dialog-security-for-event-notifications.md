...
|Provide the service broker identifier of the source database to the target server. This identifier can be obtained by querying the **service_broker_guid** column of the [sys.databases](../Topic/sys.databases%20\(Transact-SQL\).md) catalog view. For a server-level event notification, use the service broker identifier of **msdb**.|Provide the service broker identifier of the target database to the source server.|  
...
## See Also  
 ...
 [Implement Event Notifications](../../2014/database-engine/implement-event-notifications.md)   
