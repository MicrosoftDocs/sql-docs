...
[Microsoft Entra ID](/azure/active-directory/active-directory-whatis) (Microsoft Entra ID) is a central user ID management technology that operates as an alternative to [SQL Server authentication](how-to-connect-using-sql-server-authentication.md). Microsoft Entra ID allows connections to Microsoft Azure SQL Database and Azure Synapse Analytics with federated identities in Microsoft Entra ID using a username and password, Windows Integrated Authentication, or a Microsoft Entra ID access token. The PHP drivers for SQL Server offer partial support for these features.
...
||A byte string|The Microsoft Entra ID Access Token extracted from an OAuth JSON response. The connection string must not contain user ID, password, or the Authentication keyword (requires ODBC Driver version 17 or above in Linux or macOS). |
...
## Example - connect using Microsoft Entra ID Access Token
...
## Example - connect using service principal objects in Microsoft Entra ID
