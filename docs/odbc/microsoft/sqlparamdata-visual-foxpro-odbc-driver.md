---
title: "SQLParamData (Visual FoxPro ODBC Driver)"
description: "SQLParamData (Visual FoxPro ODBC Driver)"
author: David-Engel
ms.author: v-davidengel
ms.reviewer: randolphwest
ms.date: 12/15/2023
ms.service: sql
ms.subservice: connectivity
ms.topic: reference
helpviewer_keywords:
  - "SQLParamData function [ODBC], Visual FoxPro ODBC Driver"
---
# SQLParamData (Visual FoxPro ODBC Driver)

> [!NOTE]  
> This article contains Visual FoxPro ODBC Driver-specific information. For general information about this function, see the appropriate article under [ODBC API Reference](../reference/syntax/odbc-api-reference.md).

## Support

Full.

## ODBC API conformance

Level 1.

## Remarks

Used in conjunction with [SQLPutData](sqlputdata-visual-foxpro-odbc-driver.md) to specify parameter data at statement execution time.

For more information, see [SQLParamData Function](../reference/syntax/sqlparamdata-function.md) in the *ODBC Programmer's Reference*.
