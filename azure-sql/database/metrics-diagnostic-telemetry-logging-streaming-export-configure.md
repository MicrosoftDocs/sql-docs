...
This article describes the performance metrics and resource logs for Azure SQL Database that you can export to one of several destinations for analysis. It explains how to configure the streaming export of this diagnostic telemetry through the Azure portal, PowerShell, Azure CLI, the REST API, and Azure Resource Manager templates.

You also learn about the destinations to which you can stream this diagnostic telemetry and how to choose among these choices. Your destination options include:
...
You can export various performance metrics and additional database logs. The following table describes the performance metrics and resource logs that you can configure for streaming export to one of several destinations. You can configure this diagnostic telemetry for single databases, elastic pools and pooled databases, and SQL managed instances and their databases.
...
| [SQLInsights](#intelligent-insights-dataset): Contains Intelligent Insights into performance for a database. For more information, see [Intelligent Insights to monitor and troubleshoot database performance (preview)](intelligent-insights-overview.md). | Yes | Yes |
...
> [!NOTE]  
> In Azure SQL Managed Instance, you can't configure diagnostic settings for **system databases** such as `master`, `msdb`, `model`, resource, and `tempdb` databases.

## Streaming export destinations

You can stream this diagnostic telemetry to one of the following Azure resources for analysis.

- **[Log Analytics workspace](#stream-into-sql-analytics)**:

    Data streamed to a [Log Analytics workspace](/azure/azure-monitor/essentials/resource-logs#send-to-log-analytics-workspace) can be consumed by [SQL Analytics](/azure/azure-monitor/insights/azure-sql). SQL Analytics is a cloud-only monitoring solution that provides intelligent monitoring of your databases, including performance reports, alerts, and mitigation recommendations. You can analyze data streamed to a Log Analytics workspace with other monitoring data collected. This option also enables you to use other Azure Monitor features such as alerts and visualizations.
- **[Azure Event Hubs](#stream-into-event-hubs)**:
  ...
  - **Stream logs to third-party logging and telemetry systems**: Stream all of your metrics and resource logs to a single event hub to pipe log data to a third-party SIEM or log analytics tool.
...
- **[Azure Storage](#stream-into-azure-storage)**:
  ...
  By streaming data to [Azure Storage](/azure/azure-monitor/essentials/resource-logs#send-to-azure-storage), you can archive vast amounts of diagnostic telemetry for a fraction of the cost of the previous two streaming options.

You can use diagnostic telemetry streamed to one of these destinations to gauge resource utilization and query execution statistics for easier performance monitoring.

:::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/architecture.png" alt-text="Diagram shows many SQL databases and databases in managed instances sending telemetry to Azure Diagnostics." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/architecture.png":::
...
> [!NOTE]  
> To enable audit log streaming of security telemetry, see [Set up Auditing for Azure SQL Database and Azure Synapse Analytics](auditing-setup.md) and [SQL Audit logs in Azure Log Analytics and Azure Event Hubs](https://techcommunity.microsoft.com/blog/azuresqlblog/sql-audit-logs-in-azure-log-analytics-and-azure-event-hubs/386242).

## Configure the streaming export of diagnostic telemetry

Use the **Diagnostics settings** menu in the Azure portal to enable and configure streaming of diagnostic telemetry. Additionally, you can use PowerShell, the Azure CLI, the [REST API](/rest/api/monitor/diagnosticsettings), and [Resource Manager templates](/azure/azure-monitor/essentials/resource-manager-diagnostic-settings) to configure streaming of diagnostic telemetry. You can set the following destinations to stream the diagnostic telemetry: Azure Storage, Azure Event Hubs, and Azure Monitor logs.

> [!IMPORTANT]  
> The streaming export of diagnostic telemetry isn't enabled by default.
...
To configure streaming of diagnostic telemetry for elastic pools and pooled databases, you need to separately configure each resource:

- Enable streaming of diagnostic telemetry for an elastic pool
- Enable streaming of diagnostic telemetry for each database in an elastic pool
...
1. Select **Turn on diagnostics** if no previous settings exist, or select **Edit setting** to edit a previous setting.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-elasticpool-enable.png" alt-text="Screenshot of Enable diagnostics for elastic pools." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-elasticpool-enable.png":::
...
1. Select the check box for elastic pool diagnostic telemetry: **Basic** metrics.
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-elasticpool-selection.png" alt-text="Screenshot of Configure diagnostics for elastic pools.":::
...
1. Select **Add diagnostic setting** to configure parallel streaming of diagnostics data to multiple resources.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-sql-enable.png" alt-text="Screenshot of Enable diagnostics for single and pooled databases." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-sql-enable.png":::
...
1. For an advanced, one-minute-based monitoring experience, select the check box for **Basic** metrics.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-sql-selection.png" alt-text="Screenshot of Configure diagnostics for Azure SQL Database.":::
1. Select **Save**.
...
| Resource | Monitoring telemetry |
| :--- | --- |
| **Managed instance** | [ResourceUsageStats](#resource-usage-stats-for-managed-instances) contains vCores count, average CPU percentage, IO requests, bytes read and written, reserved storage space, and used storage space. |
...
1. Select **Turn on diagnostics** if no previous settings exist, or select **Edit setting** to edit a previous setting.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-mi-enable.png" alt-text="Screenshot of Enable diagnostics for managed instance." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-mi-enable.png":::
...
1. Select the check box for instance diagnostic telemetry: **ResourceUsageStats**.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-mi-selection.png" alt-text="Screenshot of Configure diagnostics for managed instance.":::
...
1. Select **Turn on diagnostics** if no previous settings exist, or select **Edit setting** to edit a previous setting.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-mi-enable.png" alt-text="Screenshot of Enable diagnostics for instance databases." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-mi-enable.png":::
...
1. Select the check boxes for database diagnostic telemetry: **SQLInsights**, **QueryStoreRuntimeStatistics**, **QueryStoreWaitStatistics**, and **Errors**.
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-mi-selection.png" alt-text="Screenshot of Configure diagnostics for instance databases.":::
1. Select **Save**.
...
> [!IMPORTANT]  
> The PowerShell Azure Resource Manager (AzureRM) module was deprecated on February 29, 2024. Use the Az.Sql module for all future development. Migrate from AzureRM to the Az PowerShell module to continue receiving support and updates. The AzureRM module is no longer maintained or supported. The command arguments in the Az PowerShell module and in the AzureRM modules are substantially identical. For more information about their compatibility, see [Introducing the new Az PowerShell module](/powershell/azure/new-azureps-module-az).
...
- You can get the resource ID of your Log Analytics workspace by using the following command:
...
Azure SQL Analytics can consume Azure SQL Database and Azure SQL Managed Instance metrics and resource logs that are streamed into a Log Analytics workspace. Azure SQL Analytics is a cloud solution that monitors the performance of single databases, elastic pools and pooled databases, and managed instances and instance databases at scale and across multiple subscriptions. It helps you collect and visualize performance metrics, and it has built-in intelligence for performance troubleshooting.

:::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/azure-sql-sol-overview.png" alt-text="Screenshot of Azure SQL Analytics Overview." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/azure-sql-sol-overview.png":::

### Installation overview

Monitor a collection of databases and database collections with Azure SQL Analytics, by performing the following steps:
...
1. Search for Azure SQL Analytics in Azure Marketplace and select it.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/sql-analytics-in-marketplace.png" alt-text="Screenshot showing how to search for Azure SQL Analytics in portal." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/sql-analytics-in-marketplace.png":::
...
1. Fill in the Azure SQL Analytics form with the additional information that is required: workspace name, subscription, resource group, location, and pricing tier.
   ...
   :::image type="content" source="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/sql-analytics-configuration-blade.png" alt-text="Screenshot showing how to configure Azure SQL Analytics in portal." lightbox="media/metrics-diagnostic-telemetry-logging-streaming-export-configure/sql-analytics-configuration-blade.png":::
...
- To learn how to set up alerts in SQL Analytics, see [Creating alerts for database, elastic pools, and managed instances](/azure/azure-monitor/insights/azure-sql#analyze-data-and-create-alerts).

## Stream into Event Hubs

You can stream Azure SQL Database and Azure SQL Managed Instance metrics and resource logs into Event Hubs by using the built-in **Stream to an event hub** option in the Azure portal. You can also enable the Service Bus rule ID by using diagnostics settings via PowerShell cmdlets, the Azure CLI, or the Azure Monitor REST API. Make sure that the event hub is in the same region as your database and server.

### What to do with metrics and resource logs in Event Hubs

After the selected data is streamed into Event Hubs, you're one step closer to enabling advanced monitoring scenarios. Event Hubs acts as the front door for an event pipeline. After data is collected into an event hub, a real-time analytics provider or a storage adapter can transform and store it. Event Hubs decouples the production of a stream of events from the consumption of those events. In this way, event consumers can access the events on their own schedule. For more information on Event Hubs, see:
...
- **Build a custom telemetry and logging platform**
  ...
  Do you already have a custom-built telemetry platform or are you considering building one? The highly scalable publish-subscribe nature of Event Hubs allows you to flexibly ingest metrics and resource logs. For more information, see [Azure Event Hubs](/azure/event-hubs/event-hubs-about).
...
If you're using Azure SQL Analytics, you can monitor your data ingestion consumption by selecting **OMS Workspace** on the navigation menu of Azure SQL Analytics, and then selecting **Usage** and **Estimated Costs**.

## Metrics and logs available

This section documents the monitoring telemetry available for single databases, pooled databases, elastic pools, managed instances, and instance databases. You can use collected monitoring telemetry inside SQL Analytics for your own custom analysis and application development by using [Azure Monitor log queries](/azure/azure-monitor/logs/get-started-queries) language.
...
> [!NOTE]  
> The Basic metrics option was formerly known as All metrics. The change was to the naming only. There was no change to the metrics monitored. This change was to allow for introduction of additional metric categories in the future.
...
> [!NOTE]  
> Both Basic and Advanced metrics might be unavailable for databases that are inactive for seven days or longer.

### Basic logs

The following tables document the telemetry details available for all logs. For more information, see [supported diagnostic telemetry](#diagnostic-telemetry-for-export).
...
| `TimeGenerated` [UTC] | Time stamp when the log is recorded |
...
| `max_cpu_time_d` | Max CPU time consumed by a single execution in microseconds |
...
For more information about database wait statistics, see [sys.dm_os_wait_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-os-wait-stats-transact-sql).
