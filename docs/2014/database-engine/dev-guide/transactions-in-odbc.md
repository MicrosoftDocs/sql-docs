...
# Transactions in ODBC
  Transactions in ODBC are managed at the connection level. When an application completes a transaction, it commits or rolls back all work completed through all statement handles on that connection. To commit or roll back a transaction, applications should call [SQLEndTran](../../../2014/database-engine/dev-guide/sqlendtran.md) instead of submitting a COMMIT or ROLLBACK statement.  
  
 An application calls [SQLSetConnectAttr](../../../2014/database-engine/dev-guide/sqlsetconnectattr.md) to switch between the two ODBC modes of managing transactions:  
