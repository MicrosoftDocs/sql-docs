...
For example, an application that is sensitive to network latency can see better performance on standard-series (Gen5) hardware vs. Gen4 due to the use of Accelerated Networking in standard-series (Gen5), but an application using intensive read IO can see better performance on Gen4 hardware versus standard-series (Gen5) due to a higher memory per core ratio on Gen4.
