...
4.  You can right-click the **Error List** and use the contextual menus to sort results, filter which entries you want to display, and which columns of information you want to appear for each entry.  
    ...
    Double-click the first warning identified and follow it to the script file that generated the warning. The problematic code section is highlighted. In our example, it is because the `ShelfLife` column is being used by both a `RETURN` and `SELECT` statement in a table-value function we created previously.  
  
5.  In the Transact\-SQL Editor, remove `ShelfLife` from the function.  
  
6.  Fix the 2nd error in a similar manner by removing the check constraint.  
