...
However, keeping a query plan's shape is not the only factor that may have performance implications. For example, if you move the database to a [!INCLUDE[ssde_md](../../includes/ssde_md.md)] that has more or less memory and CPU resources, those factors will have immediate performance implications, even when no data has changed and the query plan is the same accross versions. This is why it's important to understand that maintaining the database compatibility level protects against changes in the query plan **shape**, irrespective of other environmental aspects that influence query performance.
