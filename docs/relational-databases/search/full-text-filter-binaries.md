---
title: Full-text filter binaries
titleSuffix: SQL Server Full-Text Search
description: View the list of SQL Server Full-Text Search filter binaries by full-text index version.
author: rwestMSFT
ms.author: randolphwest
ms.date: 06/29/2026
ms.service: sql
ms.subservice: search
ms.topic: reference
ai-usage: ai-assisted
helpviewer_keywords:
  - "full-text search [SQL Server], filters"
  - "filters [full-text search]"
monikerRange: "=azuresqldb-current || >=sql-server-2016 || >=sql-server-linux-2017 || =azuresqldb-mi-current"
---
# Full-text filter binaries

[!INCLUDE [sssql25-md](../../includes/sssql25-md.md)] installs new filters, replacing previous versions of these components.

- **Version 2** - Binaries installed with [!INCLUDE [sssql25-md](../../includes/sssql25-md.md)].
- **Version 1** - Binaries installed with [!INCLUDE [sssql22-md](../../includes/sssql22-md.md)] and earlier versions.

[!INCLUDE [full-text-filter-binaries-table](includes/full-text-filter-binaries-table.md)]

## Related content

- [Configure and manage filters](configure-and-manage-filters-for-search.md)
- [Customize filters and word breakers](view-or-change-registered-filters-and-word-breakers.md)
- [Full-text index version upgrade](full-text-index-version-upgrade.md)
- [Full-text word breaker and stemmer binaries](full-text-word-breaker-and-stemmer-binaries.md)
