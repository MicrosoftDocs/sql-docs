...
#### *PARSER_VERSION = '1.0' | '2.0'*
PARSER_VERSION only applies to CSV. Default is 2.0. Specifies the CSV parser used for ingestion when the source file type is CSV. The 2.0 parser offers improved performance for ingestion of CSV files. 
...
Parser version 1.0 is available for backward compatibility only, and should be used when these limitations are encountered.  
