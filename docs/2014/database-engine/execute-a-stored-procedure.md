...
-   The calling database collation is used when matching system procedure names. Therefore, always use the exact case of system procedure names in procedure calls. For example, this code will fail if it is executed in the context of a database that has a case-sensitive collation:  
    ...
    ```  
     ...
     To display the exact system procedure names, query the [sys.system_objects](../Topic/sys.system_objects%20\(Transact-SQL\).md) and [sys.system_parameters](../Topic/sys.system_parameters%20\(Transact-SQL\).md) catalog views.  
...
-   Executing User-defined Stored Procedures  
     ...
     The following example demonstrates the recommended method to execute a user-defined procedure. Notice that the procedure accepts one input parameter. For information about specifying input and output parameters, see [Specify Parameters](../../2014/database-engine/specify-parameters.md).  
...
-   Setting, Clearing, and Controlling Automatic Execution  
     ...
     Use [sp_procoption](../Topic/sp_procoption%20\(Transact-SQL\).md) to:  
...
###  <a name="Security"></a> Security  
 For more information, see [EXECUTE AS &#40;Transact-SQL&#41;](../Topic/EXECUTE%20AS%20\(Transact-SQL\).md) and [EXECUTE AS Clause &#40;Transact-SQL&#41;](../Topic/EXECUTE%20AS%20Clause%20\(Transact-SQL\).md).  
  
####  <a name="Permissions"></a> Permissions  
 For more information, see the "Permissions" section in [EXECUTE &#40;Transact-SQL&#41;](../Topic/EXECUTE%20\(Transact-SQL\).md).  
...
3.  Copy and paste the following example into the query window and click **Execute**. This example shows how to use [sp_procoption](../Topic/sp_procoption%20\(Transact-SQL\).md) to set a procedure for automatic execution.  
...
3.  Copy and paste the following example into the query window and click **Execute**. This example shows how to use [sp_procoption](../Topic/sp_procoption%20\(Transact-SQL\).md) to stop a procedure from executing automatically.  
...
## See Also  
 [Specify Parameters](../../2014/database-engine/specify-parameters.md)   
 [Configure the scan for startup procs Server Configuration Option](../../2014/database-engine/configure-the-scan-for-startup-procs-server-configuration-option.md)   
