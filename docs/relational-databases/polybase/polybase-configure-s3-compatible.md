...
* A user (`Access Key ID`) and the secret (`Secret Key ID`) known to you. You will need both to authenticate against the S3 object storage endpoint.
...
- In AWS S3, create a custom role and specifically state which S3 API requires access.
    - Backup requires these permissions: **ListBucket** (Browse), **PutObject** (Write - for backup).
    - Restore requires these permissions: **ListBucket** (Browse), **GetObject** (Read - for restore), **GetObject** (Read - for restore).
- In other S3-compatible storage:
    - Backup requires the user (`Access Key ID`) must have both **ListBucket** and **WriteOnly** permissions.
    - Restore requires the user (`Access Key ID`) must have both **ListBucket** and **ReadOnly** permissions.
