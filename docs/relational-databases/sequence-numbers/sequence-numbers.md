...
There are more examples in the articles [CREATE SEQUENCE](../../t-sql/statements/create-sequence-transact-sql.md), [NEXT VALUE FOR](../../t-sql/functions/next-value-for-transact-sql.md), and [sp_sequence_get_range](../system-stored-procedures/sp-sequence-get-range-transact-sql.md).
...
[!INCLUDE [tsql](../../includes/tsql-md.md)] statements that use `SELECT *` receive the new column as the last column instead of the first column. If this isn't acceptable, then you must create an entirely new table, move the data to it, and then recreate the permissions on the new table.
