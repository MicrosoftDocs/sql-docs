...
When your database isn't* configured with a secure enclave, the wizard will give you the opportunity to enable a secure enclave. If you choose not to enable a secure enclave or you aren't* using enclave-enabled keys, the wizard works by moving data out of the database and performing cryptographic operations within the SQL Server Management Studio (SSMS) process. The wizard creates a new table (or tables) with the desired encryption configuration in the database, loads all data from the original tables, performs the requested cryptographic operations, uploads the data to the new table(s), and then swaps the original table(s) with the new table(s).
