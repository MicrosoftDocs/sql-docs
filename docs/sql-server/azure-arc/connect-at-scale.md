...
You can automatically connect SQL Server instances on multiple Arc-enabled machines using an Azure policy definition called *Configure Arc-enabled machines running SQL Server to have SQL Server extension installed*. This policy definition is not assigned to a scope by default. If you assign this policy definition to a scope of your choice, it will install the *Azure extension for SQL Server* on all Azure Arc-enabled servers where SQL Server is installed. Once installed, the extension will connect the SQL Server instances on the machine with Azure. After that, the extension will run continuously to detect changes of the SQL Server configuration and synchronize them with Azure. For example, if a new SQL Server instance is installed on the machine, the extension will automatically register it with Azure.

To create an Azure Policy assignment, you will need the `Resource Policy Contributor` role assignment on the scope - subscription or resource group - that that you are targeting the assignment to.  Further, if you are going to create a *new* system assigned managed identity, you will need the `User Access Administrator` role assignment in the subscription.

> [!IMPORTANT]
>The Arc-enabled SQL Server resources for the `SQL Server - Azure Arc` resources will be created in the same region and the resource group as the `Server - Azure Arc` resources on which they are hosted. 
...
To do this, navigate to the SQL Server - Azure Arc view in the Azure Portal and click on the 'Automatic Arc-enabled SQL Server registration' button at the top of the list.  Select a subscription and optionally a resource group.  Check the 'I accept the terms in the agreement' checkbox. Click 'Enable'. This will create a new Azure Policy assigment of the *Configure Arc-enabled machines running SQL Server to have SQL Server extension installed* policy definition to the selected subscription and, optionally, a specific resource group scope. A new system assigned managed identity will be created and granted the required permissions to onboard Arc-enabled SQL Servers. This new managed identity will be used by the policy remediation to install the Azure extension for SQL Server.

### Connect at-scale using Azure Policy assignment

If you want to select an existing user assigned managed identity or have more granular control over the configuration of the at-scale onboarding policy, you can create the Azure Policy assignment. Navigate to Azure Policy in the Azure Portal and choose 'Definitions'. Search for '*Configure Arc-enabled machines running SQL Server to have SQL Server extension installed.*' and click on the policy. Click on 'Assign'. Choose a Scope. Click Next. Click Next.  On the Remediation tab, click 'Create a remediation task'.  Choose 'System assigned managed identity' (recommended) or 'User assigned managed identity' and choose a managed identity which has 'User Access Administration' and 'Log Analytics Contributor' role assignments. Click Review + Create.  Click Create.
