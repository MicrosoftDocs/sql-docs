...
The following example uses the [GET_BIT](get-bit-transact-sql.md) function to get the value in specific bit for a **varbinary** representation of `@@OPTIONS`.

```sql
DECLARE @options AS VARBINARY(MAX) = CONVERT(VARBINARY(MAX), @@OPTIONS);
