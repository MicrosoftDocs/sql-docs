...
#### SECRET = '*secret*'
 ...
 Specifies the secret required for outgoing authentication. *secret* is required to import a file from Azure Blob storage. *secret* might be optional for other purposes.
...
### A. Change the password of a database scoped credential
 ...
 The following example changes the secret stored in a database scoped credential. The database scoped credential contains a Windows login and its password. The new password is added to the database scoped credential using the `SECRET` clause.
...
### B. Remove the password from a credential
 ...
 The following example removes the password from a database scoped credential named `Frames`. The database scoped credential contains a Windows login and a password. After the statement is executed, the database scoped credential will have a `NULL` password because the SECRET option is not specified.
