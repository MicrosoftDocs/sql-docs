...
#### *schema_name*  
Is optional if the default schema for the user performing the operation is the schema of the specified table. If *schema* isn't specified, and the default schema of the user performing the COPY operation is different from the specified table, COPY is canceled, and an error message is returned.  

#### *table_name*  
Is the name of the table to COPY data into. The target table can be a temporary or permanent table and must already exist in the database. For automatic schema detection mode, don't provide a column list.
...
>[!NOTE]  
> If the user who executes the COPY command intends to generate a new table and subsequently load data into it, they will require extra permissions, namely CREATE TABLE and ALTER ON SCHEMA permissions.

The following table summarizes the permissions for insert (into as existing table or to insert into a new table).
  
|Permissions|Details
|-----------|-------
|Permissions (insert into an existing table)|ADMINISTER DATABASE BULK OPERATIONS<br>INSERT
|Permissions (insert into a new table)|ADMINISTER DATABASE BULK OPERATIONS<br>INSERT<br>CREATE TABLE<br>ALTER ON SCHEMA :: SchemaName

Example: Allow mike@contoso.com to use COPY to create a new table in HR schema and insert the data from Parquet file:
...
Is optional if the current warehouse for the user performing the operation is the warehouse of the specified table. If *warehouse* isn't specified, and the specified schema and table doesn't exist on the current warehouse, COPY fails, and an error message is returned.

#### *schema_name*

Is optional if the default schema for the user performing the operation is the schema of the specified table. If *schema* isn't specified, and the default schema of the user performing the COPY operation is different from the specified table, COPY fails, and an error message is returned.  

#### *table_name*

Is the name of the table to COPY data into. The target table must already exist in the warehouse.
...
> [!NOTE]
> When working with parquet files on [!INCLUDEfabric-data-warehouse] in [!INCLUDEmicrosoft-fabric].Warehouse, column names must match exactly in the source and destination. If the name of the column in target table is different than that of the column name in the parquet file, the target table column is filled with NULL.
...
*CREDENTIAL* specifies the authentication mechanism to access the external storage account. On [!INCLUDEfabric-data-warehouse] in [!INCLUDEmicrosoft-fabric] Warehouse, the only supported authentication mechanisms are Shared Access Signature (SAS) and Storage Account Key (SAK).
