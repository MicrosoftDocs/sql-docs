...
/* Beginning with SQL Server 2022, runtimes for R, Python, and Java, are no longer shipped or installed within SQL Setup. Instead, install your desired R and/or Python custom runtime(s) and packages. For more information, see [Install SQL Server 2022 Machine Learning Services (Python and R) on Windows](install/sql-machine-learning-services-windows-install-sql-2022.md).
...
You can install built packages sqlmlutils, if you can build locally to produce binary package as output. For examples, see [Install R packages with sqlmlutils](package-management/install-additional-r-packages-on-sql-server.md) or [Install Python packages with sqlmlutils](package-management/install-additional-python-packages-on-sql-server.md).

For a **miniCRAN** binary package approach, you can also install using sqlmlutils. For more information, see [Add packages offline](package-management/install-additional-r-packages-on-sql-server.md#add-the-package-offline).
