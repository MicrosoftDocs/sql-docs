...
|**encryptor_type**|**nvarchar(32)**|The type of encryptor used, certificate or asymmetric key. If the backup was not encrypted, this value is NULL.| 
|**last_valid_restore_time**|**datetime**|The latest point in time to which the backup can be restored. Applies to SQL Server 2022 CTP1.2 and later. |  
