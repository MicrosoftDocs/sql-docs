...
Adds a table and creates internal metadata objects for [Azure Synapse Link for SQL](/azure/synapse-analytics/synapse-link/sql-synapse-link-overview). For more information, see [Manage Azure Synapse Link for SQL Server and Azure SQL Database](../../sql-server/synapse-link/synapse-link-sql-server-change-feed-manage.md).
...
The unique identifier of the change feed table group that the new link table will belong to. It is a GUID generated by the initial setup. Table_group_id must already exist in change feed. 

#### @table_id

Required. Data type is uniqueidentifier.

The unique identifier of the link table. It will be a GUID generated by the setup flow and passed to the source publisher during provision. 
