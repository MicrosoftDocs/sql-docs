...
## The 59 cmdlets Currently Available in PS Core
While the SqlServer module currently has 106 cmdlets available in Windows PowerShell, only 59 of those 106 are available in PSCore. In the interests of making the current capabilities of the SqlServer module on PSCore more widely known, a full list of 59 current cmdlets is included below.

```powershell
Get-Command -Module SqlServer -CommandType Cmdlet | SORT -Property Noun
```
