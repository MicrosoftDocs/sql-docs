...
_WITH STATUSONLY_   
Is used to generate a progress report for a specified _UOW_ or _session_ID_ that is being rolled back because of an earlier KILL statement. KILL WITH STATUSONLY doesn't end or roll back the _UOW_. The command only displays the current progress of the rollback.
