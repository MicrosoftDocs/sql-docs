...
| **Azure REST API** | For cases where Azure-connected features are used and additional troubleshooting information is needed, this channel will output information specific to Azure REST API calls.  Microsoft Entra authentication must be used.  REST API output will appear only after authentication with Microsoft Entra credentials has occurred. |
