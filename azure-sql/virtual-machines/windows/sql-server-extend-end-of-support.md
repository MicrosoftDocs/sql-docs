...
> [!NOTE]
>
> Registration with the [SQL IaaS Agent extension](sql-agent-extension-manually-register-single-vm.md) is not required for manual installation of extended security updates. Microsoft Update will automatically detect that the VM is running in Azure and present the relevant updates for download even if the extension is not present.
