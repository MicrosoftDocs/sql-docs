...
> [!IMPORTANT]
> If you're using Query Store for just-in-time workload insights in [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)], plan to install the performance scalability fixes in [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2 CU2 ([KB 4340759](https://support.microsoft.com/help/4340759)) as soon as possible. Without these fixes, when the database is under heavy workloads, spinlock contention may occur and server performance may become slow. In particular, you may see heavy contention on the `QUERY_STORE_ASYNC_PERSIST` spinlock or `SPL_QUERY_STORE_STATS_COOKIE_CACHE` spinlock. After this fix is applied, Query Store will no longer cause spinlock contention.

> [!IMPORTANT]
> If you're using Query Store for just-in-time workload insights in [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)], plan to install the performance scalability fix in [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] CU22 as soon as possible. Without these fixes, when the database is under heavy ad-hoc workloads, the Query Store may use a large amount of memory and server performance may become slow. After this fix is applied, Query Store imposes internal limits to the amount of used memory it can use in the [!INCLUDE[ssde_md](../../includes/ssde_md.md)], and automatically changes the operation mode to read-only enough memory has been returned to the free memory pool. 
