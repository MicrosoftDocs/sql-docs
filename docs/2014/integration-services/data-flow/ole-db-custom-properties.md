...
|SqlCommandVariable|String|The variable that contains the SQL command to be executed.|  
 ...
 For more information, see [OLE DB Source](ole-db-source.md).  
...
|FastLoadKeepIdentity|Boolean|A value that specifies whether to copy identity values when data is loaded. This property is available only with one of the fast load options. The default value of this property is `False`. This property corresponds to the OLE DB [IRowsetFastLoad &#40;OLE DB&#41;](../../relational-databases/native-client-ole-db-how-to/irowsetfastload-ole-db.md) property `SSPROP_FASTLOADKEEPIDENTITY`.|  
|FastLoadKeepNulls|Boolean|A value that specifies whether to copy Null values when data is loaded. This property is available only with one of the fast load options. The default value of this property is `False`. This property corresponds to the OLE DB [IRowsetFastLoad &#40;OLE DB&#41;](../../relational-databases/native-client-ole-db-how-to/irowsetfastload-ole-db.md) property `SSPROP_FASTLOADKEEPNULLS`.|  
...
|SqlCommand|String|When AccessMode is `SQL Command`, the Transact-SQL statement that the OLE DB destination uses to specify the destination columns for the data.|  
 ...
 For more information, see [OLE DB Destination](ole-db-destination.md).  
