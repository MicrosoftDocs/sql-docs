...
This article provides information and steps to start your [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] instance in single-user mode, which allows only one user connection to the instance.
...
The following example starts the [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] instance in single-user mode via command line and only allows connection through the SQL Server Management Studio Query Editor.

```console
net start "SQL Server (MSSQLSERVER)" /m"Microsoft SQL Server Management Studio - Query"
```

To restore the `master` database in [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] on Linux in single-user mode, see [Restore the master database on Linux in single-user mode](../../linux/sql-server-linux-restore-master-database-in-single-user-mode.md).

## General considerations

When you start an instance of [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] in single-user mode, note that:
...
1. Connect to the [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] using the following command and do the necessary operation:
