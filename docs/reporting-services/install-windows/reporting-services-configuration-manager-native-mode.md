...
1. From the Windows **Start** menu, enter **Report Server** and select **Report Server Configuration Manager** from the search results to start the Report Server Configuration Manager. Alternately, depending on your version of Microsoft Windows, you can select **Start**, go to **All Programs**, point to [!INCLUDE[ssCurrentUI](../../includes/sscurrentui-md.md)] Reporting Services and then select **Report Server Configuration Manager**.
