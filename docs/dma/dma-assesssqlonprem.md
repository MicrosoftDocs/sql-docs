---
title: "Assess an enterprise and consolidate assessment reports (Data Migration Assistant) | Microsoft Docs"
...
- Download and install:
    ...
    - [PowerBI desktop](https://docs.microsoft.com/en-us/power-bi/desktop-get-the-desktop).
...
2. Open the [PowerShell-Modules](https://msdnshared.blob.core.windows.net/media/2018/03/PowerShell-Modules.zip), and then save them into the folder you created.
      ...
      ![PowerShell Modules](../dma/media//dma-assesssqlonprem/dma-powershell-modules.png)

    Each folder contains the associated psm1 file, as shown in the following graphic:

   ![PowerShell modules psm1 files](../dma/media//dma-assesssqlonprem/dma-powershell-modules-psm1-files.png)
   ...
   ![psm1 file properties](../dma/media//dma-assesssqlonprem/dma-psm1-file-properties.png)
...
When using a csv file to import the data, ensure there are only two columns of data – **Instance Name** and **Database Name**, and that the columns don’t have header rows.
 ...
 ![csv file contents](../dma/media//dma-assesssqlonprem/dma-csv-file-contents.png)
...
- AssessmentFlag
 ...
 ![SQL Server table contents](../dma/media//dma-assesssqlonprem/dma-sql-server-table-contents.png)
...
After loading the PowerShell modules into the modules directory and creating an inventory, you need to run a scaled assessment by opening PowerShell and running the dmaDataCollector function.
  ...
  ![dmaDataCollector function listings](../dma/media//dma-assesssqlonprem/dma-dmaDataCollector-function-listing.png)
...
If there is an unexpected error, then the command window that gets initiated by this process will be terminated.  Review the error log to determine why it failed.
  ...
  ![Error log location](../dma/media//dma-assesssqlonprem/dma-error-log-file-location.png)
...
After your assessment has finished, you are now ready to import the data into SQL Server for analysis. To consume the assessment JSON file, open PowerShell and run the dmaProcessor function.
  ...
  ![dmaProcessor function listing](../dma/media//dma-assesssqlonprem/dma-dmaProcessor-function-listing.png)
...
2. Load the data warehouse.
      ...
      ![LoadWarehouse contents loaded](../dma/media//dma-assesssqlonprem/dma-LoadWarehouse-loaded.png)
...
You can also use the LoadWarehouse script to provide the basic TSQL statements for you to set the database owners.
  ...
  ![LoadWarehouse setting owners](../dma/media//dma-assesssqlonprem/dma-LoadWarehouse-set-owners.png)
...
2. Enter server details that point to your **DMAWarehouse** database, and then select **Load**.
    ...
    > Do not press Enter to accept the values.
      ...
      ![DMA Reports Power BI template loaded](../dma/media//dma-assesssqlonprem/dma-reports-powerbi-template-loaded.png)
   ...
   ![DMAWarehouse report view](../dma/media//dma-assesssqlonprem/dma-DMAWarehouse-report.png)
...
- On-premises assessments
  ...
  ![DMA report bookmarks](../dma/media//dma-assesssqlonprem/dma-report-bookmarks.png)
...
### Readiness %
  ...
  ![DMA Readiness percentage](../dma/media//dma-assesssqlonprem/dma-readiness-percentage.png)

This visual is updated based on the selection context (everything, instance, database [multiples of]).

### Readiness count

  ![DMA Readiness count](../dma/media//dma-assesssqlonprem/dma-readiness-count.png)
...
### Readiness bucket
  ...
  ![DMA Readiness bucket](../dma/media//dma-assesssqlonprem/dma-readiness-bucket.png)
...
### Issues Word Cloud
  ...
  ![DMA issues WordCloud](../dma/media//dma-assesssqlonprem/dma-issues-word-cloud.png)
...
### Database Readiness
  ...
  ![DMA Database Readiness report](../dma/media//dma-assesssqlonprem/dma-database-readiness-report.png)
...
This task filters the remediation plan report to the current hierarchy level based on the point at which you select the drill through option.
  ...
  ![DMA Database Readiness report drilldown filtered](../dma/media//dma-assesssqlonprem/dma-database-readiness-report-drilldown-filtered.png)

  ![DMA Remediation Plan report](../dma/media//dma-assesssqlonprem/dma-remediation-plan-report.png)
