...
|Name|Element|  
|----------|-------------|  
|Cube|*Usage*<br /> Optional, write-only `String` property<br /><br /> *Description*<br /> Contains the name of the cube that sets the context for the command. If the command itself contains a cube name, such as within the FROM clause of a Multidimensional Expressions (MDX) [SELECT](~/mdx/mdx-data-manipulation-select.md) statement, the setting of this property is ignored.<br /><br /> The default value for this property is an empty string.<br /><br /> This property can be used with the `Discover` and `Execute` methods.|  
