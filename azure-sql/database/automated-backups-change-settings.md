...
1. Go to the server with the databases whose retention period you want to change. 
...
You can't update the backup storage redundancy of a Hyperscale database directly. However, you can change it by using [the database copy command](database-copy.md) with the `--backup-storage-redundancy` parameter. This example copies a Hyperscale database to a new database by using Gen5 hardware and two vCores. The new database has the backup redundancy set to `Zone`.
