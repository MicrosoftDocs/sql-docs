...
- Place data, log, and `tempdb` files on separate drives.
  ...
  - Place [tempdb](/sql/relational-databases/databases/tempdb-database) on the temporary disk (the temporary disk is ephemeral, and defaults to D:\) for most SQL Server workloads that aren’t part of Failover Cluster Instance (FCI) after choosing the optimal VM size.
