...
The database objects you need will be created by using a second PowerShell script, RunSQL_R_Walkthrough.ps1. The script creates the table schema in the database you specify and uploads the data into the table.  It also creates some SQL functions and stored procedures that simplify data science tasks.

## 1. Download the data and scripts

All the code has been provided in a GitHub repository. You can use a PowerShell script to make a local copy of the files.

1.  On your data science client computer, open a Windows PowerShell command prompt as administrator.

2.  To ensure that you can run the download script without an error, run this command. It  temporarily allows script execution without changing system defaults.
...
4.  Downloading the data might take a while. After it is complete, the Windows PowerShell command console should look like this:
...
### Install required packages on the R development client
...
### How do I run the script without prompts
...
If your database contains a table of the same name and the same schema, the **bcp** utility will insert a new copy of the data rather than overwriting existing data. To avoid duplicate data, truncate any existing tables before re-running the script.

## What the download includes

When you download the files from the GitHub repository, you'll get the following:

+ The taxi dataset, in CSV format
...
[2. Explore the Data](/walkthrough-view-and-explore-the-data.md)

## Previous step

[Data Science End-to-End Walkthrough](/walkthrough-data-science-end-to-end-walkthrough.md)
