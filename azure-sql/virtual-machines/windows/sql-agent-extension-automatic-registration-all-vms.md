...
> [!NOTE]
> If you deployed a SQL Server VM with a marketplace image which has the SQL IaaS agent extension preinstalled, and the extension is in a failure state or it was removed, automatic registration will check the registry to see if SQL Server is installed on the VM and register it in lightweight mode.
