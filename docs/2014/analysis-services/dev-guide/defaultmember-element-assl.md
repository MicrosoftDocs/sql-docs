...
## Remarks  
 ...
 For `ManyToManyMeasureGroupDimension` elements, the `DefaultMember` element contains an MDX expression that specifies a member in the dimension identified in the `CubeDimensionID` element of the `ManyToManyMeasureGroupDimension`. The MDX expression is similar to the [StrToMember](../Topic/StrToMember%20\(MDX\).md) MDX function with the CONSTRAINED keyword, in that it cannot include MDX or user-defined functions.  
