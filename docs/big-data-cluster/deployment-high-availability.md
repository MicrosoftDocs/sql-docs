...
- Databases created as result of workflows other than `CREATE DATABASE` and `RESTORE DATABASE` (starting with SQL Server 2019 CU2 only) like `CREATE DATABASE FROM SNAPSHOT` are not automatically added to the availability group. [Connect to the instance](#instance-connect) and add the database to the availability group manually.
