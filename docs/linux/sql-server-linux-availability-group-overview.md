...
-   Have three synchronous replicas (Enterprise Edition only); or
...
A configuration-only replica can be hosted on any edition of SQL Server, including Express. This will minimize licensing costs and ensures it works with AGs in Standard Edition. This means that the third required server just needs to meet the minimum specification for SQL Server, since it is not receiving user transaction traffic for the AG.
...
An AG that has a cluster type of External or one that is WSFC cannot have its replicas cross platforms. This is true whether the AG is Standard or Enterprise Edition. That means in a traditional AG configuration with an underlying cluster, one replica cannot be on a WSFC and the other on Linux with Pacemaker.
