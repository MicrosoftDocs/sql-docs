...
## <a id="Wizard"></a> Use the wizard to enable Stretch Database on a database
...
## <a id="EnableTSQLDatabase"></a> Use Transact-SQL to enable Stretch Database on a database
...
1. To configure a SQL Server database for Stretch Database, the database has to have a database master key. The database master key secures the credentials that Stretch Database uses to connect to the remote database. Here's an example that creates a new database master key.
