...
|View Items|View items in lists, documents in document libraries, and Web discussion comments.|X|X|X|Open a report, report model, and other document and process it on the report server.|  
...
## Site Permissions  
 Site permissions determine access to report server operations that aren't directly related to items stored in a specific library. One example is creating and managing shared schedules, which items can use in multiple libraries. Another example is configuring the Report Viewer Web Part, which can be used throughout a site.  
