...
We recommend to configure alerts on Failed Ledger Digest Uploads if you want to be notified when a digest upload failed. Failures might occur due to revoked permissions on the storage account or network configuration that makes the storage account inaccessible. Optionally, you could also configure an alert on Successful Ledger Digest Uploads. This could be interesting if the number of uploads dropped under a certain value or zero due to someone that disabled the automatic digest upload. This would not be considered a failure since digest upload was explicitly disabled.
