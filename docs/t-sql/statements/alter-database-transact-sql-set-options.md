...
Click one of the following tabs for a particular SQL version with which you're working:
...
You can determine this option's status by examining the is_auto_close_on column in the sys.databases catalog view. You can also determine the status by examining the IsAutoClose property of the DATABASEPROPERTYEX function.
...
When the database is set to AUTOCLOSE = ON, an operation that starts an automatic database shutdown clears the plan cache for the instance of [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Clearing the plan cache causes a recompilation of all upcoming execution plans and can cause a sudden, temporary reduction in query performance. In [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] Service Pack 2 and higher, for each cleared cachestore in the plan cache, the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] error log contains the following informational message: " [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] has encountered %d occurrence(s) of cachestore flush for the '%s' cachestore (part of plan cache) due to some database maintenance or reconfigure operations". This message is logged every five minutes as long as the cache is flushed within that time interval.

<a name="auto_create_statistics"></a> AUTO_CREATE_STATISTICS { ON | OFF }
ON
The query optimizer creates statistics on single columns in query predicates, as necessary, to improve query plans and query performance. These single-column statistics are created when the query optimizer compiles queries. The single-column statistics are created only on columns that aren't already the first column of an existing statistics object.
...
ON
Specifies that the query optimizer updates statistics when they're used by a query. Also specifies when the statistics might be out-of-date. Statistics become out-of-date after insert, update, delete, or merge operations change the data distribution in the table or indexed view. The query optimizer determines when statistics might be out-of-date by counting the number of data modifications since the last statistics update. The query optimizer compares the number of modifications to a threshold. The threshold is based on the number of rows in the table or indexed view.
...
You can determine this option's status by examining the is_cursor_close_on_commit_on column in the sys.databases catalog view. You can also determine the status by examining the IsCloseCursorsOnCommitEnabled property of the DATABASEPROPERTYEX function.
...
Specifies whether an AFTER trigger can cascade. Cascade means that the trigger can perform an action that initiates another trigger, which initiates another trigger, and so on. This option is allowable only when CONTAINMENT has been set to PARTIAL. If CONTAINMENT is set to NONE, errors will occur.
...
ALL
Captures all queries. ALL is the default configuration value.ALL is the default configuration value for [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]
...
CHECKSUM
Calculates a checksum over the contents of the whole page. Stores the value in the page header when a page is written to disk. When the page is read from disk, the checksum is recomputed and compared to the checksum value stored in the page header. If the values don't match, error message 824 (indicating a checksum failure) is reported to both the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] error log and the Windows event log. A checksum failure indicates an I/O path problem. To determine the root cause requires investigation of the hardware, firmware drivers, BIOS, filter drivers (such as virus software), and other I/O path components.

TORN_PAGE_DETECTION
Saves a specific 2-bit pattern for each 512-byte sector in the 8 kilobytes (KB) database page. Stores the torn bits in the database page header when the page is written to disk. When the page is read from disk, the torn bits stored in the page header are compared to the actual page sector information.

Unmatched values indicate only part of the page was written to disk. Error message 824 (indicating a torn page error) is reported, in this situation, to both the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] error log and the Windows event log. Torn pages are typically detected by database recovery if it's truly an incomplete write of a page. However, other I/O path failures can cause a torn page at any time.

NONE
Database page writes won't generate a CHECKSUM or TORN_PAGE_DETECTION value. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] won't verify a checksum or torn page during a read even if a CHECKSUM or TORN_PAGE_DETECTION value is present in the page header.
...
When a torn page or checksum failure is detected, you can recover by restoring the data. You can also recover by potentially rebuilding the index if the failure is limited only to index pages. If you run into a checksum failure, to determine the type of database page or pages affected, run DBCC CHECKDB. For more information about restore options, see [RESTORE Arguments](../../t-sql/statements/restore-statements-arguments-transact-sql.md). Data restoration might resolve the problem that data corruption causes. But in any case, you should diagnose and correct the root cause. The diagnosis and correction prevents continuing errors.
...
ALLOW_SNAPSHOT_ISOLATION { ON | OFF }
ON
Enables Snapshot option at the database level. When it's enabled, DML statements start generating row versions even when no transaction uses Snapshot Isolation. Once this option is enabled, transactions can specify the SNAPSHOT transaction isolation level. All statements see a snapshot of data as it exists at the start of the transaction when a transaction runs at the SNAPSHOT isolation level. If it runs at that level, either set ALLOW_SNAPSHOT_ISOLATION to ON in all the databases. Each statement in the transaction must use locking hints on any reference in a FROM clause to a database table where ALLOW_SNAPSHOT_ISOLATION is OFF if you don't set the option.

OFF
Turns off the Snapshot option at the database level. Transactions can't specify the SNAPSHOT transaction isolation level.

When you set ALLOW_SNAPSHOT_ISOLATION to a new state, ALTER DATABASE doesn't return control to the caller until all existing transactions in the database are committed. New states include from ON to OFF or from OFF to ON. If the database is already in the state specified in the ALTER DATABASE statement, control is returned to the caller immediately. Use [sys.dm_tran_active_snapshot_database_transactions](../../relational-databases/system-dynamic-management-views/sys-dm-tran-active-snapshot-database-transactions-transact-sql.md) to determine whether there are long-running transactions if the ALTER DATABASE statement doesn't return quickly. If you cancel the ALTER DATABASE statement, the database remains in the state it was in when ALTER DATABASE was started. The [sys.databases](../../relational-databases/system-catalog-views/sys-databases-transact-sql.md) catalog view indicates the state of snapshot-isolation transactions in the database. ALTER DATABASE ALLOW_SNAPSHOT_ISOLATION OFF will pause six seconds and retry the operation if **snapshot_isolation_state_desc** = IN_TRANSITION_TO_ON.
...
Connection-level settings that are set by using the SET statement override the default database setting for CONCAT_NULL_YIELDS_NULL. ODBC and OLE DB clients issue a connection-level SET statement setting CONCAT_NULL_YIELDS_NULL to ON for the session, by default. The clients run the statement when you connect to an instance of [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. For more information, see [SET CONCAT_NULL_YIELDS_NULL](../../t-sql/statements/set-concat-null-yields-null-transact-sql.md).
...
OFF
Only direct recursive firing of AFTER triggers isn't allowed. To also disable indirect recursion of AFTER triggers, set the nested triggers server option to **0** by using **sp_configure**.
...
Clearing the plan cache causes a recompilation of all future execution plans and can cause a sudden, temporary reduction in query performance. For each cleared cachestore in the plan cache, the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] error log contains the following informational message: " [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] has encountered %d occurrence(s) of cachestore flush for the '%s' cachestore (part of plan cache) due to some database maintenance or reconfigure operations". This message is logged every five minutes as long as the cache is flushed within that time interval.
...
INCREMENTAL = ON | OFF
When AUTO_CREATE_STATISTICS is ON and INCREMENTAL is ON, automatically created stats are created as incremental whenever incremental stats are supported. The default value is OFF. For more information, see [CREATE STATISTICS](../../t-sql/statements/create-statistics-transact-sql.md).
...
The AUTO_SHRINK option causes files to be shrunk when more than 25 percent of the file contains unused space. The option shrinks the file whichever is larger:

- A size where 25 percent of the file is unused space
- The file size when it was created
...
INHERIT
Using the value INHERIT will inherit the default configuration from the parent server. This inheritance is especially useful if you want to customize automatic tuning configuration on a parent server. Inheritance also helps if you want to have all the databases on such servers INHERIT these custom settings. For the inheritance to work, the three individual tuning options FORCE_LAST_GOOD_PLAN, CREATE_INDEX and DROP_INDEX need to be set to DEFAULT on databases.

CUSTOM
Using the value CUSTOM, you'll need to manually custom configure each of the automatic tuning options available on databases.
...
You can determine this option's status by examining the is_cursor_close_on_commit_on column in the sys.databases catalog view. You can also determine the status by examining the IsCloseCursorsOnCommitEnabled property of the DATABASEPROPERTYEX function. The cursor is implicitly deallocated only at disconnect. For more information, see [DECLARE CURSOR](../../t-sql/language-elements/declare-cursor-transact-sql.md).
...
RESTRICTED_USER
RESTRICTED_USER allows for only members of the db_owner fixed database role and dbcreator and sysadmin fixed server roles to connect to the database. RESTRICTED_USER doesn't limit the number of connections. Disconnect all connections to the database using the timeframe specified by the ALTER DATABASE statement's termination clause. After the database has transitioned to the RESTRICTED_USER state, connection attempts by unqualified users are refused. **RESTRICTED_USER** can't be modified with SQL Database managed instance.
...
ALL
Captures all queries. ALL is the default configuration value.

AUTO
Capture relevant queries based on execution count and resource consumption. AUTO is the default configuration value for [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)]
...
ALLOW_SNAPSHOT_ISOLATION { ON | OFF }
ON
Enables Snapshot option at the database level. When it's enabled, DML statements start generating row versions even when no transaction uses Snapshot Isolation. Once this option is enabled, transactions can specify the SNAPSHOT transaction isolation level. All statements see a snapshot of data as it exists at the start of the transaction when a transaction runs at the SNAPSHOT isolation level. A transaction might run at the SNAPSHOT isolation level and accesses data in multiple databases. If it runs at that level, either set ALLOW_SNAPSHOT_ISOLATION to ON in all the databases. Each statement in the transaction must use locking hints on any reference in a FROM clause to a database table where ALLOW_SNAPSHOT_ISOLATION is OFF if you don't set the option.
...
To retrieve current settings for database options, use the [sys.databases](../../relational-databases/system-catalog-views/sys-databases-transact-sql.md) catalog view. You can also determine the status by examining [DATABASEPROPERTYEX](../../t-sql/functions/databasepropertyex-transact-sql.md)
