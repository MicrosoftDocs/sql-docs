...
> [!NOTE]  
> Connections are persisted in the "Most Recently Used" (MRU) history. To remove entries from the MRU, select the **Server name** dropdown list, hover over the name of the server to remove, then press the `DEL` key on your keyboard.
...
If connecting with **Azure Active Directory - Universal with MFA**, **Azure Active Directory - Password**, **Azure Active Directory - Service Principal**, or **Azure Active Directory - Default**, enter the name of the Microsoft Entra identity you're connecting with.
