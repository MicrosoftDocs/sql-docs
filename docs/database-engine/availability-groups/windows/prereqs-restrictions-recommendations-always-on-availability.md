...
|![Checkbox](../../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|Ensure that the WSFC contains sufficient nodes to support your availability group configurations.|A cluster node can only host one availability replica for a single availability group - a cluster node is unable to host two replicas from the same availability group. However, a cluster node can host multiple SQL Server instance availability replicas from multiple availability groups as long as two replicas on that node do not belong to the same availability group.<br /><br /> Ask your database administrators how many cluster nodes are required for to support the availability replicas of the planned availability groups.<br /><br /> [Overview of Always On Availability Groups &#40;SQL Server&#41;](../../../database-engine/availability-groups/windows/overview-of-always-on-availability-groups-sql-server.md).|  
