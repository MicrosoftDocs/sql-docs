...
The primary system view used to monitor queries is [sys.dm_pdw_exec_requests](../relational-databases/system-dynamic-management-objects/sys-dm-pdw-exec-requests-transact-sql.md). Use this system view to find the `request_id` for an active or recent query, based on the query text.  
