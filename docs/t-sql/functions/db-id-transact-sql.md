...
> [!NOTE]
> When used with Azure SQL Database, `DB_ID` may not return the same result as querying **sys.databases**. If the caller of `DB_ID` is comparing the result to other **sys** views, then **sys.databases** should be queried instead.
