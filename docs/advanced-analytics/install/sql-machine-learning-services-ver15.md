...
As before, additional configuration is still required for *implied authentication*, where script or code executing externally has to reach back into SQL Server to retrieve data or resources not already available to the user calling the stored procedure or running the query. For more information, see [Add SQLRUserGroup as a database user](../r/add-sqlrusergroup-to-database.md)

> [!NOTE]
> If code script passes connection instructions, such as an ODBC connection string, back to SQL Server, the server refuses the request from Launchpad service by default. This occurs because there is no login created for Launchpad or the parent **SQLRUserGroup**. If you require an impersonation token for an identity other than the originall caller, a login for **SQLRUserGroup** is required.
