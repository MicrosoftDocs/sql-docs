...
> [!NOTE]
> A double-hop typically involves delegation of user credentials across multiple remote computers. For example, assume you have a SQL Server instance named *SQL1* where you created a linked server for a remote data source named *SQL2*. In linked server configuration, you use Windows Authentication to define the connection to the linked server. In this scenario, if you execute a linked server query from a remote client computer named *Client1*, the windows credentials will first have to hop from *Client1* to *SQL1* and then from *SQL1* to *SQL2* (hence, it's called a double-hop).
