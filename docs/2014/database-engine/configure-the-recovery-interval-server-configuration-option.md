...
-   The recovery interval affects only databases that use the default target recovery time (0). To override the server recovery interval on a database, configure a non-default target recovery time on the database. For more information, see [Change the Target Recovery Time of a Database &#40;SQL Server&#41;](../../2014/database-engine/change-the-target-recovery-time-of-a-database-sql-server.md).  
...
3.  Copy and paste the following example into the query window and click **Execute**. This example shows how to use [sp_configure](../Topic/sp_configure%20\(Transact-SQL\).md) to set the value of the `recovery interval` option to `3` minutes.  
...
## See Also  
 ...
 [show advanced options Server Configuration Option](../../2014/database-engine/show-advanced-options-server-configuration-option.md)   
