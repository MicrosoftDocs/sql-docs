...
## Configuring Sybase Console Settings  
...
7.  **Stop Operation:** During the mid-operation, if the user wants to stop the operation, then **‘Ctrl+C’** hotkey can be used. SSMA for Sybase Console will wait for the operation to complete and terminates the console execution.  
...
Server connection parameters can be defined in the script file or in the server connection file. Please refer to the [Creating the Server Connection Files &#40;SybaseToSQL&#41;](../../ssma/sybase/creating-the-server-connection-files-sybasetosql.md) section for more details  
