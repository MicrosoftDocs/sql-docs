...
|-|-|-|-|
|`Authentication`|(not set)|(not set)|Authentication mode determined by other keywords (existing legacy connection options.)|
|`Authentication`|(empty string)|(not set)|(Connection string only.) Override and unset an `Authentication` value set in the DSN.|
|`Authentication`|`SqlPassword`|(not set)|Directly authenticate to SQL using a username and password.|
|`Authentication`|`ActiveDirectoryPassword`|(not set)|Authenticate with a Microsoft Entra identity using a username and password.|
|`Authentication`|`ActiveDirectoryIntegrated`|(not set)|_Windows, and Linux/Mac 17.6+, driver only_. Authenticate with a Windows credential federated through Microsoft Entra ID with integrated authentication.|
|`Authentication`|`ActiveDirectoryInteractive`|(not set)|_Windows driver only_. Authenticate with a Microsoft Entra identity using interactive authentication.|
|`Authentication`|`ActiveDirectoryMsi`|(not set)|Authenticate with a Microsoft Entra managed identity. For user-assigned identity, UID is set to the object ID of the user identity. For system-assigned identity, UID isn't required.|
|`Authentication`|`ActiveDirectoryServicePrincipal`|(not set)|(17.7+) Authenticate with a Microsoft Entra  service principal. UID is set to the client ID of the service principal. PWD is set to the client secret.|
