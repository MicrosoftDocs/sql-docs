...
[Private Link](https://learn.microsoft.com/azure/private-link/private-link-overview) is an Azure technology that makes Azure SQL Managed Instance available in a virtual network of your choice. A network administrator can deploy [private endpoints](https://learn.microsoft.com/azure/private-link/private-endpoint-overview) to Azure SQL Managed Instance in another virtual network, while an administrator of that network will have a chance to accept or reject it before the endpoint goes live. Private endpoints establish secure, isolated connectivity between a service and multiple virtual networks without exposing your service's entire network infrastructure.

### How does a private endpoint differ from VNet-local endpoint?
The default endpoint deployed with each Azure SQL Managed Instance, VNet-local endpoint, behaves as if a computer running the service were physically attached to your virtual network. This means you have near-complete control over the traffic coming to and leaving from Azure SQL Managed Instance via route tables, network security groups, DNS resolution and similar mechanisms. You can also use this endpoint to involve your instance in scenarios requiring connectivity on ports other than 1433, such as linked server, failover groups, and MI Link.
...
- **Airlock**. Private endpoints to Azure SQL Managed Instance are deployed in a virtual network with jumpboxes and an ExpressRoute gateway, providing an additional layer of security and isolation between on-premises and cloud resources.
...
- **Publisher-consumer**. Publisher tenant (e.g. an ISV) manages multiple Azure SQL Managed Instances in their virtual networks. Publisher creates private endpoints in other tenants' virtual networks to make instances available to their consumers.
...
- Strong network isolation: In a peering scenario, peered virtual networks establish two-way connectivity, while private endpoints are unidirectional and do not expose network resources inside their network to Azure SQL Managed Instance.
...
- Private endpoint only allows connections to port 1433, which means that more involved connectivity, automation and integration scenarios will not work. This includes linked server scenarios, MI Link, failover groups, and similar.
- Compared to [VNet-local endpoint](connectivity-architecture-overview.md#vnet-local-endpoint), private endpoints require additional setup.

## Limitations

- Azure SQL Managed Instance requires that exact instance name be used in the connection string; using private endpoint's IP address instead will fail. Configure your DNS server to resolve this, or use a private DNS zone as described in [Set up domain name resolution for private endpoint](#set-up-domain-name-resolution-for-private-endpoint).
...
Once a request to create a private endpoint is made, the SQL administrator can manage the private endpoint connection to Azure SQL Managed Instance. The first step to managing a new private endpoint connection is to review and approve it. This step is automatic if the user or service creating the private endpoint has sufficient Azure RBAC permissions on the Azure SQL Managed Instance resource. If they do not, then the review and approval must be done manually:

1. Navigate to your Azure SQL Managed Instance in Azure Portal.
2. In the sidebar, select Private endpoint connections.
![](media/private-endpoint/private-endpoint-connections.png)
1. Review the connections in Pending state and select one or more private endpoint connections to approve or reject.
![](media/private-endpoint/pec-select.png)
1. Approve or reject selected private endpoint connection(s) with an optional text response.
![](media/private-endpoint/pec-approve.png)
1. After approving or rejecting, the list will reflect the state of selected private endpoint connection(s) along with any text response.
![](media/private-endpoint/pec-list-after.png)

### How to get the IP address of a private endpoint

As a locally available resource, private endpoint's IP address will always be in its subnet's range. You can find the IP address of a private endpoint to Azure SQL Managed Instance either by visiting Private Link Center or by doing the following: 

1. Navigate to your Azure SQL Managed Instance in Azure portal.
2. In the sidebar, select Private endpoint connections.
![](media/private-endpoint/private-endpoint-connections.png)
1. Locate the private endpoint connection in the table and select its name.
![](media/private-endpoint/pec-click.png)
1. In Overview, select the network interface.
![](media/private-endpoint/pec-nic-click.png)
1. In Overview, Private IP address is shown in the Essentials section.
![](media/private-endpoint/pec-ip-display.png)

### Set up domain name resolution for private endpoint

After you create a private endpoint to Azure SQL Managed Instance, you will need to configure domain name resolution so that SQL clients can retain the name of SQL Managed Instance in their connection strings. Otherwise, login attempts will fail. The method below works for virtual networks which do not use custom DNS servers. If you use a custom DNS server, follow its specific instructions instead.

To set up domain name resolution for private endpoint to an instance whose FQDN is `<instance-name>.<domain>`:

1. [Create a private Azure DNS zone](../../dns/private-dns-getstarted-portal.md#create-a-private-dns-zone):
   1. If the private endpoint is not in the same virtual network where your Azure SQL Managed Instance is and the two virtual networks are not peered, name this zone `privatelink.<domain>`.
   2. If the private endpoint is in the same virtual network where your Azure SQL Managed Instance is and the two virtual networks are not peered, name this zone something else, e.g. `plink.<domain>`.
> [!IMPORTANT]
> Do not name the private DNS zone `privatelink.<domain>` when the private endpoint is in the same virtual network as its Azure SQL Managed Instance because this will disrupt the instance's internal connectivity and cause management operations to fail.
2. In the DNS zone, create a new record set with the following properties:
   - Name: `<instance-name>`
   - Type: A
   - IP address: IP address of the private endpoint
3. Link this DNS zone with the virtual network of your private endpoint:
   1. On the left pane, select **Virtual network links**.

After you complete the above steps, you'll be able to connect to the instance's private endpoint at `<instance-name>.privatelink.<domain>` or `<instance-name>.plink.<domain>`. Additionally, if you used `privatelink.<domain>` as the name of the private DNS zone, `<instance-name>.<domain>` will automatically resolve to the IP address of the private endpoint in that virtual network.
