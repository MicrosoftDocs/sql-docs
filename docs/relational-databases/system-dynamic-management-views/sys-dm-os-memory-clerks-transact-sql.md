...
|MEMORYCLERK_QSRANGEPREFETCH     |  This memory clerk is used for allocations during query execution for query scan ragne prefetch.      |
...
|MEMORYCLERK_SQLBUFFERPOOL     |    This memory clerk keeps track of commonly the largest memory consumer inside [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] - data and index pages. Buffer Pool or data cache keeps data and index pages loaded in memory to provide fast access to data. See [Buffer Management](../memory-management-architecture-guide.md#buffer-management) for more information.     |
