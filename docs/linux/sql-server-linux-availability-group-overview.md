...
- 0 - Secondary replicas don't need to be in synchronized state with the primary. However if the secondaries aren't synchronized, there will be no automatic failover. 
...
Also new in [!INCLUDE[sssql17-md](../includes/sssql17-md.md)] as of CU1 is a configuration-only replica. Because Pacemaker is different than a WSFC, especially when it comes to quorum and requiring STONITH, having just a two-node configuration won't work when it comes to an AG. For an FCI, the quorum mechanisms provided by Pacemaker can be fine, because all FCI failover arbitration happens at the cluster layer. For an AG, arbitration under Linux happens in [!INCLUDE[ssnoversion-md](../includes/ssnoversion-md.md)], where all the metadata is stored. This is where the configuration-only replica comes into play.
...
- If the primary fails and `required_synchronized_secondaries_to_commit` is 0, the secondary replica will become the new primary and be available for both reading and writing. If the value is 1, automatic failover will occur, but won't accept new transactions until the other replica is online.
- If a secondary replica fails and `required_synchronized_secondaries_to_commit` is 0, the primary replica still accepts transactions, but if the primary fails at this point, there is no protection for the data nor failover possible (manual or automatic), since a secondary replica isn't available.
- If the configuration-only replica fails, the AG will function normally, but no automatic failover is possible.
- If both a synchronous secondary replica and the configuration-only replica fail, the primary can't accept transactions, and there is nowhere for the primary to fail to.
...
More than one AG can be created per Pacemaker cluster or set of servers. The only limitation is system resources. AG ownership is shown by the master. Different AGs can be owned by different nodes; they don't all need to be running on the same node.
...
:::image type="content" source="./media/sql-server-linux-availability-group-overview/image1.png" alt-text="Hybrid None.":::
...
:::image type="content" source="./media/sql-server-linux-availability-group-overview/image2.png" alt-text="Hybrid Dist AG.":::
