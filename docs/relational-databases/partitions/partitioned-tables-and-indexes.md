...
A range type (either LEFT or RIGHT), specifies how the boundary values of the partition function will be put into the resulting partitions. A left range specifies that the highest bounding value will be included within a partition. A right range specifies that the lowest bounding value will be included in each partition.

For example, the following partition function partitions a table or index into 12 partitions, one for each month of a year's worth of values in a **datetime** column. A RIGHT range is used, indicating that boundary values will serve as the lower bounding values in each partition.
...
Memory limitations can affect the performance or ability of the database engine to build a partitioned index. This is especially the case when the index is not aligned with its base table or is not aligned with its clustered index, if the table already has a clustered index applied to it. 
