...
To run SQL Server in a Linux container with a specific time zone, configure the `TZ` environment variable (see [How to configure time zone on Linux](./sql-server-linux-configure-time-zone.md) for more information). To find the right time zone value, run the `tzselect` command from a Linux bash prompt:
