...
The majority of this article is scoped towards upgrading the version of SQL Server. However, the in-place upgrade process should also be used when patching SQL Server with a service pack or cumulative update as well. 
 ...
 > 
 > When you have Distribution DB in FCI, then make sure all participating nodes are on the same build.  We don’t recommend setup where one node is on SQL Version lower than SQL Server 2016 SP2-CU3 or SQL Server 2017 CU6 and other node on SQL Version greater than SQL Server 2016 SP2-CU3 or SQL Server 2017 CU6. From SQL Server 2016 SP2-CU3 and SQL Server 2017 CU6 version onwards we have added support for Distribution DB in Availability group and to achieve that new objects (tables, stored procedures) have been added to Distribution database. If your Distribution DB is in FCI and you are going through phased migration (and cannot upgrade all nodes to same SQL Version) then for that narrow timeframe we recommend you to do activities like adding a new subscriber/subscription/publisher/publication on the node with higher SQL Version.
