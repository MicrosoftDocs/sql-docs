...
|**last_received_lsn**|**numeric(25,0)**|When querying the secondary replica, last_received_lsn is reported for the local secondary replica database row. Log block ID identifying the point up to which all log blocks have been received by the secondary replica that hosts this secondary database.<br /><br /> **last_received_lsn** reflects a log-block ID padded with zeroes. It is not an actual log sequence number.|  
|**last_received_time**|**datetime**|When querying the secondary replica, last_received_time is reported for the local secondary replica database row. Time when the log block ID in last message received was read on the secondary replica.|  
