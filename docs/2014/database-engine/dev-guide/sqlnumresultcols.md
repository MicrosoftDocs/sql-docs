...
---
# SQLNumResultCols
  For executed statements, the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client ODBC driver does not visit the server to report the number of columns in a result set. In this case, `SQLNumResultCols` does not cause a server roundtrip. Like [SQLDescribeCol](../../../2014/database-engine/dev-guide/sqldescribecol.md) and [SQLColAttribute](../../../2014/database-engine/dev-guide/sqlcolattribute.md), calling `SQLNumResultCols` on prepared but not executed statements generates a server roundtrip.  
