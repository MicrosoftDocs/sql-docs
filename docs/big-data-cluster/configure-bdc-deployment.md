...
```
## <a id="sqlagent"></a> Enable SQL Agent

You can enable SQL Agent in the master instance. The following example shows how to enable SQL Agent in the SQL Server master instance by patching the `bdc.json` configuration file.

> [!NOTE]
> Prior to CU9, some BDC configurations were done through an `mssql-custom.conf` file. These settings are now available to set through bdc.json. The scope of some of the settings has changed as well. Visit [insert reference docs to see settings available at each scope]().
