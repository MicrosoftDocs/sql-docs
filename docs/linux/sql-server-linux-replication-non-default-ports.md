...
'Server1' listens on port 1500 on Linux. To configure 'Server1' for distribution, we would run the `sp_adddistributor` with the @distributor like this

```sql
exec sp_adddistributor @distributor = 'Server1,1500'
```

'Server1' listens on port 1500 on Linux. To configure a publisher for the distributor, we would run the sp_adddistpublisher with the @publisher input like this

```sql
exec sp_adddistpublisher @publisher = 'Server1,1500' ,  ,  
```

'Server2' listens on port 6549 on Linux. To configure 'Server2' as a subscriber, we would run the sp_addsubscription with the @subscriber input like this

```sql
exec sp_addsubscription @subscriber = 'Server2,6549' ,  ,  
```

'Server3' listens on port 6549 on Windows with server name of Server3 and instance name of MSSQL2017. To configure 'Server3' as a subscriber, we would run the sp_addsubscription with the @subscriber input like this
