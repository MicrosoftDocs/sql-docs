...
![Diagram shows a typical configuration of a geo-redundant cloud application using multiple databases and a failover group.](./media/failover-group-overview/failover-group.png)
...
## <a name="using-one-or-several-failover-groups-to-manage-failover-of-multiple-databases"></a> Use multiple failover groups to fail over multiple databases
...
## <a name="preparing-for-data-loss"></a> Potential data loss after forced failover
...
Failover groups can also be managed programmatically using Azure PowerShell, Azure CLI, and REST API. Review [Configure failover group](failover-group-configure-sql-db.md) for more information.
