...
|[sys.fulltext_catalogs](../Topic/sys.fulltext_catalogs%20\(Transact-SQL\).md)|Using the path, data_space_id, and file_id of this catalog view.|These columns return a specific value.|These columns return NULL because the full-text catalog is no longer located in the file system.|  
|[sys.sysfulltextcatalogs](../Topic/sys.sysfulltextcatalogs%20\(Transact-SQL\).md)|Using the path column of this deprecated system table.|Returns the file system path of the full-text catalog.|Returns NULL because the full-text catalog is no longer located in the file system.|  
|[sp_help_fulltext_catalogs](../Topic/sp_help_fulltext_catalogs%20\(Transact-SQL\).md)<br /><br /> [sp_help_fulltext_catalogs_cursor](../Topic/sp_help_fulltext_catalogs_cursor%20\(Transact-SQL\).md)|Using the PATH column of these deprecated stored procedures.|Returns the file system path of the full-text catalog.|Returns NULL because the full-text catalog is no longer located in the file system.|  
|[sp_help_fulltext_catalog_components](../Topic/sp_help_fulltext_catalog_components%20\(Transact-SQL\).md)|Using sp_help_fulltext_catalog_components of this stored procedure.|Returns a list of all components (filters, word-breakers, and protocol handlers), used for all full-text catalogs in the current database.|Returns empty rows.|  
|[DATABASEPROPERTYEX](../Topic/DATABASEPROPERTYEX%20\(Transact-SQL\).md)|Using the **IsFullTextEnabled** property.|The **IsFullTextEnabled** setting indicates whether full-text search is enabled in a given database.|The value of this column has no effect. User databases are always enabled for full-text search.|  
