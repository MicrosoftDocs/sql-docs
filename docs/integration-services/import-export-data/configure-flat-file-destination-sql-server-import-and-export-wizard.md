...
## Screen shot of the Configure Flat File Destination page  
 ...
 In this example, the user wants to create a typical CSV (comma-separated values) file.
...
-   If you specified on a previous page that you want to copy tables, select the source table or view from the drop-down list.
-   If you provided a query, `"Query"` is selected and is the only option.  

## Specify row and column delimiters for the output
 **Row delimiter**  
 Select from the list of delimiters for rows in the output. There is no option to specify a custom row delimiter.  
 ...
 **Column delimiter**  
 Select from the list of delimiters for columns in the output. There is no option to specify a custom column delimiter.  
