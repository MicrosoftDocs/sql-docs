...
For the diagnostic log and failover cluster property options, `ALTER SETTINGS` and `VIEW SERVER PERFORMANCE STATE` permissions are also accepted in place of `VIEW SERVER STATE`.
...
#### E. Enforce strict encryption to your failover cluster instance

The examples in this section [forces encryption](#clusterconnectionoptions) in connections to a failover cluster instance.

If the server name is listed in the certificate, you can omit the `HostNameInCertificate` option: 
