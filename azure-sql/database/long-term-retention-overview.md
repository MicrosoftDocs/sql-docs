...
> [!NOTE]  
> - It's not currently possible to configure backups of Azure SQL Database and Azure SQL Managed Instance as [immutable](/azure/storage/blobs/immutable-storage-overview). LTR backups are non-modifiable, but can be deleted by user through Azure portal, Azure CLI, PowerShell or REST API. Visit [configure LTR backups](../managed-instance/long-term-backup-retention-configure.md) to learn more.
> - In Azure SQL Managed Instance, use SQL Agent jobs to schedule [copy-only database backups](/sql/relational-databases/backup-restore/copy-only-backups-sql-server?view=azuresqldb-mi-current&preserve-view=true) and keep them on your own storage account as an alternative to LTR beyond 10 years.
