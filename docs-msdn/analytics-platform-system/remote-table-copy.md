...
-   The data to be copied must be selectable using a single valid SQL Server PDW [SELECT](../../docs/t-sql/queries/select-transact-sql.md) statement.  
...
To perform a remote table copy, use the [CREATE REMOTE TABLE AS SELECT](../../docs/t-sql/statements/create-remote-table-as-select-parallel-data-warehouse.md) SQL statement. Examples are included with the CREATE REMOTE TABLE topic.  
