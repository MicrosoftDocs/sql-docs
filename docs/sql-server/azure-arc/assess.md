...
- If Azure Monitor Agent (AMA) agent already exists at the time of enabling BPA, the existing AMA agent and related AMA proxy settings are reused by BPA. However, if AMA is freshly installed as a result of enabling SQL BPA, proxy settings are not set for AMA automatically. To configure AMA to use a proxy server, re-deploy AMA with the necessary proxy settings. Review [AMA Network Settings and Proxy Configuration](/azure/azure-monitor/agents/azure-monitor-agent-data-collection-endpoint?tabs=ArmPolicy#proxy-configuration) for more information on AMA network and proxy settings.
