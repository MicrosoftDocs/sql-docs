...
> [!NOTE]
> RLS filter predicate is supported for Azure SQL Data Warehouse external tables.  
...
Create Azure SQL Data Warehouse external table from the Sales table in example A.

```sql
CREATE MASTER KEY ENCRYPTION BY PASSWORD = 'password';
CREATE DATABASE SCOPED CREDENTIAL WASBSecret WITH IDENTITY = 'identity_name', Secret = 'secret';  
CREATE EXTERNAL DATA SOURCE wasbsrc WITH (TYPE = HADOOP, LOCATION = 'wasbs://testprivate@dwtest.blob.core.windows.net/',CREDENTIAL = WASBSecret); 
CREATE EXTERNAL FILE FORMAT WASBFormat  WITH (FORMAT_TYPE=DELIMITEDTEXT);  
CREATE EXTERNAL TABLE Sales_ext WITH (LOCATION='RLSExtTabletest.tbl', DATA_SOURCE=wasbsrc, FILE_FORMAT=WASBFormat, REJECT_TYPE=Percentage, REJECT_SAMPLE_VALUE=100, REJECT_VALUE=100)
...
Now test the filtering predicate, by selecting from the Sales_ext external table as each user.
