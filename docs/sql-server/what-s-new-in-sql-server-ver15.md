...
|Governance setting for Power BI cache refreshes | The Power BI service caches dashboard tile data and report data for initial load of Live Connect reports, causing an excessive number of cache queries being submitted to SSAS. This release introduces the **ClientCacheRefreshPolicy** property, which allows you to override this behavior at the server level. To learn more, see [General Properties](/analysis-services/server-properties/general-properties). |
