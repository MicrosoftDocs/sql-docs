...
1. Replace its contents with the following code. Then add the appropriate values for your server, database, user, and password.
    ...
    const config = {
        user: 'sa', // best stored in an app setting such as process.env.DB_USER
        password: 'P@ssw0rd', // best stored in an app setting such as process.env.DB_PASSWORD
        server: 'localhost', // best stored in an app setting such as process.env.DB_SERVER
        port: 61433, // optional, defaults to 1433, best stored in an app setting such as process.env.DB_PORT
        database: 'AdventureWorksLT', // best stored in an app setting such as process.env.DB_NAME
        authentication: {
            type: 'default'
        },
        options: {
