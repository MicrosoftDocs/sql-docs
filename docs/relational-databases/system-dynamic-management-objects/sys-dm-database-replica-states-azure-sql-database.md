...
| `is_primary_replica` | **bit** | Returns `1` if the replica is primary, or `0` if it's a secondary replica in the availability group to which the database belongs. This doesn't refer to the primary or secondary database in a distributed availability group or an active geo-replication relation.<br /><br />**Applies to**: [!INCLUDE [ssSQL14](../../includes/sssql14-md.md)] and later versions. |
