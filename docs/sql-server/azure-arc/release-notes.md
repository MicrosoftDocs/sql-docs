...
- Azure extension for SQL Server is not supported in the following environments:
  ...
  - SQL Server in Azure VMs. If a custom VM image is migrated to Azure VM, Azure extension for SQL Server will stop working. The customers need to [Delete your Arc-enabled SQL Server resource](delete-from-azure-arc.md#delete-your-arc-enabled-sql-server-resource), and [Restore a deleted Arc-enabled SQL Server resource](delete-from-azure-arc.md#restore-a-deleted-arc-enabled-sql-server-resource).
