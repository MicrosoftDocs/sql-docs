...
This article provides general guidance on how to join AD domain for a SQL Server on a Linux host machine using two methods, built-in SSSD package and third-party Active Directory providers. Examples of third-party domain join products are [PowerBroker Identity Services (PBIS)](https://www.beyondtrust.com/), [One Identity](https://www.oneidentity.com/products/authentication-services/), and [Centrify](https://www.centrify.com/). This guide includes steps to check your Active Directory configuration. It's not intended to instruct on how to join a machine to a domain when using third-party utilities.
...
> [!IMPORTANT]
> The sample steps described in this article are for guidance only and may slightly differ in your environment depending on how overall environment is configured. It is recommended to engage system and domain administrator for your environment for configuration, customization and any required troubleshooting.
...
If this command doesn't return your host's FQDN, or if the FQDN is incorrect, add a reverse DNS entry for you are SQL Server on Linux host to your DNS server.
...
> [!NOTE]
> This is the preferred method of joining a Linux host to AD domain controller.

Use the following steps to join a SQL Server host to an Active Directory domain:

1. Use [realmd](https://www.freedesktop.org/software/realmd/docs/guide-active-directory-join.md) to join your host machine to your AD Domain. If you haven't already, install both the **realmd** and Kerberos client packages on the SQL Server host machine using your Linux distribution's package manager:
...
This article covers the prerequisites of how to configure a SQL Server on a Linux host machine with Active Directory Authentication. To finish configuring SQL Server on Linux to support Active Directory accounts, follow the instructions at [Use Active Directory authentication with SQL Server on Linux](sql-server-linux-active-directory-authentication.md).
