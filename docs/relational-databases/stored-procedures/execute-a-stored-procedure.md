...
Only the system administrator (`sa`) can mark a procedure to execute automatically. The procedure must be in the `master` database, must be owned by `sa`, and can't have input or output parameters.  
