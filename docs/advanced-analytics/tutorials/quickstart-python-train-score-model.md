...
1. Verify that the model was inserted Another way to return a list of models is
...
Now that you have created, trained, and saved a model, move on to the next step: creating a stored procedure that generates predictions. You'll do this by calling `sp_execute_external_script` to start Python, and then pass in Python script that loads a serialized model you created in the last exercise and gives it new data inputs to score.
