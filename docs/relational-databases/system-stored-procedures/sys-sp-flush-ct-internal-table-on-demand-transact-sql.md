...
[!INCLUDE [SQL Server](../../includes/applies-to-version/sqlserver.md)]
  ...
  This stored procedure allows you to manually clean the side table (change_tracking_objectid) for a table in a database for which change tracking is enabled. If the tablename parameter is not passed, then this process cleans all side tables for all tables in the database where change tracking is enabled.
