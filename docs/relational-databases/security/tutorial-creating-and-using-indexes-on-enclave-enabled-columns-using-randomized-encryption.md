...
- How to create an index when you do not have access to the keys protecting the column.

## Prerequisites
This tutorial is the continuation of [Tutorial: Getting Started with Always Encrypted with secure enclaves using SSMS](./tutorial-getting-started-with-always-encrypted-enclaves.md). Make sure you have completed it, before following the below steps.
...
1. Close any SSMS instances, you used in the previous tutorial. This will close database connections you have opened, which is required to enable ADR.
...
In this step, you will create and test an index on an encrypted column. You will be acting as a single user who is assuming the roles of both a DBA, who manages the database, and the data owner who has access to the keys, protecting the data.
...
1.  Open a new SSMS instance and connect to your SQL Server instance **with** Always Encrypted enabled for the database connection.
    ...
    6. Select **Connect**.
    7. If prompted to enable parameterization for Always Encrypted queries, click **Enable**.
1. If you were not prompted to enable Parameterization for Always Encrypted, verify it is enabled.
    ...
    5. Select **OK**.
1. Open a query window and execute the below statements to encrypt the **LastName** column in the **Employees** table. You will create and use an index on that column in later steps.
    ```sql
    USE [ContosoHR];
    GO
    ...
    GO
1. Create an index on the **LastName** column. Since you are connected to the database with Always Encrypted enabled, the client driver inside SSMS transparently provides **CEK1** (the column encryption key, protecting the **LastName** column) to the enclave, which is needed to created the index.
    ```sql
    USE [ContosoHR];
    GO
...
In this step, you will create an index on an encrypted column, pretending to be two different users. One user is a DBA, who needs to create an index, but does not have an access to the keys. The other user is a data owner, who has access to the keys.
