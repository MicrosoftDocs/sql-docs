...
For information about the physical and logical architecture of the transaction log, see the [SQL Server transaction log architecture and management guide](../sql-server-transaction-log-architecture-and-management-guide.md).

> [!TIP]  
> Checkpoints create known good points from which to begin applying transaction logs during database recovery. For more information, see [Database checkpoints (SQL Server)](database-checkpoints-sql-server.md).
