...
4.  On the **General** page, use the **Source** section to specify the source and location of the backup sets to restore. Select one of the following options:  
    ...
    -   **Device**  
        ...
        |**URL**|This launches two dialog boxes in the following order:<br /><br /> 1) **Connect to Windows Azure Storage**<br /><br /> 2) **Locate Backup File in Windows Azure**|In the **Connect to Windows Azure Storage**  dialog box, Select an existing SQL Credential that stores the Windows Azure storage account name and access key information, or create new SQL Credential by specifying the storage account name and storage access key information. For more information, see [Connect to Windows Azure Storage &#40;Restore&#41;](../relational-databases/backup-restore/connect-to-microsoft-azure-storage-restore.md).<br /><br /> In the **Locate Backup File** dialog box, you can select a file from the list of containers displayed on the left frame.|  
