...
|CXPACKET <a name="cxpacket"></a>|Occurs with parallel query plans when synchronizing the query processor Exchange Iterator, and when producing and consuming rows. If waiting is excessive and cannot be reduced by tuning the query (such as adding indexes), consider adjusting the Cost Threshold for Parallelism or lowering the Max Degree of Parallelism (MaxDOP).<br /> **Note:** Starting with [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2, [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] CU3, and [!INCLUDE[ssSDS](../../includes/sssds-md.md)], CXPACKET only refers to synchronizing the query processor Exchange Iterator and when producing rows. If the consumer threads are too slow, the Exchange Iterator buffer may become full and cause CXPACKET waits. Consumer threads are tracked separately in the CXCONSUMER wait type.| 
