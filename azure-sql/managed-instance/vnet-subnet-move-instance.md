...
- SQL Managed Instance can be moved to the subnet that is either:
 1. Empty
 2. Specially prepared subnet that retains the DNS zone of SQL Managed Instance that is being moved. This can be done by populating an empty subnet with new SQL Managed Instances that are created using dnsZonePartner parameter which is to be set to resource_id of the SQL Managed Instance that you want to move.
