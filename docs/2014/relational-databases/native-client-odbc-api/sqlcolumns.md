...
|SS_UDT_ASSEMBLY_TYPE_NAME|The assembly-qualified name of the UDT.|  
 ...
 For UDTs, the existing TYPE_NAME column is used to indicate the name of the UDT; therefore no additional column for it should be added to the result set of `SQLColumns` or [SQLProcedureColumns](sqlprocedurecolumns.md). The DATA_TYPE for a UDT column or parameter is SQL_SS_UDT.  
...
## SQLColumns and Table-Valued Parameters  
 The result set returned by SQLColumns depends on the setting of SQL_SOPT_SS_NAME_SCOPE. For more information, see [SQLSetStmtAttr](sqlsetstmtattr.md). The following columns have been added for table-valued parameters:  
...
|SS_IS_COLUMN_SET|`Smallint`|If the column is the `column_set` column, this is SQL_TRUE; otherwise, SQL_FALSE.|  
 ...
 The result set returned by SQLColumns depends on the setting of SQL_SOPT_SS_NAME_SCOPE. For more information, see [SQLSetStmtAttr](sqlsetstmtattr.md).  
