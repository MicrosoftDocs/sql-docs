...
description: sys.http_endpoints returns a row for each endpoint created in the server that uses the HTTP protocol.
author: rwestMSFT
...
Returns a row for each endpoint created in the server that uses the HTTP protocol.
...
| `authorization_realm` | **nvarchar(128)** | Hint that is returned to the client as part of the HTTP DIGEST authentication challenge. The value of the AUTH REALM option.<br /><br />Is NULL if not specified or if DIGEST authentication isn't enabled. |
| `default_logon_domain` | **nvarchar(128)** | Default login domain if you enable BASIC authentication. The value of the DEFAULT LOGON DOMAIN option.<br /><br />Is NULL if not specified or if BASIC authentication isn't enabled. |
