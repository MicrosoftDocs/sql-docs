...
description: sys.soap_endpoints contains one row for each endpoint in the server that carries a SOAP-type payload.
author: rwestMSFT
...
Returns one row for each endpoint in the server that carries a SOAP-type payload. For every row in this view, there's a corresponding row with the same `endpoint_id` in the `sys.http_endpoints` catalog view that carries the HTTP configuration metadata.
...
|**wsdl_generator_procedure**|**nvarchar(776)**|The three-part name of the stored procedure that implements this method.<br /><br /> Names of methods require strict three-part syntax. One-, two-, or four-part names aren't allowed.|  
