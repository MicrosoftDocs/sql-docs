...
2. Configure delegation for the C2WTS service account.
    ...
    * Right-click each service account and open the properties dialog. In the dialog, select the **Delegation** tab.
        ...
        The delegation tab is only visible if the object has a Service Principal Name (SPN) assigned to it. C2WTS doesn't require an SPN on the C2WTS Account, however, without an SPN, the **Delegation** tab will not be visible. An alternative way to configure constrained delegation is to use a utility such as **ADSIEdit**.
