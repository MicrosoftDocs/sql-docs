...
To read values mapped to any JDBC basic type as a string, or when a strongly typed value isn't required, use the [getString](reference/getstring-method-sqlserverresultset.md) method of the [SQLServerResultSet](reference/sqlserverresultset-class.md) class. The following example shows this usage:
...
> [!NOTE]
> The getUnicodeStream and getBigDecimal with scale methods are deprecated and aren't supported by the JDBC driver.
...
> [!NOTE]
> The JDBC driver can't update a SQL Server column with a column name that is more than 127 characters long. If an update to a column whose name is more than 127 characters is attempted, an exception is thrown.
...
When using BigDecimal parameter values, the precision and scale can be passed along with the value through `setBigDecimal`. Using `setBigDecimal` this way avoids potential value truncation. Alternatively, if the connection string option `calcBigDecimalPrecision` is set to `true`, the driver calculates precision for the BigDecimal input on the user's behalf, at the cost of performance. If the value is passed in alone, without `calcBigDecimalPrecision` set, or the option set to `false`, the driver assumes the maximum allowed value (38) for precision for that BigDecimal value.
