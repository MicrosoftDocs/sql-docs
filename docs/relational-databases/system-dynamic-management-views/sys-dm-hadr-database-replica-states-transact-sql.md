...
|**last_received_lsn**|**numeric(25,0)**|When querying a secondary replica, `last_received_lsn` is reported for the local secondary replica database row. Log block ID identifying the point up to which all log blocks have been received by the secondary replica that hosts this secondary database.<br /><br /> `last_received_lsn` reflects a log-block ID padded with zeroes. It is not an actual log sequence number.|  
