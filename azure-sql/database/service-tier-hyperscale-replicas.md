...
2. On the SQL Database page, select your database, scroll to **Data management**, select **Replicas**, and then select **Create replica**.
    ...
    :::image type="content" source="./media/named-replicasconfigure-portal\azure-create-named-replicas.png" alt-text="Configure named replica":::
3. Choose Named replica under Replica configuration, select or create the server for the named replica, enter named replica database name and configure the **Compute + storage** options if necessary.
    ...
    :::image type="content" source="./media/named-replicasconfigure-portal/azure-choose-named-replica.png" alt-text="{alt-text}":::

4. Click **Review + create**, review the information, and then click **Create**.
5. The named replica deployment process begins.

    :::image type="content" source="./media/named-replicasconfigure-portal/azure-deployment-named-replica.png" alt-text="Screenshot that shows the deployment status of the named replica.":::

6. When the deployment is complete, the named replica displays its status.

    :::image type="content" source="./media/named-replicasconfigure-portal/azure-deploymentcomplete-named-replica.png" alt-text="Screenshot that shows the named replica status after deployment.":::
...
# [T-SQL](#tab/tsql)
```sql
ALTER DATABASE [guestlist]
ADD SECONDARY ON SERVER [contosoeast] 
WITH (SERVICE_OBJECTIVE = 'HS_Gen5_2', SECONDARY_TYPE = Named, DATABASE_NAME = [guestlist_NamedReplica]);
```
# [PowerShell](#tab/azure-powershell)
```azurepowershell
New-AzSqlDatabaseSecondary -ResourceGroupName "MyResourceGroup" -ServerName "contosoeast" -DatabaseName "guestlist" -PartnerResourceGroupName "MyResourceGroup" -PartnerServerName "contosoeast" -PartnerDatabaseName "guestlist_NamedReplica" -SecondaryType Named -SecondaryServiceObjectiveName HS_Gen5_2
```
# [Azure CLI](#tab/azure-cli)
```azurecli
az sql db replica create -g MyResourceGroup -n guestlist -s contosoeast --secondary-type named --partner-database guestlist_NamedReplica --partner-server contosoeast --service-objective HS_Gen5_2
```
...
You can define the service level objective of a named replica when you create it, via the `ALTER DATABASE` command or in any other supported way (Portal, AZ CLI, PowerShell, REST API). If you need to change the service level objective after the named replica has been created, you can do it using the `ALTER DATABASE ... MODIFY` command on the named replica itself. For example, if `guestlist_NamedReplica` is the named replica of `guestlist` database, you can do it as shown below. 

# [Portal](#tab/portal)

Open named replica database page, and then select **Compute + storage**. Update the vCores.
    :::image type="content" source="./media/named-replicasconfigure-portal/azure-update-named-replica.png" alt-text="{alt-text}":::

# [T-SQL](#tab/tsql)
```sql
ALTER DATABASE [guestlist_NamedReplica] MODIFY (SERVICE_OBJECTIVE = 'HS_Gen5_4')
```
# [PowerShell](#tab/azure-powershell)
```azurepowershell
Set-AzSqlDatabase -ResourceGroup "MyResourceGroup" -ServerName "contosoeast" -DatabaseName "guestlist_NamedReplica" -RequestedServiceObjectiveName "HS_Gen5_4"
```
# [Azure CLI](#tab/azure-cli)
```azurecli
az sql db update -g MyResourceGroup -s contosoeast -n guestlist_NamedReplica --service-objective HS_Gen5_4
```
...
Open named replica database page, and choose `Delete` option.
    :::image type="content" source="./media/named-replicasconfigure-portal/azure-delete-named-replicas.png" alt-text="{alt-text}":::
...
```sql
DROP DATABASE [guestlist_NamedReplica];
```
# [PowerShell](#tab/azure-powershell)
```azurepowershell
Remove-AzSqlDatabase -ResourceGroupName "MyResourceGroup" -ServerName "contosoeast" -DatabaseName "guestlist_NamedReplica"
```
# [Azure CLI](#tab/azure-cli)
```azurecli
az sql db delete -g MyResourceGroup -s contosoeast -n guestlist_NamedReplica
```
---
