...
The Open Database Connectivity (ODBC) Programmer's Reference provides comprehensive documentation for developing applications that use the ODBC interface to connect to SQL Server and other data sources.

## Quick-start ODBC connection string examples

If you need a basic ODBC connection string to connect to SQL Server, use one of the following formats:
...
The ODBC interface is designed for use with the C programming language. Use of the ODBC interface spans three areas: SQL statements, ODBC function calls, and C programming. This documentation assumes the following knowledge:

- A working knowledge of the C programming language.
- General database management system (DBMS) knowledge and a familiarity with SQL.
...
The code examples in this guide are designed for illustration purposes only. Because they're written primarily to demonstrate ODBC principles, efficiency is sometimes set aside in the interest of clarity. In addition, whole sections of code are sometimes omitted for clarity. These omissions include the definitions of non-ODBC functions (those functions whose names don't start with "SQL") and most error handling.
...
For more information about ODBC, many books are available, including:
