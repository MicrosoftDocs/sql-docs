...
## Exporting Data Mining Structures  
 ...
 If the mining structure has been processed and the results have been cached, which is the default behavior, when you export the mining structure, the definition contains a summary of the data on which the structure is based. To remove this summary, you must clear the cache associated with the mining structure by performing a `Process Clear Structure` operation. For more information, see [Process a Mining Structure](data-mining/process-a-mining-structure.md).  
