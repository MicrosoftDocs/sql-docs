...
|--------|-------------|-------------|
| ADAL Output Window Trace Level | **Information** <br> **None** <br> **Verbose** <br> **Warning** | The level of Azure Active Directory (Azure AD) login tracing to send to the Output window. |
| Allow single sign-on to SQL using Azure AD authentication | **True** <br> **False** | When set, the first connection to an MFA-protected SQL instance may use a locally cached credential to avoid the need for a PIN or other MFA option. |
...
| Use System Browser | **True** <br> **False** | Disable to launch the legacy embedded browser instead of an external browser. |

## Resource Management

| Option | Information | Description |
|--------|-------------|-------------|
| Active Directory Authority | `https://login.microsoftonline.com` | Specifies the base authority for Azure Active Directory (Azure AD) authentication. |
