...
Error 19407 is raised when the communication between SQL Server and the Windows Server Failover cluster is lost and a corrective action takes place. Typically the corrective action is a failover to another Always On node. The  

A lease is a time-based communication that takes place between the SQL Server and the Windows Server Failover Cluster (WSFC) process. The two processes communicate with each other periodically to ensure the other process is running and responding. This communication takes place using Windows [event objects](/windows/win32/sync/event-objects) and ensures that a fail over of the AG resource does not occur without the knowledge of the WSFC. If one the processes does not respond to the lease communication based on a predefined lease period, a lease timeout occurs. For detailed information see [Lease Mechanism](../../database-engine/availability-groups/windows/availability-group-lease-healthcheck-timeout.md)


### Causes

Events are light-weight Windows synchronization objects and there is relatively small number of factors that impact them negatively. Typical issues can can cause the lease timeout involve system-wide issues. Here's a list of possibilities that can lead lease expiration and cause a restart or failover:
...
- SQL process not responding while generating a large memory dump, 
- Entire Operating System not responding
