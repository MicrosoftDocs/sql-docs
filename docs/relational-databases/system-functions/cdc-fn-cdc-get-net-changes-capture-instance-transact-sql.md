 ...
 **Wait, what is an LSN?** Every record in the [SQL Server transaction log](/sql-docs/docs/relational-databases/logs/the-transaction-log-sql-server) is uniquely identified by a log sequence number (LSN). LSNs are ordered such that if LSN2 is greater than LSN1, the change described by the log record referred to by LSN2 occurred **after** the change described by the log record LSN.  
