...
1. **Does SQL Server on Linux support multiple instances on the same host?**
   ...
   We recommend running multiple containers on a host to have multiple distinct instances. This is easily achieved using docker, but each container needs to listen on a different port. For more information, see [Run multiple SQL Server containers](./sql-server-linux-docker-container-deployment.md#run-multiple-sql-server-containers).
