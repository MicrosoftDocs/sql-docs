...
1. If a UDF references built-in functions such as `SCOPE_IDENTITY()`, `@@ROWCOUNT`, or `@@ERROR`, the value returned by the built-in function will change with inlining. This change in behavior is because inlining changes the scope of statements inside the UDF. Beginning in [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)] CU2, we will block inlining if the UDF references certain intrinsic functions (e.g. @@ROWCOUNT).
