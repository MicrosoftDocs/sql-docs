...
|`MemorySafetyMargin`|Specifies a percentage of `WorkingSetMaximum` that defines the boundary between medium and low pressure scenarios. The default value is 80. For more information about `WorkingSetMaximum` and configuring available memory, see [Configure Available Memory for Report Server Applications](../configure-available-memory-for-report-server-applications.md).|N,S|  
|**MemoryThreshold**|Specifies a percentage of `WorkingSetMaximum` that defines the boundary between high and medium pressure scenarios. The default value is **90**. This value should be greater than the value set for `MemorySafetyMargin`. For more information, see [Configure Available Memory for Report Server Applications](../configure-available-memory-for-report-server-applications.md).|N,S|  
