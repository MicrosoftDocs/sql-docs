...
description: The sys.database_event_session_events dynamic management view (DMV) returns a row for each event in a database-scoped event session.
author: rwestMSFT
...
The `sys.database_event_session_events` dynamic management view (DMV) returns a row for each event in a database-scoped event session. For information on events in *active* database-scoped sessions, see [sys.dm_xe_database_session_events](../system-dynamic-management-objects/sys-dm-xe-database-session-events-azure-sql-database.md).
