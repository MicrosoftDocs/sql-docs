...
# Processing Objects (XMLA)
 ...
 To process an [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] object, you can use the [Process](../dev-guide/process-element-xmla.md) command. The `Process` command can process the following objects on an [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] instance:  
...
## Specifying Out-of-Line Bindings  
 If the `Process` command is not contained by a `Batch` command, you can optionally specify out-of-line bindings in the [Bindings](../dev-guide/bindings-element-xmla.md), [DataSource](../dev-guide/datasource-element-xmla.md), and [DataSourceView](../dev-guide/datasourceview-element-xmla.md) properties of the `Process` command for the objects to be processed. Out-of-line bindings are references to data sources, data source views, and other objects in which the binding exists only during the execution of the `Process` command, and which override any existing bindings associated with the objects being processed. If out-of-line bindings are not specified, the bindings currently associated with the objects to be processed are used.  
