...
| `##MS_DatabaseManager##` | This is a highly privileged role. Members of the `##MS_DatabaseManager##` fixed server role can create databases and they can drop databases they own. A member of the `##MS_DatabaseManager##` role that creates a database, becomes the owner of that database, which allows that user to connect to that database as the `dbo` user. The `dbo` user has all database permissions in the database. Furthermore, members of the `##MS_DatabaseManager##` has ALTER permissions in other databases they have permission to connect to. Note that this includes system databases like `msdb` where the `guest` user is enabled. This permission permits members of `##MS_DatabaseManager##` to create, alter and drop any type of object in all databases they can connect to. However, with no other permission than membership in `##MS_DatabaseManager##` they have no permission to view or modify data, nor to execute stored procedures. |
