...
|Distributed availability group |- Changing `REQUIRED_SYNCHRONIZED_SECONDARIES_TO_COMMIT` is supported. This helps ensure a zero dataa loss failover of the Distributed Availability Group. For more information, visit [ALTER AVAILABILITY GROUP ()](../t-sql/statements/alter-availability-group-transact-sql.md)<br/> - Now using multiple TCP connections for better network bandwidth utilization across a remote link with long tcp latencies.|
