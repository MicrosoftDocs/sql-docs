...
|initial_job_object_memory_limit_mb |int, null |Job object memory limit on database engine startup, in megabytes. For more information on Job Objects, see [Resource governance in Azure SQL Database](https://azure.microsoft.com/blog/resource-governance-in-azure-sql-database/). |
...
## Permissions
 For Azure SQL Database, this view can only be accessed by users with the VIEW DATABASE STATE permission.
 For Azure SQL Managed Instance, this view can only be accessed by users with the VIEW SERVER STATE or VIEW DATABASE STATE permission.

## Remarks

Each row in this view represents a recent out of memory (OOM) event occurring in the database engine. Not all OOM events may be captured. Older OOM events may disappear from the result set as more recent OOM events occur. Result set is not persisted across restarts of the database engine.
