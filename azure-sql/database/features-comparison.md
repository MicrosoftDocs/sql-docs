...
| [Logins and users](/sql/relational-databases/security/authentication-access/principals-database-engine) | Yes, but `CREATE` and `ALTER` login statements don't offer all the options (no Windows and server-level Azure Active Directory logins). `EXECUTE AS LOGIN` isn't supported - use `EXECUTE AS USER` instead.  | Yes, with some [differences](../managed-instance/transact-sql-tsql-differences-sql-server.md#logins-and-users). [Windows logins are supported via Kerberos](../managed-instance/winauth-implementation-aad-kerberos.md) - the process requires Active Directory synchronization to Azure Active Directory. |
...
| [Transactional Replication](../managed-instance/replication-transactional-overview.md) | Yes, [Transactional and snapshot replication subscriber only](migrate-to-database-from-sql-server.md) | Yes - see [limitations](../managed-instance/transact-sql-tsql-differences-sql-server.md#replication). |
...
| --- | --- | --- |
| **Active geo-replication** | [Yes](active-geo-replication-overview.md) - all service tiers. | No - see [Auto-failover groups](auto-failover-group-sql-db.md) as an alternative. |
| **Auto-failover groups**| [Yes](auto-failover-group-sql-db.md) - all service tiers. | Yes - see [Auto-failover groups](../managed-instance/auto-failover-group-sql-mi.md).|
...
| **Automatic backups** | [Yes](automated-backups-overview.md). Full backups are taken every 7 days, differential 12 hours, and log backups every 5-10 min. | [Yes](../managed-instance/automated-backups-overview.md). Full backups are taken every 7 days, differential 12 hours, and log backups every 5-10 min. |
| **Automatic tuning (indexes)**| [Yes](automatic-tuning-overview.md)| No |
...
| **Short-term backup retention** | [Yes](automated-backups-overview.md#short-term-retention). 7 days default, max 35 days. | [Yes](../managed-instance/automated-backups-overview.md#short-term-retention). 1-35 days, 7 days default.  |
...
| **Elastic jobs** | Yes - see [Elastic jobs](elastic-jobs-overview.md) (public preview) | No. [SQL Agent](../managed-instance/transact-sql-tsql-differences-sql-server.md#sql-server-agent) can be used instead. |
...
| **Geo-restore**| [Yes](recovery-using-backups.md#geo-restore) | [Yes](../managed-instance/recovery-using-backups.md#geo-restore) |
| **Long-term retention - LTR **| [Yes](long-term-retention-overview.md), keep automatically taken backups up to 10 years. | [Yes](long-term-retention-overview.md), keep automatically taken backups up to 10 years. |
| **Pause/resume** | Yes, in [serverless model](serverless-tier-overview.md) | Yes, with [Stop / start preview](../managed-instance/instance-stop-start-how-to.md) |
...
| [Point in time database restore](/sql/relational-databases/backup-restore/restore-a-sql-server-database-to-a-point-in-time-full-recovery-model) | [Yes](recovery-using-backups.md#point-in-time-restore) | [Yes](../managed-instance/recovery-using-backups.md#point-in-time-restore) |
| **Resource pools** | Yes, as [Elastic pools](elastic-pool-overview.md) | Yes. A single instance of SQL Managed Instance can have multiple databases that share the same pool of resources. In addition, you can deploy multiple instances of SQL Managed Instance in [instance pools](../managed-instance/instance-pools-overview.md) (public preview) that can share the resources. |
...
| **SQL Data Sync**| [Yes](sql-data-sync-sql-server-configure.md)  | No, use [Transactional Replication](../managed-instance/replication-transactional-overview.md) instead.  |
...
| **Private connectivity** | Yes, using [Private Link](/azure/private-link/private-endpoint-overview) | Yes, using [VNet-local endpoint or private endpoint](../managed-instance/connectivity-architecture-overview.md?#communication-overview) | 
...
| --- | --- | --- |
| SQL Server (on-premises, AzureVM, Amazon RDS) | **Online:**  [Transactional Replication](../managed-instance/replication-transactional-overview.md) <br/> **Offline:** [Data Migration Service (DMS)](/sql/dma/dma-overview), [BACPAC file (import)](/sql/relational-databases/data-tier-applications/import-a-bacpac-file-to-create-a-new-user-database), BCP | **Online:** [Managed Instance link](../managed-instance/managed-instance-link-feature-overview.md), [Log Replay Service](../managed-instance/log-replay-service-overview.md), [Azure Data Migration Service (DMS)](/azure/dms/tutorial-sql-server-managed-instance-online-ads), [Transactional Replication](../managed-instance/replication-transactional-overview.md) <br/> **Offline:** [Azure Database Migration Service](/azure/dms/tutorial-sql-server-managed-instance-offline-ads) Native backup/restore, [BACPAC file (import)](/sql/relational-databases/data-tier-applications/import-a-bacpac-file-to-create-a-new-user-database), BCP, [Snapshot replication](../managed-instance/replication-transactional-overview.md) |
