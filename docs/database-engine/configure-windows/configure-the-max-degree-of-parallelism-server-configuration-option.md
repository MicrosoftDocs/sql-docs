...
However, in Azure SQL, the default MAXDOP setting for each new single database and elastic pool database is 8. For more on MAXDOP in [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)], see [onfigure the max degree of parallelism (MAXDOP) in Azure SQL Database](/azure-sql/database/configure-the-max-degree-of-parallelism-server.md).
