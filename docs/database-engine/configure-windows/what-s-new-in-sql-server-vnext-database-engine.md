...
-  [!INCLUDEssNoVersion] now offers graph database capabilities to model many-to-many relationships. This includes new [CREATE TABLE](../../docs/t-sql/statements/create-table-sql-graph.md) syntax for creating node and edge tables, and the keyword [MATCH](../../docs/t-sql/statements/match-sql-graph.md) for queries. For more information, see [Graph Processing with SQL Server 2017](docs/relational-databases/graphs/sql-graph-overview.md).
