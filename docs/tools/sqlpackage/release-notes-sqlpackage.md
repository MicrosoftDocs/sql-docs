...
|Deployment|SqlPackage.exe on .NET Core for Windows, macOS, and Linux fails during a publish operation with an error message "Unrecognized configuration section system.diagnostics" when in-place encryption is used.|Remove the file `sqlpackage.dll.config` from the SqlPackage folder.|
