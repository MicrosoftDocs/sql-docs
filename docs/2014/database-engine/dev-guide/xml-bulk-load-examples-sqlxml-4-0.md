...
## J. Bulk loading in xml Data Type columns  
 If the mapping schema specifies a [xml data type](../Topic/xml%20\(Transact-SQL\).md) column by using the `sql:datatype="xml"` annotation, XML Bulk Load can copy XML child elements for the mapped field from the source document into this column.  
