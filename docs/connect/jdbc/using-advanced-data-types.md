...
The [!INCLUDE[jdbcNoVersion](../../includes/jdbcnoversion_md.md)] uses the JDBC advanced data types to convert the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] data types to a format understood by the Java programming language.
...
When you retrieve a binary large-value data type-such as the **varbinary(max)** data type-from a database, there are several approaches that you can take. The most efficient approach is to read the data as a binary stream, as in the following example:
...
You can also use the [getBytes](reference/getbytes-method-sqlserverresultset.md) method to read the data as a byte array, as in the  example:
...
> [!NOTE]
> You can also read the data as a BLOB. However, reading a BLOB is less efficient than the two methods shown previously.
...
Using a String or bytes is also an option, as in the following example:
...
> [!NOTE]
> Using either the CLOB or BLOB method isn't an efficient way to upload large data.
...
If you have to replace the instance of a word in a large text file, such as an archived HTML file, you can use a Clob object, as in the following example:
...
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] requires a leading BOM for UTF-16-encoded XML. The application must provide the leading BOM when XML parameter values are supplied as byte arrays. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] always outputs XML values as UTF-16 strings with no BOM or embedded encoding declaration. When XML values are retrieved as byte[], BinaryStream, or Blob, a UTF-16 BOM is prepended to the value.
