...
description: sys.service_broker_endpoints contains one row for the Service Broker endpoint.
author: rwestMSFT
...
This catalog view contains one row for the Service Broker endpoint. For every row in this view, there's a corresponding row with the same `endpoint_id` in the `sys.tcp_endpoints` view that contains the TCP configuration metadata. TCP is the only allowed protocol for Service Broker.
...
| `is_message_forwarding_enabled` | **bit** | Specifies whether the endpoint supports message forwarding. This value is initially set to **0** (disabled). Not nullable. |
| `message_forwarding_size` | **int** | The maximum number of megabytes of `tempdb` space allowed to be used for messages being forwarded. This value is initially set to **10**. Not nullable. |  
