---
title: "Setup Alerts and Notifications in Azure Portal"
description: Use the Azure portal to create Azure SQL Managed Instance alerts, which can trigger notifications or automation when the conditions you specify are met.
...
### Alert metrics available for Azure SQL Managed Instance

> [!IMPORTANT]  
> Alerting metrics are available for the SQL managed instance only. Alerting metrics for individual databases in a SQL managed instance aren't available.
> Database diagnostics telemetry is on the other hand available in the form of [diagnostics logs](../database/metrics-diagnostic-telemetry-logging-streaming-export-configure.md#diagnostic-telemetry-for-export).
...
| **Storage space reserved** | Current max. storage space reserved for the SQL managed instance. Changes with resource scaling operation. | MB (Megabytes) |
...
| **Virtual core count** | vCores provisioned for the SQL managed instance. Changes with resource scaling operation. | 4-80 (vCores) |

### Create an alert rule on a metric with the Azure portal

1. In the [Azure portal](https://portal.azure.com/), locate the SQL managed instance you're interested in monitoring, and select it.
...
1. In the **When to evaluate** section:
  ...
  | **Lookback period** | Choose the lookback period, which is the time period to look back at each time the data is checked. For example, every 1 minute you'll be looking at the past 5 minutes. |
   ...
   In the example shown in the screenshot, the value of 1,840,876 MB is used to represent a threshold value of 1.8 TB. As the operator in the example is set to greater than, the alert will be triggered if the storage space consumption on the SQL managed instance goes over 1.8 TB. The threshold value for storage space metrics must be expressed in MB.
...
1. In the **Actions** tab, select or create the required action group. This action defines what happens upon triggering an alert (for example, sending an email or calling you on the phone). Select an existing [Action group](/azure/azure-monitor/alerts/action-groups) from the **Select action groups** pane, or **Create action group** in your subscription.
   ...
   1. To review your settings, select the **Review + create** tab. This step quickly checks your inputs to make sure that you entered all required information. If there are issues, they're reported here. After you review the settings, select **Create** to create the action group.
...
1. In the **Details** tab, fill in the alert rule details and settings for your records and select the severity type. You can also use the **Custom properties** to add your own properties to the alert rule.
...
1. In the [Azure portal](https://portal.azure.com/), locate the resource you're interested in monitoring and select it.
...
1. On the **Create an alert rule** page, the **Scope** is automatically configured to the resource.
   ...
   > [!NOTE]  
   > To avoid the sprawl of many independent alerts, you might want to configure alerts for all resources of a type in a subscription, resource group, or an Azure SQL Database logical server. Use the **Scope** tab to change the scope of the new alert rule.
...
1. Review the sample Kusto query. Use the **Logs** query window to sample the query's results. When you're satisfied with your KQL query, select **Continue Editing Alert**.
...
After you've defined the alert condition, continue with these steps to create the alert rule: 
...
1. Under **Advanced options**:
   1. Make sure to select **Enable upon creation** for the alert rule to start running as soon as you're done creating it.
   1. The **Automatically resolve alerts** option is enabled by default. This option makes the alert stateful, which means that the alert is resolved when the condition isn't met anymore.
1. Select **Next: Tags >**.
...
Upon setting up an alerting rule, verify that you're satisfied with the alerting trigger and its frequency. For the example shown on this page for setting up an alert on storage space used, if your alerting option was email, you might receive an email like the following example:
...
- Select **View Resource >** to view the SQL managed instance for which the alert was triggered.

<a id="view-suspend-activate-modify-and-delete-existing-alert-rules"></a>

## Manage alert rules

> [!NOTE]  
> Existing alerts need to be managed from the **Alerts** menu in the Azure portal resource menu. Existing alerts can't be modified from the Azure SQL Managed Instance resource pane.
