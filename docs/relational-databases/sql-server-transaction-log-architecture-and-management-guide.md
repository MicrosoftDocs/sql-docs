...
- If the next growth is more than 1/8 of the current log size, use the pre-2014 method, namely:
  - If growth is less than 64 MB, create 4 VLFs that cover the growth size (for example, for 1-MB growth, create 4 VLFs of size 256 KB).
    - In [!INCLUDE[ssSDSfull](../includes/ssazure-sqldb.md)], and starting with [!INCLUDE[sssql22-md](../includes/sssql22-md.md)] (all editions), the logic is slightly different. If the growth is less than or equal to 64 MB, the Database Engine creates only one VLF to cover the growth size.
