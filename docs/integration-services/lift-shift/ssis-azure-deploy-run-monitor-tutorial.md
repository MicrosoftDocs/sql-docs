...
To deploy a project with PowerShell to SSISDB on Azure SQL Database, adapt the following script to your requirements. This script requires SQL Server Data Tools version 17.x or SQL Server Management Studio installed on the computer where you run the script. The script enumerates the child folders under `$ProjectFilePath` and the projects in each child folder, then creates the same folders in SSISDB and deploys the projects to those folders.
