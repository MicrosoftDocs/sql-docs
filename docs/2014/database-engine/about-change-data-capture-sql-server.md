...
## Understanding Change Data Capture and the Capture Instance  
 Before changes to any individual tables within a database can be tracked, change data capture must be explicitly enabled for the database. This is done by using the stored procedure [sys.sp_cdc_enable_db](../Topic/sys.sp_cdc_enable_db%20\(Transact-SQL\).md). When the database is enabled, source tables can be identified as tracked tables by using the stored procedure [sys.sp_cdc_enable_table](../Topic/sys.sp_cdc_enable_table%20\(Transact-SQL\).md). When a table is enabled for change data capture, an associated capture instance is created to support the dissemination of the change data in the source table. The capture instance consists of a change table and up to two query functions. Metadata that describes the configuration details of the capture instance is retained in the change data capture metadata tables `cdc.change_tables`, `cdc.index_columns`, and `cdc.captured_columns`. This information can be retrieved by using the stored procedure [sys.sp_cdc_help_change_data_capture](../Topic/sys.sp_cdc_help_change_data_capture%20\(Transact-SQL\).md).  
...
## Change Data Capture Validity Interval for a Capture Instance  
 ...
 The function [sys.fn_cdc_get_min_lsn](../Topic/sys.fn_cdc_get_min_lsn%20\(Transact-SQL\).md) is used to retrieve the current minimum LSN for a capture instance, while [sys.fn_cdc_get_max_lsn](../Topic/sys.fn_cdc_get_max_lsn%20\(Transact-SQL\).md) is used to retrieve the current maximum LSN value. When querying for change data, if the specified LSN range does not lie within these two LSN values, the change data capture query functions will fail.  
...
## Change Data Capture Agent Jobs  
 ...
 Internally, change data capture agent jobs are created and dropped by using the stored procedures [sys.sp_cdc_add_job](../Topic/sys.sp_cdc_add_job%20\(Transact-SQL\).md) and [sys.sp_cdc_drop_job](../Topic/sys.sp_cdc_drop_job%20\(Transact-SQL\).md), respectively. These stored procedures are also exposed so that administrators can control the creation and removal of these jobs.  
  
 An administrator has no explicit control over the default configuration of the change data capture agent jobs. The stored procedure [sys.sp_cdc_change_job](../Topic/sys.sp_cdc_change_job%20\(Transact-SQL\).md) is provided to allow the default configuration parameters to be modified. In addition, the stored procedure [sys.sp_cdc_help_jobs](../Topic/sys.sp_cdc_help_jobs%20\(Transact-SQL\).md) allows current configuration parameters to be viewed. Both the capture job and the cleanup job extract configuration parameters from the table msdb.dbo.cdc_jobs on startup. Any changes made to these values by using [sys.sp_cdc_change_job](../Topic/sys.sp_cdc_change_job%20\(Transact-SQL\).md) will not take effect until the job is stopped and restarted.  
  
 Two additional stored procedures are provided to allow the change data capture agent jobs to be started and stopped: [sys.sp_cdc_start_job](../Topic/sys.sp_cdc_start_job%20\(Transact-SQL\).md) and [sys.sp_cdc_stop_job](../Topic/sys.sp_cdc_stop_job%20\(Transact-SQL\).md).  
