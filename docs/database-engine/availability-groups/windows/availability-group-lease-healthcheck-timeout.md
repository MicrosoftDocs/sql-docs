...
The lease is primarily a synchronization mechanism between the primary instance and the cluster, but it can also create failure conditions where there was otherwise no need to fail over. For example, high CPU, out-of-memory conditions, SQL process failing to respond while generating a memory dump, system-wide hang, cluster (WSFC) going offline due to quorum loss, or tempdb pressure can starve the lease worker thread, preventing lease renewal from the SQL instance and causing a failover. 
