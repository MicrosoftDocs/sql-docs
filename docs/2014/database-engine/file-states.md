...
---
# File States
  In [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], the state of a database file is maintained independently from the state of the database. A file is always in one specific state, such as ONLINE or OFFLINE. To view the current state of a file, use the [sys.master_files](../Topic/sys.master_files%20\(Transact-SQL\).md) or [sys.database_files](../Topic/sys.database_files%20\(Transact-SQL\).md) catalog view. If the database is offline, the state of the files can be viewed from the [sys.master_files](../Topic/sys.master_files%20\(Transact-SQL\).md) catalog view.  
