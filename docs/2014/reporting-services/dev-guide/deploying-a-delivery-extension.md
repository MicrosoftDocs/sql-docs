...
|`Visible`|A value of `false` indicates that the delivery extension should not be visible in user interfaces. If the attribute is not included, the default value is `true`.|  
 ...
 For more information about the RSReportServer.config file, see [Reporting Services Configuration Files](../../../2014/reporting-services/reporting-services-configuration-files.md).  
