...
- Deploy your SQL Server VMs to multiple subnets whenever possible to avoid the dependency on an Azure Load Balancer or a distributed network name (DNN) to route traffic to your HADR solution. Review the various ways to deploy [Availability Groups]/azure/azure-sql/virtual-machines/windows/availability-group-overview#deployment-options) on Azure.
