...
description: Reference article for azdata arc sql mi config commands.
author: MikeRayMSFT
ms.author: mikeray
ms.reviewer: seanw
ms.date: 09/22/2020
...
|Command|Description|
| --- | --- |
[azdata arc sql mi config init](#azdata-arc-sql-mi-config-init) | Initializes the CRD and specification files for a SQL managed instance.
[azdata arc sql mi config add](#azdata-arc-sql-mi-config-add) | Add a value for a json path in a config file.
[azdata arc sql mi config remove](#azdata-arc-sql-mi-config-remove) | Remove a value for a json path in a config file.
[azdata arc sql mi config replace](#azdata-arc-sql-mi-config-replace) | Replace a value for a json path in a config file.
[azdata arc sql mi config patch](#azdata-arc-sql-mi-config-patch) | Patches a config file based on a json patch file.
## azdata arc sql mi config init
...
#### `--verbose`
Increase logging verbosity. Use --debug for full debug logs.
## azdata arc sql mi config patch
