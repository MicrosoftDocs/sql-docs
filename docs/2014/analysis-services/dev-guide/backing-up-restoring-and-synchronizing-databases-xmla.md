...
### Specifying the Database and Backup File  
 ...
 To specify the file that is to be created and used by the backup process, you set the [File](../../../2014/analysis-services/dev-guide/file-element-xmla.md) property of the `Backup` command. The `File` property should be set to a UNC path and file name for the backup file to be created.  
  
 Besides specifying which file to use for backup, you can set the following options for the specified backup file:  
  
-   If you set the [AllowOverwrite](../../../2014/analysis-services/dev-guide/allowoverwrite-element-xmla.md) property to true, the `Backup` command overwrites the backup file if the specified file already exists. If you set the `AllowOverwrite` property to false, an error occurs if the specified backup file already exists.  
  
-   If you set the [ApplyCompression](../../../2014/analysis-services/dev-guide/applycompression-element-xmla.md) property to true, the backup file is compressed after the file is created.  
  
-   If you set the [Password](../../../2014/analysis-services/dev-guide/password-element-xmla.md) property to any non-blank value, the backup file is encrypted by using the specified password.  
...
-   For each original folder used to store the remote partitions on the remote data source, you can specify a [Folder](../../../2014/analysis-services/dev-guide/folder-element-xmla.md) element to indicate the new folder in which to restore all the remote partitions stored in the original folder. If a `Folder` element is not specified, the `Restore` command uses the original folders specified for the remote partitions that are contained in the remote backup file.  
