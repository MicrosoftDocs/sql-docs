...
| **create_date** | **datetime** | Date the database was created or renamed. For `tempdb`, this value changes every time the server restarts. create_date does not change when a rename operation is performed on Azure SQL Database. |
...
| **is_query_store_on** | **bit** | 1 = The query store is enable for this database. Check [sys.database_query_store_options](sys-database-query-store-options-transact-sql.md) to view the query store status.<br />0 = The query store isn't enabled<br /><br />**Applies to**: [!INCLUDE [sssql16-md](../../includes/sssql16-md.md)] and later versions. |
...
Due to possible metadata synchronization, the `sys.databases` view may provide incorrect information regarding database encryption. To ensure the accurate results, we recommend to always use the [sys.dm_database_encryption_keys] (https://learn.microsoft.com/en-us/sql/relational-databases/system-compatibility-views/sys-sysdatabases-transact-sql?view=sql-server-ver16) view to obtain the actual encryption status.
