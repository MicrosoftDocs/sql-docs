...
- **Spinlock**
  ...
  Without lock partitioning, one spinlock manages all lock requests for a single lock resource. On systems that experience a large volume of activity, contention can occur as lock requests wait for the spinlock to become available. In this situation, acquiring locks can become a bottleneck and can negatively affect performance.
