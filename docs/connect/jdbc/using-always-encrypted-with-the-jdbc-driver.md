...
**`clientID`** is the Application ID of an App registration in a Microsoft Entra instance. **`clientKey`** is a Key Password registered under that Application, which provides API access to the Azure Key Vault.
