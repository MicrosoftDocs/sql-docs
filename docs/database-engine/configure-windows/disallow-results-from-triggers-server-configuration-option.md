---
title: "Server Configuration: disallow results from triggers"
...
---
# Server configuration: disallow results from triggers

[!INCLUDE [SQL Server](../../includes/applies-to-version/sqlserver.md)]

Use the `disallow results from triggers` option to control whether triggers return result sets. Triggers that return result sets might cause unexpected behavior in applications that aren't designed to work with them.
...
The `disallow results from triggers` option is an advanced option. If you're using the `sp_configure` system stored procedure to change the setting, you can change disallow results from triggers only when `show advanced options` is set to `1`. The setting takes effect immediately without a server restart.
