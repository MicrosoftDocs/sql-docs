...
The following article provides reference for the **bdc config** commands in the **azdata** tool. For more information about other **azdata** commands, see [azdata reference](reference-mssqlctl.md).
...
```bash
azdata bdc config show [--target -t] 
                         [--force -f]
```
### Examples
