...
- [Create a migration project](#create-a-new-migration-project)
...
1. If you can’t provide a shared location that both source and target SQL Servers have access to, select **Copy the database backups to a different location that the target server can read and restore from**. Then, enter a value for the **Location for backups for restore option** box. 
...
DMA performs validations on the backup folders, data and log file locations. If any validation fails, please fix the options and Click **Next**.
...
1. Select specific logins for migration.
   ...
   By default, all the SQL and Windows logins that qualify for migration are selected for migration.
...
1. Review the saved file for details about Logins migration, and then verify the changes.
