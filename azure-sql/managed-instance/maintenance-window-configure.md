...
The ability to change to a different maintenance window is available for every Azure SQL Managed Instance service level or in every region. For details on feature availability, see [Maintenance window availability](maintenance-window.md#feature-availability). There are [subscription type requirements to support maintenance window](maintenance-window.md#supported-subscription-types), however.
