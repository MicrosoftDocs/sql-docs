...
| --- | --- | --- | --- |
| `request_id` | **nvarchar(32)** | `request_id` and `step_index` make up the key for this view.<br /><br />Unique numeric ID associated with the request. | See `request_id` in [sys.dm_pdw_exec_requests (Transact-SQL)](sys-dm-pdw-exec-requests-transact-sql.md). |
...
| `error_id` | **nvarchar(36)** | Unique ID of the error associated with this step, if any. | See `error_id` of [sys.dm_pdw_errors (Transact-SQL)](sys-dm-pdw-errors-transact-sql.md). `NULL` if no error occurred. |
| `start_time` | **datetime** | Time at which the step started execution. | Smaller or equal to current time and larger or equal to `end_compile_time` of the query to which this step belongs. For more information on queries, see [sys.dm_pdw_exec_requests (Transact-SQL)](sys-dm-pdw-exec-requests-transact-sql.md). |
