...
2. Follow steps from [Steps needed to configure c2WTS](?view=sql-server-2017&preserve-view=true#steps-needed-to-configure-c2wts)
...
2. Configure delegation for the C2WTS service account.
    ...
    * Select the service listing. This will show the SPNs that are available on that account. If you don't see the service listed on that account, it may be missing or placed on a different account. you can use the SetSPN utility to adjust SPNs. For the **Report Viewer web part**, you will see the http SPN configured in [Report Viewer web part configuration](?view=sql-server-2017&preserve-view=true#report-viewer-native-mode-web-part-configuration).
