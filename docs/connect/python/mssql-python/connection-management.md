...
- **Set `MultiSubnetFailover=yes` in the connection string** for faster failover recovery on availability group listeners and failover cluster instances. It's safe on single-IP targets, so leave it on for all Microsoft SQL family endpoints.
