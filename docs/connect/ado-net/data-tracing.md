...
The .NET features built-in data tracing functionality that is supported by the Microsoft SqlClient Data Provider for SQL Server and the SQL Server network protocols.
...
> [!NOTE]
> The Microsoft SqlClient Data Provider for SQL Server supports the server process ID since 2.1.0 edition. You can get it programmatically by using the `SqlConnection.ServerProcessId` property.
