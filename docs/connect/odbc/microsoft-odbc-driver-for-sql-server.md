...
The ODBC driver comes with tools such as [**sqlcmd**](/sql-docs/docs/tools/sqlcmd-utility) and [**bcp**](/sql-docs/docs/tools/bcp-utility). The **sqlcmd** utility lets you run Transact-SQL statements, system procedures, and SQL scripts. The **bcp** utility bulk copies data between an instance of Microsoft SQL Server and a data file in a format you choose. You can use **bcp** to import many new rows into SQL Server tables or to export data out of tables into data files.  
