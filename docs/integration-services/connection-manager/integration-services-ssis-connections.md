...
- Connecting to relational databases to run SQL statements such as SELECT, DELETE, and INSERT commands and stored procedures.  
...
- Specify existing or create new files and folders to use with Foreach Loop enumerators and tasks.  
  
- Connecting to message queues and Windows Management Instrumentation (WMI), [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Management Objects (SMO), Web, and mail servers.  
...
## Connection Managers  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] provides different types of connection managers that enable packages to connect to various data sources and servers:  
...
- You can create your own custom connection manager if the existing connection managers don't meet your needs.  

### Package level and project level connection managers

A connection manager can be created at the package or project levels. The connection manager created at the project level is available for all the project packages. A connection manager created at the package level is available for that specific package.  
  
 You use connection managers created at the project level in place of data sources to share connections to sources. To add a connection manager at the project level, the [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] project must use the project deployment model. When a project is configured to use this model, the **Connection Managers** folder appears in **Solution Explorer**, and the **Data Sources** folder is removed from **Solution Explorer**.  
  
> [!NOTE]  
> To use data sources in your package, you need to convert the project to the package deployment model.  
>
...
3. In [!INCLUDE[ssIS](../../includes/ssis-md.md)] Designer, select the **Control Flow** tab, the **Data Flow** tab, or the **Event Handler** tab to make the **Connection Managers** area available.  
  
4. Right-click anywhere in the **Connection Managers** area, and then do one of the following:  
  
    - Select the connection manager type to add to the package.  
  
         -or-  
  
    - If the type that you want to add isn't listed, select **New Connection** to open the **Add SSIS Connection Manager** dialog box, select a connection manager type, and then select **OK**.  
...
5. Optionally, right-click the connection manager, select **Rename**, and then modify the default name of the connection manager.  
  
6. To save the updated package, select **Save Selected Item** on the **File** menu.  
...
2. In **Solution Explorer**, right-click **Connection Managers**, and select **New Connection Manager**.  
  
3. In the **Add SSIS Connection Manager** dialog box, select the type of connection manager, and then select **Add**.  
    ...
    |[WMI Connection Manager](../../integration-services/connection-manager/wmi-connection-manager.md)|[WMI Connection Manager Editor](./wmi-connection-manager.md)|  
     ...
     The connection manager you added will show up under the **Connections Managers** node in the **Solution Explorer**. It will also appear in the **Connection Managers** tab in the **SSIS Designer** window for all the packages in the project. The name of the connection manager in this tab has a **(project)** prefix in order to differentiate this project level connection manager from the package level connection managers.  
  
4. Optionally, right-click the connection manager in the **Solution Explorer** window under **Connection Managers** node (or) in the **Connection Managers** tab of the **SSIS Designer** window, select **Rename**, and then modify the default name of the connection manager.  
 ...
 **Connection manager type**  
 Select a connection type and then select **Add**, or double-click a connection type to specify connection properties using the editor for each type of connection.  
...
1. In the **Connection Managers** area, right-click the connection manager that you want to create a parameter for and then select **Parameterize**.  
...
4. Right-click the connection manager that you want to delete, and then select **Delete**.  
  
     If you delete a connection manager that a package element, such as an Execute SQL task or an OLE DB source, uses, you'll experience the following results:  
  
    - An error icon appears on the package element that used the deleted connection manager.  
  
    - The package fails to validate.  
  
    - The package can't be run.  
...
1. To delete a project-level connection manager, right-click the connection manager under **Connection Managers** node in the **Solution Explorer** window, and then select **Delete**. [!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)] displays the following warning message:  
...
2. Select OK to delete the connection manager or Cancel to keep it.  
...
3. In SSIS Designer, select the **Control Flow** tab, the **Data Flow** tab, or the **Event Handler** tab to make the **Connection Managers** area available.  
  
4. Right-click the connection manager and select **Properties**.  
  
5. In the **Properties** window, edit the property values. The **Properties** window provides access to some properties that aren't configurable in the standard editor for a connection manager.  
  
6. Select **OK**.  
  
7. To save the updated package, select **Save Selected Items** on the **File** menu.  
