...
| `session_id` | **nvarchar(32)** | Unique numeric ID associated with the session in which this query was run. See [sys.dm_pdw_exec_sessions (Transact-SQL)](sys-dm-pdw-exec-sessions-transact-sql.md). | |
...
| `error_id` | **nvarchar(36)** | Unique ID of the error associated with the request, if any. | See [sys.dm_pdw_errors (Transact-SQL)](sys-dm-pdw-errors-transact-sql.md); set to `NULL` if no error occurred. |
| `database_id` | **int** | Identifier of database used by explicit context (for example, `USE DB_X`). | See ID in [sys.databases (Transact-SQL)](../system-catalog-views/sys-databases-transact-sql.md). |
...
| `client_correlation_id` | **nvarchar(255)** | Optional user-defined name for a client session. To set for a session, call `sp_set_session_context 'client_correlation_id', <CorrelationIDName>`. You can retrieve its value with `SELECT SESSION_CONTEXT (N'client_correlation_id')`. For more information, see [sp_set_session_context (Transact-SQL)](../system-stored-procedures/sp-set-session-context-transact-sql.md). |
