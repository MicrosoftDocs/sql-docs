...
Always Encrypted with secure enclaves expands confidential computing capabilities of [Always Encrypted](always-encrypted-database-engine.md) by enabling in-place encryption and richer confidential queries. Always Encrypted with secure enclaves is available in [!INCLUDE[sql-server-2019](../../../includes/sssql19-md.md)] and later, as well as in [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)].

Introduced in [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] in 2015 and in [!INCLUDE[sssql16](../../../includes/sssql16-md.md)], Always Encrypted protects the confidentiality of sensitive data from malware and high-privileged *unauthorized* users: Database Administrators (DBAs), computer admins, cloud admins, or anyone else who has legitimate access to server instances, hardware, etc., but shouldn't have access to some or all of the actual data.
...
The type of the enclave available for your database depends on the SQL product hosting it ([!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] vs. [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]) and (in the case of [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)]) the configuration of your database.
...
- In [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)], a database can use either an Intel SGX enclave or a VBS enclave, depending on the hardware the database is configured to run on:
  ...
  > [!IMPORTANT]
  > VBS enclaves in [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] are currently in preview. The [Supplemental Terms of Use for Microsoft Azure Previews](https://azure.microsoft.com/support/legal/preview-supplemental-terms/) include additional legal terms that apply to Azure features that are in beta, in preview, or otherwise not yet released into general availability.

  > [!NOTE]
  > VBS enclaves are currently available in all [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] regions **except**: Jio India Central.
...
| [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] | SGX enclaves (DC-series databases) | Microsoft Azure Attestation |
| [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] | VBS enclaves  | No attestation |
...
- With Intel SGX enclaves (in DC-series databases) in [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)], attestation is mandatory and it requires Microsoft Azure Attestation.
- VBS enclaves in [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] (in preview) currently don't support attestation.
...
| Operation| [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] | [!INCLUDE[sql-server-2022](../../../includes/sssql22-md.md)] | [!INCLUDE[sql-server-2019](../../../includes/sssql19-md.md)] | 
|:---|:---|:---| :---|
...
> [!IMPORTANT]
> Microsoft strongly recommends enabling [Accelerated database recovery (ADR)](../../backup-restore/restore-and-recovery-overview-sql-server.md#adr) for your database, **before** creating the first index on an enclave-enabled column encrypted with randomized encryption. ADR is enabled by default in [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)], but not in [!INCLUDE[sql-server-2019](../../../includes/sssql19-md.md)] and later.
...
- In [!INCLUDE[sql-server-2019](../../../includes/sssql19-md.md)] (this limitation doesn't apply to [!INCLUDE[ssSDSfull](../../../includes/ssazure-sqldb.md)] or [!INCLUDE [sssql22-md](../../../includes/sssql22-md.md)]) only nested loop joins (using indexes, if available) are supported on enclave-enabled columns using randomized encryption. For information about other differences among different products, see [Confidential queries](#confidential-queries).
