...
Learn how to install SQL Server Reporting Services (SSRS) in native mode. This installation provides access to a [!INCLUDE[ssRSWebPortal](../../includes/ssrswebportal.md)] that you can use to manage reports and other items.
...
Setup installs the following SSRS features when you select the default configuration for native mode option:
...
- The SSRS Configuration Manager
...
- Connection information, also known as the data source name (DSN), for the report server databases.
 ...
 Setup does not configure the unattended execution account, report server e-mail, back up the encryption keys, or a scale-out deployment. You can use the SSRS Configuration Manager to configure these properties. For more information, see [Report Server Configuration Manager &#40;Native Mode&#41;](../../reporting-services/install-windows/reporting-services-configuration-manager-native-mode.md).

## <a name="bkmk_whentoinstalldefaultconfig"></a> When to install the default configuration for native mode

A default configuration installs SSRS in an operational state so that you can use the report server immediately after Setup is finished. Specify this mode when you want to save steps by eliminating any required configuration tasks you would otherwise have to perform in the SSRS Configuration tool.
...
- SSRS and [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] must be installed together in the same instance. The [!INCLUDE[ssDE](../../includes/ssde-md.md)] instance hosts the report server database that Setup creates and configures.
...
If your computer does not meet all requirements for a default installation, you must install SSRS in files-only mode and then use the SSRS Configuration Manager to configure it after Setup is finished.
...
|Port|The default port is 80. Note that if you use any port other than port 80, you will have to explicitly add it to the URL when you open a SSRS Web application in a browser window.|
...
The following list describes the SSRS specific steps and options you select in the SQL Server Installation Wizard. The list does not described each page you will see in the installation wizard, only the SSRS related pages that are part of a Native mode installation.
...
6. **Server Configuration**: If you plan to use the SSRS subscription feature, then on the **Server Configuration** page, configure SQL Server Agent **Automatic** Startup type. The default is manual.
...
Specific to SSRS, the rules verify a report server catalog and temp catalog database do not already exist.
...
11. After the SQL Server installation wizard is complete, verify the default Native mode installation using the following basic steps.
    ...
    - Open SSRS Configuration Manager and confirm you can connect to the report server.
    ...
    - Open your browser with administrative privileges and connect to the SSRS report server page. For example, `https://localhost/ReportServer`
