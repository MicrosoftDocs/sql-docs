...
The count that `RowsAffected()` reports depends on what produced the row counts:

- **Stored procedures:** When every statement runs inside a procedure that uses [`SET NOCOUNT ON`](../../t-sql/statements/set-nocount-transact-sql.md), `RowsAffected()` returns 0 because no row count is sent. A 0 is therefore ambiguous: either no rows changed, or the counts were suppressed. The driver gives you no way to tell those apart. To get a count, remove `SET NOCOUNT ON`, or return the count through an `OUTPUT` parameter or `SELECT` statement.
...
- **`INSTEAD OF` triggers:** The count includes the trigger's statements and the original statement, even though the original statement doesn't run. A single-row `UPDATE` whose trigger writes two audit rows and performs the update returns 4. If the trigger doesn't perform the update, the call still returns 3 and no rows change, so a nonzero count doesn't confirm that data changed. Query the affected table when you need to verify the result. Adding `SET NOCOUNT ON` to the trigger body returns 1 in both cases.
