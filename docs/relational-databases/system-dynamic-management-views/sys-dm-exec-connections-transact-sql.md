...
## Permissions  
**[!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)]**: Requires `VIEW SERVER STATE` permission on the server.   
**[!INCLUDE[ssSDS_md](../../includes/sssds-md.md)]**: Requires `VIEW DATABASE STATE` permission in the user database. `dm_exec_connections` cannot be queried in the `master` database.   
