...
In SQL Server 2016 and SQL Server 2017, a computer administrator can install packages for each instance where machine learning has been enabled. Packages are installed in the file system in instance libraries and cannot be shared across instances. This is currently the recommended method for both SQL Server 2016 and SQL Server 2017.

Installation of packages requires administrative access to the server.
...
+ [New R functions for package installation](#a name="bkmk_remoteInstall)

### Installation on servers with no Internet access

To make it easier to prepare the required R package versions and package dependencies, you can use [miniCRAN](https://mran.microsoft.com/package/miniCRAN). This R package takes a list of target packages and creates a local repository that contains the target packages along with all their dependencies, in zipped format. You can then copy that to the offline server, or share the repository among multiple instances.
...
Installation of new Python packages follows the same guidelines. For now, we recommend that a computer administrator install new packages in the instance library, for all users of the instance.

## Changes in package management between SQL Server 2016 and SQL Server 2017

SQL Server 2017 added some new features to support easier management of R (and Python) packages by database administrators in future releases. These new features include the ability to install package libraries using T-SQL, and to manage user permissions at the database level through database roles. 
...
For details about each of these features, see these links:
