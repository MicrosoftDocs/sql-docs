...
### D: Listing permissions on schema objects within a database  
 The following query joins `sys.database_principals` and `sys.database_permissions` to `sys.schemas` to list permissions granted or denied to specific schema objects.  
