...
For step-by-step instructions, including the `ALTER CREDENTIAL` example and availability group considerations, see [Renew an expiring or expired client secret](set-up-transparent-data-encryption-with-azure-key-vault-for-sql-server.md#renew-an-expiring-or-expired-client-secret).
