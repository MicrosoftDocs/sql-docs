...
In addition, consider adjusting the **max degree of parallelism (MAXDOP)** server configuration option for these operations. For more information and guidelines about adjusting the max degree of parallelism option, see [Configure the max degree of parallelism Server Configuration Option](../database-engine/configure-windows/configure-the-max-degree-of-parallelism-server-configuration-option.md).
