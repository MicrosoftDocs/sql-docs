...
The primary system view used to monitor queries is [sys.dm_pdw_exec_requests](https://msdn.microsoft.com/library/mt203887.aspx). Use this system view to find the `request_id` for an active or recent query, based on the query text.  
...
After the `request_id` has been identified for a query, use the other information in the `dm_pdw_exec_requests` table to find out about the processing of the query, or use [sys.dm_pdw_request_steps](https://msdn.microsoft.com/library/mt203913.aspx) to view the status of the individual query steps for the query execution.  
