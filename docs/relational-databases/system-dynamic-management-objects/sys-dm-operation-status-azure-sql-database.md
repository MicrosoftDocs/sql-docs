...
| `state` | **tinyint** | The state of the operation.<br /><br />0 = Pending<br />1 = In progress<br />2 = Completed<br />3 = Failed<br />4 = Cancel in progress<br />5 = Canceled |
| `state_desc` | **nvarchar(120)** | `PENDING` = operation is waiting for resource or quota availability.<br /><br />`IN_PROGRESS` = operation has started and is in progress.<br /><br />`COMPLETED` = operation completed successfully.<br /><br />`FAILED` = operation failed. See the `error_desc` column for details.<br /><br />`CANCEL_IN_PROGRESS` = operation is in the process of being canceled.<br /><br />`CANCELLED` = operation stopped at the request of the user. |
