...
Discussions in this section only apply to Linux and macOS users, in particular those who deal with more than one locale in their php applications.
...
Starting with version 5.8.0, the users may configure the localization settings by using the php.ini file, as shown in the examples below.
...
|1|The driver reads LC_CTYPE variable.|
|2|The driver reads LC_ALL variable (this is the default if nothing is defined in php.ini).|
  

The `LC_CTYPE` category determines character handling rules, which govern the interpretation of sequences of bytes of text data characters, the classification of characters, and the behavior of character classes. It controls recognition of upper and lower case, alphabetic or non-alphabetic characters, and so on.
