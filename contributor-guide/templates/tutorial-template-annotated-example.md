...
GUIDELINES for H1
	The H1 is the top heading in the article. It tells users what they will do. Use strong keywords like deploy, ASP.NET, web app, App Service, Visual Studio, etc. (examples of this below)
...
<!------------------
GUIDELINES for Time estimate with or without diagrams
	Give a time estimate with any useful details. Diagrams are optional but help users see the big picture of what they will do. An optional video of the steps can be included for users' use as an alternative to following the steps in the topic. The video doesn't have to be professional.
...
GUIDELINES for Prerequisites
	Include getting started info like how to set up an account. List the technical expertise and software that will be required. You can also add links to conceptual information for the user.
-------------------->
## Prerequisites
...
GUIDELINES for Procedures (steps)
	Format headings as H2s and make them short and clear. H2s appear in the right pane on the web page and are important for navigation. H2s usually begin with a verb (configure, download, etc.) to let users know they will be doing something. Keep number of steps under 10.
...
3. Enter a **Web App Name** that is unique in the *azurewebsites.net* domain. For example, you can name it MyExample with numbers to the right to make it unique, such as MyExample810. If a default web name is created for you, it will be unique, and you can use that.
	...
	The URL for your application is the name you entered plus *.azurewebsites.net*. For example, if the name is `MyExample810`, the URL is `myexample810.azurewebsites.net`.
