...
This article documents the tasks that must be done on both [!INCLUDE[ssnoversion-md](../includes/ssnoversion-md.md)] and Pacemaker to deploy a Linux Always On availability group (AG) or failover cluster instance (FCI). Unlike an AG or FCI on Windows, there is no set order, and the Pacemaker cluster creation as well as the configuration of the AG on Linux can be done before or after the installation of [!INCLUDE[ssnoversion-md](../includes/ssnoversion-md.md)]. This is different than on the tightly coupled Windows Server/[!INCLUDE[ssnoversion-md](../includes/ssnoversion-md.md)] stack. The integration and configuration of resources for the Pacemaker portion of an AG or FCI deployment are done after the cluster is configured.
> [!IMPORTANT]
