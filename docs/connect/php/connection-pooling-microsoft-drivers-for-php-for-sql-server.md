...
> [!NOTE]  
> [Multiple Active Result Sets (MARS)](../../connect/php/how-to-disable-multiple-active-resultsets-mars.md) is enabled by default as well. When both MARS and pooling are in use, in order for MARS to work correctly, the driver requires a longer time to reset the connection on the *first* query, thus ignoring any query timeout specified. However, query timeout setting will take effect in the subsequent queries.
