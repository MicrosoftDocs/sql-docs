...
Decryption occurs via the client. This means that some actions will not work when using Always Encrypted. For example, if you execute an update that attempts to move data from an encrypted column to an unencyprted column without returning a result set to the client, this action will fail. 
...
This error occurs because the column is being updated server side, without any interaction with the client, and interaction with the client is necessary for decryption. 
...
2. INSERT/UPDATE the data from the result set in the application to SQL Server one row at a time. Paramterization will pass the data into SQL Server ia parameters so the values can be encrypted. 
