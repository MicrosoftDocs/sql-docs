...
> [!NOTE]
> For both the `Fill` operations of the `DataAdapter` and the `Get` methods of the `DataReader`, the .NET Framework type is inferred from the type returned from the Microsoft SqlClient Data Provider for SQL Server. The inferred .NET Framework types and accessor methods for Microsoft SQL Server data types are described in [Data Type Mappings in ADO.NET](data-type-mappings-ado-net.md).
