...
|`VALUE`|The unformatted value of the cell.|  
 ...
 Only the `CELL_ORDINAL`, `FORMATTED_VALUE`, and `VALUE` cell properties are required. All cell properties, intrinsic or provider-specific, are defined in the `PROPERTIES` schema rowset, including their data types and provider support. For more information about the `PROPERTIES` schema rowset, see [MDSCHEMA_PROPERTIES Rowset](../../dev-guide/mdschema-properties-rowset.md).  
