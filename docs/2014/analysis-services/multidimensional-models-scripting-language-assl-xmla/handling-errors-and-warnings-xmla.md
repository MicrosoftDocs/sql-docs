...
|Errors or warnings on a successful method call|[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] includes an [error](../xmla/xml-elements-properties/error-element-xmla.md) or [warning](../xmla/xml-elements-properties/warning-element-xmla.md) element for each error or warning, respectively, in the [Messages](../xmla/xml-elements-properties/messages-element-xmla.md) property of the [root](../xmla/xml-elements-properties/root-element-xmla.md) element that contains the results of the method call.<br /><br /> For more information, see the section, [Handling Errors and Warnings](#handling_errors_and_warnings).|  
|Errors in the result for a successful method call|[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] includes an inline `error` or `warning` element for the error or warning, respectively, within the appropriate [Cell](../xmla/xml-elements-properties/cell-element-xmla.md) or [row](../xmla/xml-elements-properties/row-element-xmla.md) element of the results of the method call.<br /><br /> For more information, see the section, [Handling Inline Errors and Warnings](#handling_inline_errors_and_warnings).|  
...
##  <a name="handling_inline_errors_and_warnings"></a> Handling Inline Errors and Warnings  
 ...
 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] supplies inline `error` and `warning` elements if issues specific to a cell or to other data that are contained within a `root` element using the [MDDataSet](../xmla/xml-data-types/mddataset-data-type-xmla.md) data type occur, such as a security error or formatting error for a cell. In these cases, [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] returns an `error` or `warning` element in the `Cell` or `row` element that contains the error or warning, respectively.  
  
 The following example illustrates a result set that contains an error in the rowset returned from an `Execute` method using the [Statement](../xmla/xml-elements-commands/statement-element-xmla.md) command.  
