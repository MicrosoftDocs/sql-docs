...
|SharePoint web application|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] for SharePoint 2010 only supports SharePoint web applications that are configured for classic-mode authentication. If you are adding [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] for SharePoint to an existing farm, be sure that the web application you plan to use it with is configured for classic-mode authentication. For instructions on how to check authentication mode, see the section “Verify the Web application uses Classic mode authentication” in [Deploy PowerPivot Solutions to SharePoint](../../relational-databases/spatial/point.md).|  
