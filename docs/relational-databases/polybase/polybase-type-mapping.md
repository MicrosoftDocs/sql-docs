...
**Timestamp** and **Timestamp with local timezone** in Oracle supports 9 fractional seconds precision. SQL server DateTime2 supports 7 fractional seconds precision.

Also, Simba maps **Date**, **Timestamp**, and **Timestamp with local zone** to ODBC type **SQL_TIMESTAMP** and eventually SQL Server type **Timestamp**. **Timestamp** is an automatically generated unique row id. Ideally, **SQL_TIMESTAMP** should map to SQL server type **DateTime/DateTime2** or Oracle types **Date**, **Timestamp**, and **Timestamp with local zone** should map to ODBC type **SQL_TYPE_TIMESTAMP**.

### Driver Configuration option

The default buffer size for **Long/LOB** columns is 1024 KB (configurable).
...
The **Period** data type represents a duration marked by a beginning bound and ending bound. Essentially, it is a tuple. There is no SQL Server equivalent type for **Period**.

### Time with Time Zone and Timestamp

**Time with Time Zone** and **Timestamp** contains the timezone offset, which is lost during translation. This can be fixed if mapped to **SQL_Type_Time/SQL_Type_Timestamp** to **datetimeoffset** instead of **Time/DateTime2**.

### ByteInt

Simba maps Teradata type **ByteInt** that can hold values between 0-255 into **TinyInt** that can hold values between -127 to 127. This could result in truncation.  

### CLOB

The **CLOB** data type with **LATIN** charset should be able to accept 2097088000 characters. However, beyond 1073741823, the buffer size becomes negative.  
...
- Return Max. **CHAR/VARCHAR** length as 32 K.
