...
- If the key vault is behind a VNet that uses a firewall, the option to **Allow Trusted Microsoft Services to bypass this firewall** must be enabled in the key vault's **Networking** menu if you want to use a user-assigned managed identity. Once this option is enabled, available keys cannot be listed in the SQL server or managed instance TDE menu in the Azure portal. To set an individual CMK, a *key identifier* must be used . When option to **Allow Trusted Microsoft Services to bypass this firewall** is not enabled, the following error is returned:
