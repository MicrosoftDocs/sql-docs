...
description: sys.system_parameters contains one row for each system object that has parameters.
author: rwestMSFT
...
| `max_length` | **smallint** | Maximum length of the parameter, in bytes. Value is -1 for when column data type is **varchar(max)**, **nvarchar(max)**, **varbinary(max)**, or **xml**. |
...
| `xml_collection_id` | **int** | Non-zero if the data type of the parameter is **xml** and the XML is typed. The value is the ID of the collection that contains the validating XML schema namespace for the parameter.<br /><br />`0` = There's no XML schema collection. |
