...
- For stand-alone servers, wildcard certificates can be set via the SQL Server Configuration Manager UI. However, in case of an Always On cluster, wildcard certificates cannot be selected via the UI. To use a wildcard certificatein an always on cluster, you must edit the `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL12.MSSQLSERVER\MSSQLServer\SuperSocketNetLib` registry key, and enter the thumbprint of the certificate, without spaces, to the **Certificate** value.
