...
This article describes how to update the Active Directory groups provided during deployment for clusterAdmins and clusterUsers.
...
* `clusterUsers`: The Active Directory groups that are regular users without administrator permissions in the big data cluster. These users have permissions to log in to the SQL Server master instance, but they have no permissions to objects or data by default.

One way to grant additional Active Directory groups permissions to the big data cluster after deployment is to add any additional users and groups to the already-nominated groups upon deployment. 
...
1. Connect to the master SQL endpoint by using your favorite SQL client. Use any administrator login (for example, `AZDATA_USERNAME`, which was provided during the deployment). Alternatively, it could be any Active Directory account that belongs to the Active Directory group provided as `clusterAdmins` in the security configuration.
...
1. To create a login for the Active Directory user or group, run the following TSQL command:
   ...
   If you're granting administrator privileges in the SQL Server instance, also grant the following permission:
...
1. Use the preceding connection to insert a row in the roles table. Type the realm value in uppercase.

   If you're granting administrator privileges, use the `bdcAdmin` role in the `<role name>` below. For non-administrator users, use the `bdcUser` role.
