...
An underlying Pacemaker cluster needs access to the SQL Server instance as well as permissions on the availability group (AG) itself. These steps create the login and the associated permissions, as well as a file that tells Pacemaker how to log into SQL Server.
