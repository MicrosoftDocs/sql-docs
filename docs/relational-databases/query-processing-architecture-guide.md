...
For more information on troubleshooting bad parameter sniffing issues, see [Troubleshoot queries with parameter-sensitive query execution plan issues](azure/sql-database/sql-database-monitor-tune-overview.md#troubleshoot-queries-with-parameter-sensitive-query-execution-plan-issues).
