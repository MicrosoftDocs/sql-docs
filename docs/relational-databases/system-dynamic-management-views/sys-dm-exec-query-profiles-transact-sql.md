...
On [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] Premium Tiers, requires the `VIEW DATABASE STATE` permission in the database. On [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] On SQL Database Basic, S0, S1 service objectives and for databases in elastic pools, requires the `Server admin` or an `Azure Active Directory admin` account. On all other SQL Database service objectives, requires the VIEW DATABASE STATE permission in the database..   
