...
|-----------------|-------------------|-----------------|  
|**ApplicationName** <sup>1</sup>|10|The name of the client application that created the connection to an instance of [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]. This column is populated with the values passed by the application and not the name of the program.|  
...
|**ClientProcessID** <sup>1</sup>|9|The ID assigned by the host computer to the process where the client application is running. This data column is populated if the client process ID is provided by the client.|  
...
|**HostName** <sup>1</sup>|8|The name of the computer on which the client is running. This data column is populated if the host name is provided by the client. To determine the host name, use the HOST_NAME function.|  
...
|**LoginSid** <sup>1</sup>|41|The security identifier (SID) of the logged-in user. You can find this information in the **sys.server_principals** view of the **master** database. Each login to the server has a unique ID.|  
...
|**NTDomainName** <sup>1</sup>|7|The Microsoft Windows domain to which the user belongs.|  
|**NTUserName** <sup>1</sup>|6|The Windows user name.|  
...
|**ObjectType** <sup>2</sup>|28|The value representing the type of the object involved in the event. This value corresponds to the **type** column in **sysobjects**.|  
...
|----------------------|-----------|  
|Describes how to create and run traces using Transact-SQL stored procedures.|[Create and Run Traces Using Transact-SQL Stored Procedures](../database-engine/create-and-run-traces-using-transact-sql-stored-procedures.md)|  
