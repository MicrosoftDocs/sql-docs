...
This article teaches you how to migrate your SQL Server Always On availability group to SQL Server on Azure virtual machines (VMs) by using the [Azure Migrate: Server Migration tool](/azure/migrate/migrate-services-overview#azure-migrate-server-migration-tool). Using the migration tool, you can migrate each replica in the availability group to an Azure virtual machine hosting SQL Server, along with the cluster metadata, availability group metadata, and other necessary high availability components.
...
This guide uses the agent-based migration approach of Azure Migrate, which treats any server or virtual machine as a physical server. When it's migrating physical machines, Azure Migrate: Server Migration uses the same replication architecture as the agent-based disaster recovery in the Azure Site Recovery service, and some components share the same code base. Some content might link to Site Recovery documentation.
...
To migrate to Azure, ensure that your source machines comply with requirements. Follow these steps:
...
1. Some [Windows](/azure/migrate/prepare-for-migration#windows-machines) sources require a few more changes. Migrating the source before making these changes could prevent the VM from booting in Azure. For some operating systems, Azure Migrate makes these changes automatically.
...
> [!NOTE]  
> The replication appliance should be installed on a machine other than the source machine you're replicating or migrating. It shouldn't be installed on any machine that the Azure Migrate discovery and assessment appliance was installed on before.
...
1. Select **Create resources**. This operation creates an Azure Site Recovery vault in the background.
   - If migration is already set up  with Azure Migrate: Server Migration, the target option can't be configured, since resources were set up previously.
...
1. In **Download and install the replication appliance software**, download the appliance installer, and the registration key. You need to download the key in order to register the appliance. The key is valid for five days after you download it.
...
1. After setup completes, and the appliance restarts, in **Discover machines**, select the new appliance in **Select Configuration Server**, and select **Finalize registration**. Finalize registration prepares the replication appliance by performing a couple of final tasks.
...
1. Connect to the machine and extract the contents of the installer file to a local folder (such as c:\temp). Run the following commands in an admin command prompt:
...
To prepare source machines, run the `Get-ClusterInfo.ps1` script on a cluster node to retrieve information on the cluster resources. The script outputs the role name, resource name, IP, and probe port in the `Cluster-Config.csv` file.
...
For the cluster and cluster roles to respond properly to requests, an Azure Load balancer is required. Without a load balancer, the other VMs are unable to reach the cluster IP address because it isn't recognized as belonging to the network or the cluster.
...
| --- | --- | --- |
| `ConfigFilePath` | Mandatory | Specify the path for the `Cluster-Config.csv` file that you filled out in the previous step. |
| `ResourceGroupName` | Mandatory | Specify the name of the resource group in which the load balancer is to be created. |
| `VNetName` | Mandatory | Specify the name of the Azure virtual network that the load balancer is to be associated to. |
| `SubnetName` | Mandatory | Specify the name of the subnet in the Azure virtual network that the load balancer is to be associated to. |
| `VNetResourceGroupName` | Mandatory | Specify the name of the resource group for the Azure virtual network that the load balancer is to be associated to. |
...
After machines are replicated, they're ready for migration. To migrate your servers, follow these steps:
...
1. In **Replicating machines** > select server name > **Overview**. Before moving on to the next step, ensure that the last synchronized timestamp is after you stopped the SQL Server service on the servers to be migrated. This process should only take a few minutes.
...
1. In **Migrate** > **Shut down virtual machines and perform a planned migration with no data loss**, select **No** > **OK**.
   ...
   > [!NOTE]  
   > For physical server migration, shut down of the source machine isn't supported automatically. The recommendation is to bring the application down as part of the migration window (don't let the applications accept any connections). Then, initiate the migration (the server needs to be kept running, so remaining changes can be synchronized) before the migration is completed.
...
After your VMs are migrated, follow these steps to reconfigure the cluster:
...
1. Copy the `Cluster-Config.csv` file and run the `Update-ClusterConfig.ps1` script passing the CSV as a parameter. This script ensures the cluster resources are updated with the new configuration for the cluster to work in Azure.
...
1. After the migration is done, right-click the VM > **Stop migration**. This command:
