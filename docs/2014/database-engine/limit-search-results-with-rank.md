...
# Limit Search Results with RANK
  The [CONTAINSTABLE](../Topic/CONTAINSTABLE%20\(Transact-SQL\).md) and [FREETEXTTABLE](../Topic/FREETEXTTABLE%20\(Transact-SQL\).md) functions return a column named RANK that contains ordinal values from 0 through 1000 (rank values). These values are used to rank the rows returned according to how well they match the selection criteria. The rank values indicate only a relative order of relevance of the rows in the result set, with a lower value indicating lower relevance. The actual values are unimportant and typically differ each time the query is run.  
...
### Ranking of CONTAINSTABLE  
 [CONTAINSTABLE](../Topic/CONTAINSTABLE%20\(Transact-SQL\).md) ranking uses the following algorithm:  
