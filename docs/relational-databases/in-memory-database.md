...
Hybrid Buffer Pool [hybrid buffer pool](../database-engine/configure-windows/hybrid-buffer-pool.md) allows the database engine to directly access data pages in database files stored on Persistent Memory (PMEM) devices.
...
[!INCLUDE[sqlv15](../includes/sssqlv15-md.md)] adds support for Persistent Memory (PMEM) devices to Linux, providing full enlightenment of data and transaction log files placed on PMEM [pmem for sql server](../linux/sql-server-linux-configure-pmem.md).
