...
> [!NOTE]
> TDE cannot be used to encrypt system databases, such as the `master` database in Azure SQL Database and Azure SQL Managed Instance. The `master` database contains objects that are needed to perform TDE operations on user databases. It is recommended not to store any sensitive data in system databases.  The exception is `tempdb`, which is always encrypted with TDE to protect data stored there. 
...
- For more information about Key Vault, see [Secure access to a key vault](/azure/key-vault/general/security-features).
