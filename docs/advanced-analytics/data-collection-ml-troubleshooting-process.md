...
This article describes the kind of data, including logs, error messages, and system information, that you should collect when attempting to resolve problems with the setup, configuration, or performance of machine learning in SQL Server.

The goal is to describe the sources of information that will be most useful to users of machine learning services in SQL Server who intend to perform diagnostics on a self-help basis. Collection of this information will also be useful when you request technical support for an issue.

**Applies to:** SQL Server 2016 R Services, SQL Server 2017 Machine Learning Services (R and Python) 

## SQL Server and R Version

Note whether the installation was a new install or an upgrade. If a upgrade, determine how the upgrade was performed:
...
As a last resort, you can open files on the server to determine the version that was installed. To do this, locate the file, rlauncher.config, to get the location of the R runtime and the current working directory. We recommend that you make a copy and open the copy so that yo don;t accidentally change anything.
...
The R console will display the version information on startup. For example, the following version represents the default configuration for SQL server 2017 CTP 2.0. 
...
> [!NOTE] 
> The exact folder name will differ based on the instance name.
...
SQL Server generates separate logs for the external script language runtimes. Note that these are not the errors generated by the Python or R language but rather errors from the extensibility components in SQL Server, including language-specific launchers and their satellite processes.
...
1. Determine whether the instance supports Mixed Mode authentication, SQL logins only, or Windows authentication only. This will affect your R or Python code requirements. 
...
3. Create roles or add users to the following roles as necessary to enable script execution:
  ...
  - All but database owner require EXECUTE ANY EXTERNAL SCRIPT
...
5. If the user needs to install new R packages, or use R packages installed by other users, you might need to enable package maagement on the instance, and assign additional permissions. For more information, see LINK.
...
1. Determine whether SQL Server supports remote connections: [configur eremote server cnnections](
https://docs.microsoft.comsql/database-engine/configure-windows/view-or-configure-remote-server-connection-options-sql-server).
...
For more information, see [Troubleshooting Connecting to SQL Server](https://docs.microsoft.com/sql/database-engine/configure-windows/troubleshoot-connecting-to-the-sql-server-database-engine)
...
[R Libraries and Data Types](r/r-libraries-and-data-types)
...
+ [Generating a Stored Procedure for R Code using the sqlrutils Package](r/generating-an-r-stored-procedure-for-r-code-using-the-sqlrutils-package)

+ [How to Create a Stored Procedure using sqlrutils](r/how-to-create-a-stored-procedure-using-sqlrutils)
