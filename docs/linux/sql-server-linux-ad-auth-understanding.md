...
This article provides you details on how AD (Active Directory) authentication works for [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] deployed on Linux or containers.
...
When you work in a heterogeneous (mixed) environment where you have Windows and non-Windows servers and clients, there are two kinds of files you need to work with AD-based directory services:
...
Server processes on Linux or Unix systems can't be configured to run processes with a Windows service account. When you want a Linux or Unix system to automatically log into Active Directory (AD) on startup, you must use a *keytab* file.
...
This file is necessary for AD Authentication to work. `krb5.conf` is an INI file, but each value in the key-value pair can be a subgroup enclosed by `{` and `}`.
...
## Configuring Kerberos for [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] on Linux
...
- `realms` (optional) - For each realm, the `kdc` value may be set to specify which Key Distribution Centers the machine should contact when looking up AD accounts. If you have set more than one KDC, the KDC for each connection will be selected by round-robin.
