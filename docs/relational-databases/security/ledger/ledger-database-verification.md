...
> [!NOTE]
> The database option [ALLOW_SNAPSHOT_ISOLATION](../../sql/t-sql/statements/alter-database-transact-sql-set-options?view=sql-server-ver16#allow_snapshot_isolation--on--off-) has to be enabled on the database before you can run the verifcation stored procedures.
