...
# Fetching Result Data
 ...
 Excess use of **SQLBindCol** to bind a result set column to a program variable is expensive because **SQLBindCol** causes an ODBC driver to allocate memory. When you bind a result column to a variable, that binding remains in effect until you either call [SQLFreeHandle](../../../2014/database-engine/dev-guide/sqlfreehandle.md) to free the statement handle or call [SQLFreeStmt](../../../2014/database-engine/dev-guide/sqlfreestmt.md) with *fOption* set to SQL_UNBIND. The bindings are not automatically undone when the statement completes.  
