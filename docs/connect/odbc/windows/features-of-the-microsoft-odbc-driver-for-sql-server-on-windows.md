...
In [!INCLUDE[ssNoVersion](../../../includes/ssnoversion_md.md)] Native Client, the `-y0` option for `sqlcmd.exe` caused output to be truncated at 1 MB if the display width was 0.
  
Beginning in the ODBC Driver 11 for [!INCLUDE[ssNoVersion](../../../includes/ssnoversion_md.md)], there is no limit on the amount of data that can be retrieved in a single column when `–y0` is specified. `sqlcmd.exe` now streams columns as large as 2 GB ([!INCLUDE[ssNoVersion](../../../includes/ssnoversion_md.md)] data type maximum).  
