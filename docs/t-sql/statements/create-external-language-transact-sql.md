...
> [!NOTE]
> **R** and **Python** are reserved names and no external language can be created with those specific names. Currently, only **Java** is supported as an external language. For more information on how to use **R** or **Python**, see [SQL Server Machine Learning Services](https://docs.microsoft.com/sql/advanced-analytics/sql-server-machine-learning-services).
