...
|max_size|**int**|Maximum file size, in 8-KB pages:<br /><br /> -1 = File will grow until the disk is full.<br /><br /> 268435456 = Log file will grow to a maximum size of 2 TB.<br /><br /> Note: Databases that are upgraded with an unlimited log file size will report -1 for the maximum size of the log file. <br /><br /> Note: If `max_size` is -1 but `growth` is 0, then no growth is allowed|  
