...
| Thread management | - `ParallelRedoThreadPool`: Instance level thread pool shared with all databases having redo work. With this, each database can take the benefit of parallel redo. The parallel redo thread pool allows multiple threads to access the same log record at the same time. In previous versions, the number of available threads for redo was limited to 100.<br /><br />- `ParallelRedoBatchRedo`: Redo of log records are batched under one latch improving speed. This improves recovery, catchup redo, and crash recovery redo. |
