...
**batch mode execution**  
*Batch mode execution* is a query processing method used to process multiple rows together, from tables where data is stored in columnar format. Batch mode execution is closely integrated with, and optimized around, the columnstore storage format. Batch mode execution is sometimes known as vector-based or vectorized execution. Queries on columnstore indexes use batch mode execution which improves query performance typically 2-4x. For more information on execution modes, refer to the [Query Processing Architecture Guide](../relational-databases/query-processing-architecture-guide.md#execution-modes). 
