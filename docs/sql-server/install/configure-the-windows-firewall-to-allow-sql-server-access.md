...
- **Automatically**: When a computer with a firewall enabled initiated communication, the firewall creates an entry int he list so that the response is allowed. This response is considered solicited traffic, and there is nothing that needs to be configured. 
