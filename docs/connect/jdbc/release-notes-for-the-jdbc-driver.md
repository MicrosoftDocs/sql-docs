...
| Allow failover partner to be tried if there is a socket timeout | Allows a reconnection attempt with a supplied failover partner following a socket timeout.  |
...
| Fixed high thread count when using findSocketUsingThreading | Fixes an issue where an uncaught exception lead to interruption in sleep calls upstream, causing retries to happen too quickly. [GitHub Issue #2104](https://github.com/microsoft/mssql-jdbc/issues/2104).|
