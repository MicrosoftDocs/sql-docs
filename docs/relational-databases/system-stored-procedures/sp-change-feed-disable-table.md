...
Removes a table from the change feed for [Synapse Link for SQL Server](/azure/synapse-analytics/synapse-link/sql-server-2022-synapse-link). For more information, see [Manage Azure Synapse Link for SQL Server](../../sql-server/synapse-link/synapse-link-sql-server-change-feed-manage.md).
...
The id of the change feed table group that the table belongs to. It is a GUID generated by the initial setup.

#### @table_id

Required. Data type is uniqueidentifier.

The id of the link table. It is the GUID generated by the setup flow and passed to the source publisher during provision. 
