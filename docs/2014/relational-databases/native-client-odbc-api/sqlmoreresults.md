...
>  A [!INCLUDE[tsql](../../includes/tsql-md.md)] SELECT statement containing a COMPUTE clause is only supported when connecting to a server version prior to [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)].  
 ...
 The developer can determine properties of the result sets columns and rows that are generated by the COMPUTE clause of a [!INCLUDE[tsql](../../includes/tsql-md.md)] SELECT statement. For more detail, see [SQLColAttribute](sqlcolattribute.md).  
