...
```  
 ...
 Server-scoped event notifications are created in the **master** database and can be queried for metadata by using the [sys.server_event_notifications](../Topic/sys.server_event_notifications%20\(Transact-SQL\).md) catalog view.  
  
 Database-scoped or object-scoped event notifications are created in the specified database and can be queried for metadata by using the [sys.event_notifications](../Topic/sys.event_notifications%20\(Transact-SQL\).md) catalog view. (You must prefix the catalog view with the corresponding database name.)  
