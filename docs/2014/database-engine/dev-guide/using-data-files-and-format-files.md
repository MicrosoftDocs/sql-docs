...
1.  Calls [bcp_init](../../../2014/database-engine/dev-guide/bcp-init.md) to specify bulk copying out (set BCP_OUT) from a table or view to a data file.  
  
2.  Calls [bcp_exec](../../../2014/database-engine/dev-guide/bcp-exec.md) to execute the bulk copy operation.  
...
2.  Call [bcp_control](../../../2014/database-engine/dev-guide/bcp-control.md) with *eOption* set to BCPHINTS and *iValue* set to a pointer to a SQLTCHAR string containing the Transact-SQL statement.  
...
3.  Call **bcp_exec** to execute the bulk copy operation.  
 ...
 To create a data file in which column data is stored in a different format than in the table, call [bcp_columns](../../../2014/database-engine/dev-guide/bcp-columns.md) to specify how many columns will be changed, then call [bcp_colfmt](../../../2014/database-engine/dev-guide/bcp-colfmt.md) for each column whose format you want to change. This is done after calling **bcp_init** but before calling **bcp_exec**. **bcp_colfmt** specifies the format in which the column's data is stored in the data file. It can be used when bulk copying in or out. You can also use **bcp_colfmt** to set the row and column terminators. For example, if your data contains no tab characters, you can create a tab-delimited file by using **bcp_colfmt** to set the tab character as the terminator for each column.  
  
 When bulk copying out and using **bcp_colfmt**, you can easily create a format file describing the data file you have created by calling [bcp_writefmt](../../../2014/database-engine/dev-guide/bcp-writefmt.md) after the last call to **bcp_colfmt**.  
  
 When bulk copying in from a data file described by a format file, read the format file by calling [bcp_readfmt](../../../2014/database-engine/dev-guide/bcp-readfmt.md) after **bcp_init** but before **bcp_exec**.  
