...
monikerRange: "=azuresqldb-current || >=sql-server-2017 || >=sql-server-linux-2017 || =azuresqldb-mi-current || =fabric-sqldb || =fabric"
---
...
This article describes how to generate actual graphical execution plans by using [!INCLUDE [ssManStudioFull](../../includes/ssmanstudiofull-md.md)]. The SQL Database Engine generates actual execution plans after the T-SQL queries or batches finish executing. The generated execution plan shows the actual query execution plan that the [!INCLUDE [ssDEnoversion](../../includes/ssdenoversion-md.md)] used to execute the queries. An actual execution plan contains runtime information, such as actual resource usage metrics and runtime warnings (if any).  

To use this feature, you must have the appropriate permissions to execute the [!INCLUDE [tsql](../../includes/tsql-md.md)] queries for which you're generating a graphical execution plan. You must also have the `SHOWPLAN` permission for all databases referenced by the query.
...
1. Enter the query for which you want to display the actual execution plan.
...
1. Pause the mouse over the logical and physical operators to view the description and properties of the operators in the displayed ToolTip, including properties of the overall execution plan, by selecting the root node operator, the `SELECT` node.
   ...
   :::image type="content" source="media/display-an-actual-execution-plan/plan-properties.png" alt-text="Screenshot from SQL Server Management Studio indicating where to right-click Properties in a plan operator." lightbox="media/display-an-actual-execution-plan/plan-properties.png" :::
