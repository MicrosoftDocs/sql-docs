...
Azure confidential VMs are available in both the [General Purpose](performance-guidelines-best-practices-vm-size.md#general-purpose) and [memory optimized](performance-guidelines-best-practices-vm-size.md#memory-optimized) VM size series. 
...
To view all SQL Server images that support confidential VMs, go to [Azure marketplace](https://ms.portal.azure.com/#view/Microsoft_Azure_Marketplace/MarketplaceOffersBlade/selectedMenuItemId/home) and filter by **Security Type = Confidential**. 

### Limitations

- Currently, only the following SQL Server image supports Azure confidential VMs: `SQL Server 2017 Enterprise on Windows Server 2019 Database Engine Only - Gen 2`. 
...
This section provides guidance for disk encryption, but the recommendations vary depending on if you're deploying an Azure confidential VM or not. 

### VMs other than Azure confidential VMs
