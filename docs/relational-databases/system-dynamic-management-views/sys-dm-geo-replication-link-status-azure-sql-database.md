...
|last_replication|**datetimeoffset**|The time when the primary replica received the acknowledgement that the last log block has been hardened by the secondary replica, based on the primary database clock. Log blocks are sent to the geo-secondary continuously, without waiting for transactions to commit on the primary. This value is available on the primary database only.|  
