...
| `'DISABLE_OPTIMIZED_NESTED_LOOP'` | Instructs the query processor not to use a sort operation (batch sort) for optimized nested loop joins when generating a query plan. This hint name is equivalent to [trace flags](../database-console-commands/dbcc-traceon-trace-flags-transact-sql.md) 2340 and 8751 together. |
