...
SECRET **='**_secret_**'**
Specifies the secret required for outgoing authentication. `SECRET` is required to import a file from Azure Blob storage. To load from Azure Blob storage into Azure Synapse Analytics or Parallel Data Warehouse, the Secret must be the Azure Storage Key. The Secret provided in Azure Synapse Analytics cannot contain more than 120 characters.
> [!WARNING]
