...
> [!NOTE]  
>  When a database is enabled for change data capture, even if the recovery model is set to simple recovery the log truncation point will not advance until all the changes that are marked for capture have been gathered by the capture process. If the capture process is not running and there are changes to be gathered, executing CHECKPOINT will not truncate the log.  
...
Delayed durability isn't supported for databases enabled with Change Data Capture (CDC).

Starting with SQL Server 2022 CU 2 and SQL Server 2019 CU 20, you may see errors when enabling Change Data Capture:
...
### Columnstore indexes
You can't enable Change Data Capture on tables with a clustered columnstore index. However, starting with SQL Server 2016, you can enable it on tables with a nonclustered columnstore index.
...
|Data size insufficient|**Error 1105** - Couldn't allocate space for object in database because the filegroup is full. Create space by deleting unneeded files, dropping objects in the filegroup, adding other files to the filegroup, or setting autogrowth on for existing files in the filegroup.|Review the recommendations and take the actions in:<br/><br/>&bull; [Manage the size of the transaction log file in SQL Server](../logs/manage-the-size-of-the-transaction-log-file.md).<br/>&bull; [Manage file space for databases in Azure SQL Database](/azure/azure-sql/database/file-space-manage).<br/>&bull; [Manage file space for databases in Azure SQL Managed Instance](/azure/azure-sql/managed-instance/file-space-manage).|
