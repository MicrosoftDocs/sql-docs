...
| `SQL_COLUMN_SEARCHABLE` | Returns `SQL_UNSEARCHABLE` for columns of type General; these columns can't be used in a `WHERE` clause.<br /><br />Returns `SQL_SEARCHABLE` for columns of type Character or Memo with `NOCPTRANS` not set; these columns can be used in a `WHERE` clause with any comparison operator.<br /><br />Returns `SQL_ALL_EXCEPT_LIKE` for all other column types; these columns can be used in a `WHERE` clause with all comparison operators except `LIKE`. |
| `SQL_COLUMN_TABLE_NAME` | Always returns an empty string. |

For more information, see [SQLColAttributes Function](../reference/syntax/sqlcolattributes-function.md) in the *ODBC Programmer's Reference*.
