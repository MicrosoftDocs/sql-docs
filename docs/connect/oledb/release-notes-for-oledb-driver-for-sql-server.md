...
| Fixed a bug in scenarios involving a sql_variant data type and non-ASCII strings. | Executing certain scenarios involving the `sql_variant` data type and non-ASCII strings may result in data corruption. For details, see: [Problems with the sql_variant data type and recovery procedure](ole-db-data-types/ssvariant-structure.md#problems-with-the-sql_variant-data-type-and-recovery-procedure).
