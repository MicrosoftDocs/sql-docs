...
1. Enable least privileges with Azure CLI.
   ...
   ```azurecli
   az sql server-arc extension feature-flag set --name LeastPrivilege --value true --resource-group <resource-group> --machine-name <machine-name>  
   ```

   For example, the following command enables least privileges for a server named `myserver` in a resource group named `myrg`:

   ```azurecli
   az sql server-arc extension feature-flag set --name LeastPrivilege --value true --resource-group myrg --machine-name myserver 
   ```
...
```azurecli
az sql server-arc extension feature-flag set --name LeastPrivilege --value false --resource-group <resource-group> --machine-name <machine-name> 
```

For example, the following command disables least privileges for a server named `myserver` in a resource group named `myrg`:

```azurecli
az sql server-arc extension feature-flag set --name LeastPrivilege --value false --resource-group myrg --machine-name myserver 
```
